...
- **Worktree subagents don't run Bash**: a code-modifying subagent only edits files inside its worktree; it does **not** run tests/validation. Run all `pnpm` / `uv run` checks in the **main process after** the branch is merged. This keeps subagent runs deterministic and removes Bash as a path-escape vector.
- **Worktree guard hook**: a `PreToolUse` hook (`.claude/hooks/guard-worktree-paths.mjs`, matcher `Edit|Write|MultiEdit|NotebookEdit|Bash`) enforces two rules **only when the session cwd is inside `.claude/worktrees/`**: (1) denies any file write whose resolved target escapes the worktree subtree (safety net against harness path-resolution bugs leaking subagent edits into the main repo); (2) denies `Bash` outright (per the no-Bash policy above). The main agent (cwd = repo root) is unaffected — it edits outside the repo (auto-memory, global config) and runs Bash freely.
