...
数据区域使用页面滚动：项目标题、范围摘要和查询工具栏在结果上方，向下滚动时它们随页面移出视野，任务或对象列表随之占据可见页面。任务表保持每页 50 行，第 50 行之后紧跟分页控件；对象和轨迹列表沿用游标加载与虚拟化，滚动的是同一个页面滚动容器，而不是表格内部的短滚动条。表头在结果仍可见时保持吸顶，宽表的横向滚动同样由页面提供。切换查询、排序、保存视图或粒度时回到结果开头；游标加载下一批时不改变当前视口位置。空结果保留原有的提示高度。
