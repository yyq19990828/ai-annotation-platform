...
**分派**先选择标注员和审核员，可保留原值或清空指派。预览列出这组任务的前后指派、可更新数量及不可用原因；确认时再次校验权限、成员和锁。任务或指派已变化时需重新预览。锁定、归档或状态不适用的任务不会被强制改写，完成后显示实际成功、跳过和失败数量。
...
缺少采集或历史证据时显示不可用或覆盖不完整，不将其当成零。首次审核事实和计时采集的技术口径见[项目成员绩效数据](../../dev/concepts/project-performance.md)。
