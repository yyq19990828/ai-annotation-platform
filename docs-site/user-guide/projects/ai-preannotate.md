...
- **运行态**：批跑**过程中**逐阶段统计实时更新并落到各阶段卡上——源阶段显检出框数，每张下游卡显运行态徽标（待运行 / 运行中 / 已完成）+「目标 / 成功 / 失败 / 几何跳过」计数，并行兄弟各一条。跑完或刷新后与终态统计一致。
...
- **谁能当父**：只有产几何的阶段（选中**框→分割**或**检测**类下游模型）才会出现「加子阶段」按钮；纯分类阶段（只写属性）是叶子，不能再挂子。链路最深 3 层（源检测 + 两层下游），到第 3 层不再显示「加子阶段」。
- **加子阶段**：在一张产几何的阶段卡下点「加子阶段」，新卡缩进显示在其下方，表示「对该阶段产出的每个框」运行。卡上方的 ASCII 摘要条实时画出整棵树（`检测(源) → 子 → 孙`）。
