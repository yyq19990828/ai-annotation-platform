...
2. 要跑批量预标的批次处于 `active` 状态；或者批次仍处于 `draft` 且尚未分派标注员与质检员——管理员可以先跑 AI 再分派人工（Issue #124）。
...
- ML Backend 状态；
- 可预标（`active` 批次与未分派人员的 `draft` 批次）、已就绪、近期失败数量；
- 最近后台任务时间。

打开图像项目后，详情页会列出可预标的批次：`active` 批次，以及尚未分派标注员与质检员的 `draft` 批次（后者带「草稿·未分派」徽标）。已分派人员或已进入标注 / 审核流程的批次不会出现在列表里。打开视频项目时，详情页进入编排画布：点输入节点选执行单位（整段序列 / 逐帧），源模型下拉据此过滤——整段序列只列 tracker，逐帧只列图像检测。你仍可进视频工作台选中轨迹后用 `Ctrl+B` 发起单轨迹追踪。
...
1. 勾选一个或多个可预标批次（`active` 批次，或未分派人员的 `draft` 批次）。
