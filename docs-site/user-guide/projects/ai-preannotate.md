...
2. 项目已从全局注册表启用至少一个可用 ML Backend，并设了项目主后端。
...
存了项目编排后，标注工作台「当前题 AI」面板会多出一个 **运行当前题（按项目编排 · N 阶段）** 入口，对正在标的这一张图按该编排跑完整流水线（见[工作台](../workbench/)）——无需回到本页、无需选批次。本页仍是定义和保存编排的唯一地方；工作台只执行、不编辑编排。重新保存即覆盖，点「清除」即移除。
