...
存了项目编排后，标注工作台「当前题 AI」面板会多出一个 **运行当前题（按项目编排 · N 阶段）** 入口，对正在标的这一张图按该编排跑完整流水线（见[工作台](../workbench/)）——无需回到本页、无需选批次。本页是**项目内**定义和保存编排的主要入口，工作台只执行、不编辑编排；重新保存即覆盖，点「清除」即移除。若想把一套编排跨项目复用，可在**全局 Pipeline 库**（`/pipelines`）里维护命名编排模板（支持私有 / 组织 / 公开三档可见范围），再应用到具体项目。
