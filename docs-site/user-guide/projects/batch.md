...
| 视频 | `video_json`、`aap_json`、`mot`、`kitti`（tracking label）、`yolo-frames-det`、`yolo-frames-seg`（逐帧分割） |
...
如果要把视频标注导成传统检测训练集，选择 **YOLO 逐帧**：系统会按项目采样网格生成 label，并附 `fetch_frames.py` 用本地 ffmpeg 抽出对应帧图。要导成分割训练集则选 **YOLO 逐帧分割**：同样按采样网格抽帧，但保留多边形顶点（bbox / polyline 跳过）。
