...
**属性字段 ·「遮挡样式」开关**（v0.11.27）：在属性 schema 里给某个 **boolean（勾选）字段** 勾上「遮挡样式」后，当该属性值为 `true` 时，工作台 / 审核界面上对应的标注框会渲染为**虚线 + 半透**，用来表达「目标被遮挡」。

- 切换方式沿用属性自带的**数字键 hotkey（1-9）**或属性面板里的勾选；图片侧旧的字母 `O` 键 / 右键「标记遮挡」已废弃。
