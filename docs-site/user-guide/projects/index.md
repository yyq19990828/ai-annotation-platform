...
图片项目可在 **项目设置 → 基本信息** 开启「原生 Raster Mask 编辑」。部署默认允许创建，项目开关仍默认关闭，只影响新的原生 Mask 写入，不会批量迁移已有 polygon。运维人员可通过创建总闸紧急停止所有新写入，而不影响存量 Mask 的安全读取。
...
其中任一条不满足时，项目开关不会绕过部署安全边界。已有 Mask 在读取总闸开启时仍可查看和诊断；未开启原生写入的旧项目继续使用 polygon 兼容提交。
