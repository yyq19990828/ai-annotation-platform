...
通过导入脚本或 API 创建数据集时，可把它声明为**时序数据集**（字段 `is_temporal`，目前没有独立的界面开关；nuScenes 转换脚本会自动声明）。声明为时序后，导入完成时如果系统没有识别出任何 scene，会直接失败并提示检查目录结构；未声明的数据集保持普通导入行为。点云 / 多模态数据集如何在导入时产生 scene，详见 [点云 / 多模态数据集导入格式](../datasets/import-formats.md)。
