...
模型列表会根据项目已启用 backend 的 `supported_trackers` 能力过滤。项目启用了真实追踪 backend 时，测试模型不会出现在列表中。
...
运行期间，轨迹卡片显示 `queued / running` 进度与取消按钮，顶栏「后台任务」和 `/ai-pre/jobs?tab=video` 保留任务记录。取消只停止后续计算；如果模型已经产出部分帧，这部分结果仍会进入候选审阅。
