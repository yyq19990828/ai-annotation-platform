...
- 顶部「AI 追踪候选」审阅条可明确切换待审作业，并勾选目标、填写起止源帧。审阅条、当前轨迹条和时间轴显示同一组目标数、帧窗口、所选未决数与全部未决数；画布只叠加当前审阅范围内的候选。
...
每次决定都绑定当前 job revision 和源轨迹版本。如果其他用户或页面已经处理候选，审阅条会刷新服务端状态并保留当前范围，请重新确认后提交。切换任务、作业或范围后，旧请求不会重新打开原选区的人工帧覆盖确认。接受落库后，关键帧来源仍标为 prediction；如果还要逐帧修订，可以在轨迹关键帧列表中继续接受、拒绝或手工调整单帧结果。
