...
![从顶部「发现目标」打开的 AI 追踪面板：作用范围为「画布级 · 新建」](../images/video-propagate/ai-tracking-panel.png)

追踪器是工作台布局面板，可停靠、组成标签、浮动或隐藏；顶部入口会显示或聚焦同一实例。隐藏或切换布局不会取消后台 job，也不会清空方向、范围、模型和种子草稿。**AI 追踪**与 **AI 单题**可以同时显示。
...
- 左上方「AI 追踪候选」胶囊在悬停后向下展开目标数、起止源帧、所选与全部待审数，以及**接受所选／拒绝所选**。通过**更多**打开完整面板，切换待审作业、勾选目标和填写起止源帧。审阅条、当前轨迹条和时间轴显示同一组目标数、帧窗口、所选未决数与全部未决数；画布只叠加当前审阅范围内的候选。
- 切帧、查看其它参考轨迹或收到候选更新时，保留当前作业的审阅范围。切换作业再返回，也会保留各自的目标与窗口。只有点击当前轨迹条的「加入审阅目标」或「替换审阅目标」，才会把参考轨迹对应的候选加入或替换到选区；没有对应候选时按钮不可用。
- 选择**接受所选**后，只有选中目标且落在帧窗口内的候选会写入。单轨延展把主目标回填所选源轨迹；多选延展把各目标回填对应源轨迹；画布级发现的目标新建轨迹。窗口外、未选目标和剩余未决候选保持不变。
- 选择**拒绝所选**只移除该范围的暂存候选，不修改现有标注。部分决定后，目标只保留仍有合法候选的交集，帧窗口不自动扩大；当前选区可能显示零个未决候选。点击剩余区间可跳到对应源帧查看，再明确调整目标或窗口继续审阅。跳转会先保护未保存绘制与 Mask 草稿。
- 刷新页面会从服务端恢复仍待审的作业与剩余候选，并以该次恢复的默认范围开始审阅。
- 人工关键帧默认受保护。选区命中人工帧时，第一次接受会被拒绝并显示二次确认；只有确认覆盖后才写入，操作同时进入审计记录。

收起面板只改变显示，不重置审阅范围、不拒绝候选，也不取消作业。正在使用 Mask 或交互式 AI 时，新到达的追踪候选不会遮住当前工具；完成或取消当前编辑并切回选择工具后，可继续审阅。

每次决定都绑定当前 job revision 和源轨迹版本。如果其他用户或页面已经处理候选，审阅条会刷新服务端状态并保留当前范围，请重新确认后提交。切换任务、作业、范围或进入另一编辑工具后，旧请求不会重新打开原选区的人工帧覆盖确认。接受落库后，关键帧来源仍标为 prediction；如果还要逐帧修订，可以在轨迹关键帧列表中继续接受、拒绝或手工调整单帧结果。
