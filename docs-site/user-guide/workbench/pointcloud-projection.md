...
2. 点左上角「**种框 ⊹**」进入种框模式（光标变十字；无标定相机不显示此入口）。
3. 在目标上**拖一个 2D 矩形**框住它，松手即生成 3D 框并自动选中——回到主视图微调（gizmo / 三视图 / 数值面板）即可。
...
> **唯一产物是 3D 框**:拖出的 2D 矩形只是种框手势,**不落库**、不产生任何 2D 标注;松手后矩形即消失。按 `Esc` 退出种框模式(再次 `Esc` 关闭放大视图)。
>
