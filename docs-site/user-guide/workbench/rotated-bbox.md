...
<DocsVideo
  ...
  caption="在标准标注布局中拖框确定中心和尺寸，选择目标类别，再拖顶部手柄调整角度。"
/>

1. 按 `W` 切到旋转框工具。图片和暂停的视频帧使用相同操作。
2. 在画布上按下鼠标 → 拖动 → 松开，先生成一个**轴对齐**矩形（角度 0）。
3. 在旋转框旁弹出的类别浮层中选择对应的目标类别。
...
- 使用选择工具（`V`）单独选中一个旋转框，再按住鼠标左键拖动框主体 → 移动位置，尺寸和角度保持不变。松手后保存，可撤销或重做；锁定、只读或同时选中多个对象时不能拖动。视频画布按真实旋转区域命中，不会把外接矩形空角当作框内。
- 列表中显示框的尺寸与当前角度。
