...
视频追踪标注用于在连续帧中维护同一目标的矩形框、轨迹和关键帧。完成后可提交质检，并按需要导出结果。

开始前，打开已加载视频的可编辑任务，并确认项目已配置所需的类别与工具。先完成第一条轨迹，再使用单帧工具、AI 或分段协同。

## 基本操作

1. 打开 `video-track` 项目的任务。
2. 用悬浮时间轴、`←` / `→` 或 `,` / `.` 定位到目标帧（详见 [播放与帧导航](./video-playback)）。
3. 暂停视频，先在左侧工具坞顶部选择「单帧」或「轨迹」，再选择该范围的工具。选择工具 `V` 始终保留；`B` 是当前帧独立矩形框，`W` 是当前帧旋转框，`F` 是当前帧关键点，`M` 是当前帧逐像素 Mask，`T` 是矩形框轨迹。按 `Esc` 取消当前绘制或回到选择工具；平移视图用右键拖拽或按住 `Space` 后拖拽画布。
4. 在当前帧完成矩形框、旋转框、关键点、多边形、折线或新 Mask 后，在紧邻几何的类别浮层里选择该工具单位的类别。
...
<DocsVideo
  ...
  caption="在同一轨迹的两个关键帧画框，切到中间帧查看插值结果；浏览插值帧不会新增关键帧。"
/>
...
<DocsVideo
  ...
  alt="视频工作台双向逐帧、播放暂停后，手动圈定货车并查看保存的轨迹"
  caption="在视频追踪布局中切到标注详情，核对帧位置，再手动画出轨迹的首个关键帧。"
/>

- 播放、逐帧、帧采样软网格与章节导航 → [视频播放、帧导航与采样](./video-playback)
- 复制后续（关键帧传播）与 AI 追踪 → [视频关键帧传播与 AI](./video-propagate)

追踪候选审阅可收起为画布左上方胶囊，悬停向下展开当前目标数、帧范围和所选候选的接受／拒绝操作；更多设置与人工帧保护说明见[AI 延展与候选审阅](./video-propagate)。

## 单帧与轨迹范围

范围选择只显示当前范围可用的工具。单帧范围包含独立几何和已启用的交互 AI；轨迹范围包含矩形框、多边形、折线与 Mask 轨迹。切换范围时，同一种几何会切到对应工具。旋转框、模板关键点和交互 AI 没有轨迹对应工具，切到轨迹范围后保留选择工具并说明原因。

新选中一条已保存轨迹时，会进入轨迹范围及其对应工具。此后按 `B`、`P` 或 `M` 会明确进入单帧矩形框、多边形或 Mask，不会因轨迹仍被选中而自动切回。Mask 正在编辑时，`B` / `E` 仍用于笔刷与橡皮。`T` 明确进入矩形框轨迹；`V` 保留最近范围，用选择工具点击真正的空白处只清除选择。

::: warning 切换范围前先处理草稿

未完成的顶点、关键点、新建拖框或待选类别草稿会在切换前提示「继续绘制」或「丢弃并切换」。继续绘制保留原工具、范围和内容。续画已有矩形框轨迹时，请先松手完成当前关键帧，再切换工具或范围。Mask 沿用保存、丢弃或继续编辑的提示，保存失败时保留稿件。新建预览旁会显示「仅当前源帧」或「新建轨迹，从当前源帧开始」；续画已有轨迹按当前轨迹的关键帧编辑处理。

从对象列表选择轨迹时，跳到轨迹首帧也会等到草稿确认后执行；选择继续绘制会留在原源帧。追踪面板的「落点选目标」或「画框选目标」同样先保护原草稿，退出种子采集后恢复原工具和范围。

:::

范围只在当前工作台会话中保留，不写入个人布局。工具较多或窗口较矮时，可从「更多」找到当前范围的其余工具；切换后激活工具仍保持可见。项目未启用某轨迹工具时，选中轨迹仍保留其身份，同时使用中性选择工具并给出原因。

## 矩形框、轨迹与关键帧

画布上缘的**当前轨迹条**会常驻显示选中轨迹的类别、短 ID、颜色、锁定状态和当前源帧（从 F0 开始）。收起或隐藏详细时间轴、折叠选中卡后，轨迹条仍可使用；没有选中轨迹时显示选择提示。

有追踪候选待审时，轨迹条还显示当前审阅作业的目标数、帧窗口、所选和全部未决数。切帧或查看其它参考轨迹保留这个范围；只有明确点击「加入审阅目标」或「替换审阅目标」才改变审阅目标。作业切换、部分决定和剩余区间导航见[候选审阅](./video-propagate#运行取消与候选审阅)。

| 当前状态   | 含义                                                                                                       |
| ---------- | ---------------------------------------------------------------------------------------------------------- |
| 关键帧     | 当前源帧有实际保存的关键帧。遮挡另行标注。                                                                 |
| 插值帧     | 矩形框、多边形或折线由两侧有效关键帧计算。可点击「补关键帧」保存当前几何，再按需微调。                     |
| 保持帧     | Mask 使用最近的可见关键帧，并显示对应 F 帧；距离相同取较早帧，不插值像素。编辑并保存后才写入当前帧关键帧。 |
| outside    | 当前帧已标记为目标消失，轨迹身份仍保留。                                                                   |
| 本帧无几何 | 当前帧没有可解析的几何，例如超出点集或框轨迹的插值范围；画布参考框不算实际关键帧。                         |

来源只显示该关键帧实际记录的人工、AI 预测或插值记录；缺少记录时显示「来源未知」，不会根据整条标注的来源猜测。计算得到的插值帧没有当前关键帧来源，也显示「来源未知」。

上一 / 下一关键帧按钮适用于四类轨迹，只跳到可见关键帧。矩形框和 Mask 可在条件满足时直接标记 outside、恢复显示或打开延展轨迹；预测产生的 Mask outside 仍需在预测审阅流程处理。锁定、只读、播放或已有绘制草稿时不提供新的写入动作。快捷键提示只列出当前已支持的操作，`K` 仍为暂停播放。

视频工作台明确区分两类对象：

左侧工具栏按顶部选择的范围显示工具：「单帧」内先列基础几何工具，再列 SAM 交互工具；「轨迹」内列各类跨帧轨迹工具。画布级 **发现目标**入口位于顶部 **AI 单题**按钮左侧，不选轨迹也能新建多个目标；它与选中卡里的 **延展此轨迹**是不同作用范围。两个 AI 面板可同时停靠、成组或浮动。项目未启用的几何或 AI 能力会被自动隐藏，空分组不会显示。

| 工具                                 | 数据                                           | 说明                                                                            |
| ------------------------------------ | ---------------------------------------------- | ------------------------------------------------------------------------------- |
| 选择 `V`                             | —                                              | 点选、移动、resize 已有视频标注，不在空白处创建新框                             |
| 矩形框 `B`                           | `video_bbox`                                   | 只属于当前 `frame_index`，其它帧不显示                                          |
| 旋转框 `W`                           | `video_rotated_bbox`                           | 拖框后用顶部手柄旋转，支持局部轴缩放；只属于当前帧                              |
| 关键点 `F`                           | `video_keypoint`                               | 按骨骼模板依次落点；`Alt` 标遮挡，右键跳过；只属于当前帧                        |
...
各类标注只读取自身工具单位的类别与属性：矩形框使用 `bbox`，旋转框使用 `rotated_bbox`，关键点使用 `keypoint`，多边形与 Mask 使用 `region`，折线使用 `polyline`。关键点单位没有骨骼模板时入口会保留但置灰。项目管理员可在**项目设置 → 类别与属性**里控制可用形态；关闭的创建工具不会出现在工具栏，选择工具始终保留。选择、移动、缩放已有标注在选择工具下明确可用，平移视图使用右键拖拽或 `Space`+拖拽。
...
- 参考框的运动预测可在设置窗口「性能与实验 › 参考框运动预测」切换四档：`关（最近关键帧）`（默认）、`恒速外推（前两关键帧）`、`卡尔曼 · 平稳`、`卡尔曼 · 灵敏`。卡尔曼档会遍历当前帧之前所有可见关键帧做前向滤波再外推到当前帧，对变速 / 转向目标比最近关键帧更贴合（平稳档更顺滑抗噪、灵敏档更紧跟最新关键帧），参考框标签相应显示「卡尔曼 F{帧}↗」，并在框外画一圈淡色虚线**误差椭圆**表示预测不确定度（椭圆越大越不确定）。该预测为实验特性，默认关闭。
- 如果某段目标消失，给当前帧标记「消失」后，插值不会跨过该帧。
- 「遮挡」用于标记目标仍存在但可见性差，画布会用虚线状态展示。
- 单个关键帧的新增、移动、消失和遮挡标记支持 `Ctrl + Z` / `Ctrl + Shift + Z` 撤销重做。
- Mask 轨迹的关键帧删除、outside / 保持状态切换以及已保存的逐帧修正也进入同一历史；当 Mask 编辑器没有未保存笔画时，`Ctrl / Cmd + Z` 与 `Ctrl / Cmd + Y` 会撤销 / 重做这些轨迹操作，有未保存笔画时则优先撤销 / 重做笔画。
...
- **当前帧**：几何指标与逐帧属性，以及复制到当前帧、复制 / 粘贴关键帧、标记消失 / 遮挡等帧级操作。状态与关键帧来源集中在画布上缘的当前轨迹条。
- **关键帧**：上一 / 下一关键帧导航 + 关键帧表（逐帧跳转、接受 / 拒绝预测帧、复制 / 转为独立框、删除关键帧）。

卡片可拖动、缩放、折叠，内容超出时滚动查看。关键帧的跳转已并入卡内关键帧表与上 / 下关键帧按钮。切到「标注详情」页签后，选中卡恢复自己的折叠状态；保留在后台的追踪配置不妨碍查看或编辑标注。
...
“当前帧”范围覆盖矩形框、多边形、折线、旋转框、关键点和 Mask 等单帧视频标注；轨迹按该帧实际可见的几何判断，Mask 轨迹也遵守复审显示模式。切换到“全部”仍可查看其他帧的条目。

从标注列表隐藏对象后，视频画布、关键帧预览和参考虚影都会遵守该隐藏状态。隐藏条目仍保留在列表中，可以恢复显示；这与轨迹列表的本地显隐设置分别生效。
...
项目管理员启用视频分段协同后，同一个视频任务会按 segment 分给多人并行标注。开始前先在顶栏选择分段；
领取成功后才能编辑，状态栏会显示 core 范围、包含 overlap 的 work 范围和租约倒计时。

顶栏较窄时，分段选择器收进“更多工具”。聚焦选择器后可使用原生键盘操作选择分段，不会触发智能切题等菜单操作。

- 每个 segment 只保存自己的 track fragment，不会覆盖相邻标注员的数据。
- 时间轴的浅色带表示可工作的 overlap，实色带表示最终归属该 segment 的 core。播放、跳帧、创建标注和 AI
  追踪都被限制在 work 范围内。
- 切换分段会释放旧租约；租约丢失或分段已完成时，工作台自动变为只读。重新选择该分段可再次领取。
- 标完点击「提交分段」。所有分段完成后任务进入审核，相邻 overlap 会自动生成 Track 边界质量报告。

协同项目一旦已有标注，就不能修改 overlap 或帧采样配置；这样两侧始终比较同一组源帧。需要返工时由审核员
重开对应分段，原边界报告会标为已过期并在重新提交后重算。

## AI 结果审阅

选中**当前帧的普通待决候选**后，按 `A` 采纳、`D` 忽略；卡片、列表和画布按钮使用同一流程。开启「决策后自动前进」时，保存成功后才选择当前帧内的下一项待决候选，不会跳到其它帧。请求失败保留候选和选择，可再次重试；模型类别不在项目标签集时，先在弹层补选项目类别，成功后才前进。按住按键不会重复提交。

人工轨迹、人工单帧对象、其它帧候选或无选中时，`A` 不执行候选决策，`D` 仍切换智能框工具。输入框、下拉框、菜单、停靠标签和类别弹层优先处理自己的按键。交互式 SAM 保留 `Enter` / `Esc` / `Tab`；追踪作业使用作业审阅操作。普通 `video_track_*` 预测仍按整个 shape 接受，当前帧限制决定 A/D 的候选范围及自动前进范围。「全部」帧列表保留显式采纳、忽略按钮；操作其它帧候选不会跳帧或推进当前选择。

备用工具键按工作台区分：图片 `Alt+2` 为多边形、`Alt+3` 为 AI 工具循环；视频 `Alt+2` 为轨迹、`Alt+3` 为选择。
...
<DocsVideo
  ...
  caption="用 Tab 选中待续写的 ghost 框，拖动后保存到同一轨迹，并自动选中下一条待续轨迹。"
/>
...
视频导出支持 Video JSON、YOLO / COCO 逐帧、MOT、KITTI、DAVIS 与 AAP JSON。单帧 OBB 和关键点在 Video JSON / AAP JSON 中保真；其它专业格式没有对应表示时，预检会明确报告不支持。Mask 轨迹在 Video JSON / AAP JSON 中保真，在 COCO 中输出标准 RLE，在 bbox-only 格式中按非空像素外接框降级，在 DAVIS 中输出 palette PNG；章节信息暂不进这些导出格式。
...
| `video_bbox`     | 单帧视频几何数组（字段名保留旧格式兼容）；bbox 保存 `bbox`，OBB 保存 `cx/cy/w/h/angle`，关键点保存完整 `{x,y,v}` 数组                                                                             |
