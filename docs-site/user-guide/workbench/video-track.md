...
6. 如果目标在当前帧消失或被遮挡，可在画布内选中卡点击「标记消失」/「标记遮挡」，右键轻点轨迹框打开菜单，或用 `O` / `Q` 快捷键；单帧 `video_bbox` 也支持右键改类 / 删除。
7. 标完后点击「提交质检」。
...
右侧**轨迹清单**聚焦「全部轨迹」的清单态操作；选中单条轨迹后的详情、关键帧与逐帧编辑改由画布内**选中卡**承载（见下文「画布内选中卡」）。
...
AI 追出的预测无需先接受就能核对，画布与右栏统一用 **violet** 表示「来自 AI」：

- **候选先可见**：检测式轨迹候选（`video_track_bbox`）在采纳前就渲染到画布，可逐帧核对再决定接受 / 拒绝，而不是接受后才看得到。
