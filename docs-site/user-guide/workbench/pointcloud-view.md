...
在当前帧选中具有跨帧轨迹的 3D 框后，打开「跨帧任务」并切到「轨迹修正」页签。轨迹修正只改变框之间的身份关系，不移动、插值或删除任何 3D 框。

- 「拆分轨迹」以当前帧为边界：当前帧及之前保留原轨迹，之后的框获得一个新的轨迹 ID。只有轨迹在后续帧仍有成员时才能拆分。
- 「合并轨迹」会列出同一 Scene 中类别相同、帧范围不重叠且当前用户可以完整编辑的候选。当前选中轨迹始终是保留的 survivor；平台不会隐式交换保留轨迹，也不会覆盖同帧冲突。
- 两种操作都必须先点「预览影响」，核对更新成员数和实际改写轨迹 ID 的成员数，再显式确认。
