...
- **ego 对齐（v0.15.1）**：scene 有 ego 轨迹时，邻帧参考框会先变换到当前帧车体坐标系再叠加——静止物的历史/未来框与当前帧重合，而不是沿车辆运动方向拖出一串影子；偏离重合位置的部分就是目标自身的真实运动。无轨迹 scene 维持原样叠加。
- 当前档位保存到个人工作台设置，刷新、换任务或换设备登录同一账号后保持。
