...
「工作台设置 → 界面布局 → 我的预设」可以把当前排列另存为命名布局，随账号保存，最多 5 组。填写名称后点“保存当前布局”；使用已有名称保存即为更新该预设，不额外占用名额。每条预设记录保存时的工作类型，只有在同类型任务中才能应用，其它类型只显示来源并禁用“应用”。预设可随时重命名或删除，删除需二次确认并立即腾出名额。紧凑布局、布局只读或账号布局尚未读取完成时，保存与应用暂时禁用。
