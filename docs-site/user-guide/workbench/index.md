...
顶部“布局”菜单可以调整任务队列、类别面板、标注详情和讨论面板；图片与视频标注还可调整当前题 AI，视频标注可调整视频追踪，点云任务可调整三视图精修和相机视图。画布可放到整个工作区的左、右、上、下边缘。外围面板可停靠在左侧、右侧或底部，也可拖到另一面板的中央组成标签。拖动分隔条调整停靠区大小；面板标题旁的菜单提供停靠、浮动和隐藏等操作。两个外围面板拖成上下共用一列时，默认平分高度；之后仍可拖动分隔条调整比例。类别列表随独立面板铺满可用高度，内容超出时在面板内滚动。相机视图通过“悬浮显示”切回逐路相机浮层，不生成独立的图库浮窗。
...
通过菜单停靠、拖放或找回面板生成新列时，初始列宽为工作区宽度的 15%，拖拽示意框同步显示该宽度（受面板最小宽度约束），之后仍可拖动分隔条调整。停靠、拖放、合并标签、浮动或隐藏面板时，其他停靠区域会保留原有尺寸，腾出或新增占用的空间由画布所在区域吸收。将两列拖成上下排列时沿用目标列宽，将两行拖成左右排列时沿用目标行高。窗口空间不足或面板内容要求更大的最小尺寸时，布局会按最小尺寸约束调整。
...
应用预设只改变布局，不改变当前工具、选中对象、任务、视频进度或标注草稿。提示中的撤销可恢复上一次替换前的布局；应用下一个预设或刷新页面后，不再保留此前的撤销点。“重置为标准布局”恢复当前工作台的标准标注布局。

隐藏面板后，可从“布局”菜单的面板列表找回；选择已经显示的面板会将其聚焦。恢复时优先回到原来的位置，原位置不可用时放到默认区域。顶部左右两个按钮分别收起或展开画布左侧、右侧的所有停靠面板，跟随实际位置，不绑定面板名称。收起会保留分栏、标签顺序、列宽和上下比例，刷新后仍可原位展开；浮窗及画布上方、下方的面板不受影响。某侧没有面板时，对应按钮禁用。通过面板列表找回收起侧的内容时会展开该侧并聚焦目标。单独隐藏面板可用标题栏 × 或标题菜单。进入评论或 Issue 时，讨论面板会先显示再切换到相应内容；隐藏讨论面板会保留尚未发送的输入。
