...
顶部左右箭头用于切换任务，悬停显示“上一个任务 / 下一个任务”；“提交”按钮将当前任务提交质检。

顶部栏始终保持单行。窗口变窄时逐步省略项目名、文件名及辅助信息，操作按钮收起文字并保留图标与悬停提示；更窄时快捷键、智能切题、主题和设置收进“更多工具”。

顶部“布局”菜单可以调整任务队列、类别面板、标注详情和讨论面板；图片与视频标注还可调整当前题 AI，视频标注可调整视频追踪，点云任务可调整三视图精修和相机视图。画布可放到整个工作区的左、右、上、下边缘。外围面板可停靠在左侧、右侧或底部，也可拖到另一面板的中央组成标签。拖动分隔条调整停靠区大小；拖动面板标签完成停靠和合并；按住 Shift 拖动标签可浮动面板，点击标签旁的 × 隐藏面板。两个外围面板拖成上下共用一列时，默认平分高度；之后仍可拖动分隔条调整比例。类别列表随独立面板铺满可用高度，内容超出时在面板内滚动。相机视图通过“悬浮显示”切回逐路相机浮层，不生成独立的图库浮窗。

每个可隐藏面板的标签标题旁常驻 **×**，点击即可隐藏该标签对应的面板，无需先切换到它。合并标签时只隐藏点击的标签，上下排列时只隐藏当前这一块，不影响同列其他面板；最后一块隐藏后回收空组。画布没有 ×。隐藏后的面板可从“布局”菜单找回，位置和业务状态继续保留。
...
隐藏面板后，可从“布局”菜单的面板列表找回；选择已经显示的面板会将其聚焦。恢复时优先回到原来的位置，原位置不可用时放到默认区域。顶部左右两个按钮分别收起或展开画布左侧、右侧的所有停靠面板，跟随实际位置，不绑定面板名称。收起会保留分栏、标签顺序、列宽和上下比例，刷新后仍可原位展开；浮窗及画布上方、下方的面板不受影响。某侧没有面板时，对应按钮禁用。通过面板列表找回收起侧的内容时会展开该侧并聚焦目标。单独隐藏面板可用标签旁的 ×。进入评论或 Issue 时，讨论面板会先显示再切换到相应内容；隐藏讨论面板会保留尚未发送的输入。
