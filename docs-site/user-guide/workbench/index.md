...
当网络断开或后端返回 5xx 时，操作会进入**IndexedDB 离线队列**（`anno.offline-queue.v1`），网络恢复后自动顺序回放（`drain`），多标签页通过 `BroadcastChannel` 同步队列状态。编辑和删除已有标注会在队列确认写入后保留本地结果；刷新或切换任务后，队列仍归属于原账号并等待该账号恢复连接后同步。底部状态栏会区分“保存中”“已保存”“仅保存在本机”和“同步失败”；离线状态可展开「离线队列抽屉」查看待回放的操作条目，并支持逐条重试。队列按当前账号隔离，切换账号不会处理其他账号的操作。
