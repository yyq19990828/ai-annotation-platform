...
顶部「布局」菜单提供标准、专注／恢复和「更多布局设置」。完整预设位于「工作台设置 → 界面布局」，配有按真实分栏比例绘制的 16:9 示意图，并按当前工作类型显示：
...
点云任务还在同一菜单提供三种工作方式：“框体精修”显示三视图并隐藏相机，“传感器融合”显示相机并隐藏三视图，“点级分割”隐藏两者。它们保留画布及其他工作台面板的位置；相机显示时沿用上次悬浮或停靠模式。

预设卡片以高亮边框和勾选标记显示当前布局；面板排列、显隐或浮窗不匹配预设时，自动高亮“自定义布局”。仅调整分隔条尺寸不会改变预设归属。“面板与高级布局”显示每个面板的当前状态，点击可显示或隐藏。

「工作台设置 → 界面布局 → 我的预设」可以把当前排列另存为命名布局，随账号保存，最多 5 组。填写名称后点“保存当前布局”；使用已有名称保存即为更新该预设，不额外占用名额。每条预设记录保存时的工作类型，只有在同类型任务中才能应用，其它类型只显示来源并禁用“应用”。预设可随时重命名或删除，删除需二次确认并立即腾出名额。紧凑布局、布局只读或账号布局尚未读取完成时，保存与应用暂时禁用。若另一设备先修改了预设，本次操作不会覆盖其结果；界面会刷新最新清单并提示重新操作。
...
项目发布指引后，工作台会显示可展开的「标注指引」。阅读正文后点击「确认已阅读」，保存成功才会同步到首页开工清单。收起浮层不会自动确认阅读；指南更新后需重新确认。若保存失败，可保留页面并点击「重试」。
