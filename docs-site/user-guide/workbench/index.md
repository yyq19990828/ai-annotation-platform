...
任务批次、显示范围、来源、离线操作和质检筛选使用统一漏斗标识与紧凑控件，并留在原面板中；筛选本身不修改标注，也不会重置编辑状态。 [筛选操作说明](/user-guide/reference/filtering)。
...
Mask、交互 AI、二次推理和追踪审阅平时只显示左上角的小胶囊，宽度仅容纳图标与关键信息。悬停、键盘聚焦或点击后，同一表面向右、向下同步展开常用操作；移开光标后收起。文本输入与候选决策也在展开区内，完整设置通过“更多”打开。展开和收起保持左上角位置不变，宽高连贯过渡。展开的设置面板内触发的应用内决策弹窗（如覆盖受保护人工关键帧的二次确认）属于同一流程：确认或取消后工具条保持展开，不会因弹窗交互而收起；点击画布等真正的外部区域才会收起。
...
拖动面板标签到画布的上 / 下 / 左 / 右边缘可停靠或分屏，拖到另一个标签上可合并为标签组，按住 Shift 拖动可浮动；拖到窗口最外侧可把面板停靠到布局边缘。停靠过程中画布内容暂停接收指针事件，松手或按 Esc 取消后立即恢复，已合并的标签组与浮动面板在后续停靠后保持不变。
...
顶部栏始终保持单行，窗口变窄时也不会互相重叠。宽度不足时逐步省略项目名、文件名、任务位置徽章等辅助信息；操作按钮收起文字、保留图标与悬停提示，前后任务、提交、通过和退回仍可通过图标和键盘访问；更窄时快捷键、智能切题、主题、版本更新、Bug 反馈和设置收进“更多工具”。收起的任务位置仍可在左侧任务队列的“当前 / 总数”中查看。
