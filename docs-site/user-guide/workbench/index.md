...
**讨论草稿**不使用标注离线队列。同一次登录、同一浏览器标签页内，文字、附件和画布批注随原任务与目标保留，切题或站内离开后返回可恢复；刷新或重新登录不保证恢复文字。正在绘制的画布批注还保留五分钟的本标签页恢复记录，按用户、项目、任务和标注隔离。有未发送内容时，浏览器会在支持的情况下提示离开风险。详见[草稿保留到什么时候](./discussion#草稿保留到什么时候)。
