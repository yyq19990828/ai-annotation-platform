...
选中一个**已落库的人工或已采纳标注**（非 AI 待审候选、非交互工具态、图片任务）时，画布顶部会浮出 **✦ 二次推理** 面板：把这个框当父框喂给下游模型，一次跑出缺失属性字段（如颜色 / 车型）或从中检出子物体（如车牌）。运行结果按项目 schema 补进选中框的 `attributes`，或作为**子框**挂到它下面，右栏「标注详情」和画布会立即更新，无需刷新。平台会按所选模型声明的输出能力选择框 / 掩膜结果；参数（阈值、可选模型档位、开集文本 prompt）在浮层里改，按 backend 记忆到当前用户。文本 prompt 支持开集词表 / 自由输入；超时或 backend 不可达时会给出可重试的明确错误。
