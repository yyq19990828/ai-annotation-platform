...
隐藏面板后，可从“布局”菜单的面板列表找回；选择已经显示的面板会将其聚焦。恢复时优先回到原来的位置，原位置不可用时放到默认区域。顶部左右两个按钮分别收起或展开画布左侧、右侧的所有停靠面板，跟随实际位置，不绑定面板名称。收起会保留分栏、标签顺序、列宽和上下比例，刷新后仍可原位展开；浮窗及画布上方、下方的面板不受影响。某侧没有面板时，对应按钮禁用。通过面板列表找回收起侧的内容时会展开该侧并聚焦目标。隐藏面板使用其标题菜单，类别面板独立开关。进入评论或 Issue 时，讨论面板会先显示再切换到相应内容；隐藏讨论面板会保留尚未发送的输入。
