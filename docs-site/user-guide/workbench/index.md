...
顶部“布局”菜单可以调整任务队列、类别面板、标注详情和讨论面板；图片与视频标注还可调整当前题 AI，视频标注可调整视频追踪，点云任务可调整三视图精修和相机视图。画布可放到整个工作区的左、右、上、下边缘。外围面板可停靠在左侧、右侧或底部，也可拖到另一面板的中央组成标签。拖动分隔条调整停靠区大小；面板标题旁的菜单提供停靠、浮动和隐藏等操作。相机视图通过“悬浮显示”切回逐路相机浮层，不生成独立的图库浮窗。

可隐藏面板的组标题栏右侧常驻 **×**，点击即可隐藏当前活动面板。合并标签时只隐藏当前标签，上下排列时只隐藏当前这一块，不影响同列其他面板；最后一块隐藏后回收空组。画布没有 ×。隐藏后的面板可从“布局”菜单找回，位置和业务状态继续保留。
...
通过菜单停靠、拖放或找回面板生成新列时，初始列宽为工作区宽度的 15%，之后仍可拖动分隔条调整。停靠、拖放、合并标签、浮动或隐藏面板时，其他停靠区域会保留原有尺寸，腾出或新增占用的空间由画布所在区域吸收。将两列拖成上下排列时沿用目标列宽，将两行拖成左右排列时沿用目标行高。窗口空间不足或面板内容要求更大的最小尺寸时，布局会按最小尺寸约束调整。

最大化画布和专注画布布局会保留各停靠区域的尺寸；恢复画布后继续使用这些尺寸。在最大化状态保存布局、刷新页面或切换到紧凑布局，也会保留恢复时所需的尺寸。

顶部「布局」菜单提供标准、专注／恢复和「更多布局设置」。完整预设位于「工作台设置 → 界面布局」，配有示意图，并按当前工作类型显示：
...
应用预设只改变布局，不改变当前工具、选中对象、任务、视频进度或标注草稿。提示中的撤销可恢复上一次替换前的布局；应用下一个预设或刷新页面后，不再保留此前的撤销点。选择“标准标注”或“重置为标准布局”都会恢复左侧任务与类别、中心画布、右侧标注详情与讨论，不保留此前的画布靠边位置。

隐藏面板后，可从「工作台设置 → 界面布局 → 面板与高级布局」的面板列表找回；选择已经显示的面板会将其聚焦。恢复时优先回到原来的位置，原位置不可用时放到默认区域。顶部左右两个按钮分别收起或展开画布左侧、右侧的所有停靠面板，跟随实际位置，不绑定面板名称。收起会保留分栏、标签顺序、列宽和上下比例，刷新后仍可原位展开；浮窗及画布上方、下方的面板不受影响。某侧没有面板时，对应按钮禁用。通过面板列表找回收起侧的内容时会展开该侧并聚焦目标。单独隐藏面板可用标题栏 × 或标题菜单。进入评论或 Issue 时，讨论面板会先显示再切换到相应内容；隐藏讨论面板会保留尚未发送的输入。
