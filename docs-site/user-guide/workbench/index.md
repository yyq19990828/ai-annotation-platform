...
- 3D 点云任务切到 Three.js 工作台：可旋转 / 平移 / 缩放查看点云、按高度上色或经标定的相机图 RGB 上色，在主 3D 视图绘制并编辑 `lidar_box_3d` 框（gizmo W/E/R 平移 / 转 Z / 缩放 + 数值面板补齐三轴朝向）；三正交视图浮层可在 3D 画布内拖动、调整尺寸、折叠并记住位置，内部支持拖边 / 拖角 / 拖方向线精修；3D 框经标定实时投影到悬浮在主视图四周的相机面板，3D↔2D 双向选中。详见 [3D 立体框标注](./3d-box)。
