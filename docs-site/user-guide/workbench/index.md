...
画布右上的 **当前题 AI** 面板是单图推理入口：调好模型 / 参数后点 **运行当前题**，只对正在标的这一张图跑一次预标，候选随即出现在「AI 待审」里。若项目在 [AI 预标注](../projects/#ai-预标注) 页保存过项目编排，面板会多出 **运行当前题（按项目编排 · N 阶段）**，对当前图按那条多阶段编排跑完整流水线（检测 → 分类 → 写属性），产出带属性的候选可在选中信息卡的「属性审阅」区改后采纳。该面板只执行、不编辑编排——编排在 AI 预标注页定义保存。
