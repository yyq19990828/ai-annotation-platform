...
Mask 工具默认显示为画布左上角的胶囊，只保留当前工具图标、笔刷半径和未保存状态点。悬停、键盘聚焦或触屏点击后展开笔刷、橡皮、套索加减和撤销；按住鼠标绘制经过时不会自动展开。点击「更多」在画布顶部居中打开完整工具栏：顶部显示状态，中间调整工具与半径，撤销/重做成组排列，底部提供退出和保存。点击画布即可收起并开始绘制，也可点收起按钮或按 Esc。展开时完整栏从胶囊位置过渡到画布顶部，收起时回到胶囊；系统开启减少动态效果时使用简短过渡。收起不会清空参数或草稿；回到画布后，滚轮调半径、Enter 保存、Esc 退出等快捷键继续沿用。键盘焦点位于胶囊时，Esc 先收起已展开的常用工具，再按 Esc 执行当前次操作（如取消预览或经未保存保护退出）；关闭完整工具栏后，焦点回到已收起的胶囊，也可直接继续按 Esc。
