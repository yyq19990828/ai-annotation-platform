...
Mask 质检的范围、状态、严重级别和规则使用统一紧凑筛选控件，仍在质检面板内即时更新，不遮挡整个画布。 [筛选操作说明](/user-guide/reference/filtering)。

> 图片项目默认开启原生 Mask 能力，图片与视频都保存逐像素 RLE；按项目关闭后，图片仍保留 polygon 兼容提交。
...
Mask 工具默认显示为画布左上角的胶囊，只保留当前工具图标、笔刷半径和未保存状态点。悬停、键盘聚焦或触屏点击后向下展开笔刷、橡皮、套索加减和撤销；按住鼠标绘制经过时不会自动展开。点击「更多」在画布顶部居中打开完整工具栏：顶部显示状态，中间调整工具与半径，撤销/重做成组排列，底部提供退出和保存。点击画布即可收起并开始绘制，也可点收起按钮或按 Esc。展开时完整栏从胶囊位置过渡到画布顶部，收起时回到胶囊；系统开启减少动态效果时使用简短过渡。收起不会清空参数或草稿；回到画布后，滚轮调半径、Enter 保存、Esc 退出等快捷键继续沿用。键盘焦点位于胶囊时，Esc 先收起已展开的常用工具，再按 Esc 执行当前次操作（如取消预览或经未保存保护退出）；关闭完整工具栏后，焦点回到已收起的胶囊，也可直接继续按 Esc。
...
5. **已有 Raster Mask 再编辑**
   ...
   - 保存顺序为读取、编辑、上传不可变内容、带版本条件更新 annotation；冲突、网络或服务暂时不可用时保留 Buffer 和笔画历史，等待手动重试，不会在后台重复提交
...
图片 Mask 普通保存成功后保留当前 Mask 工具和笔刷参数，取消结果对象的选中并开始新的空白草稿，可直接绘制下一个实例。需要继续修改刚保存的对象时，再选中它并进入「编辑 Mask」。保存失败仍保留当前草稿供重试。

## 当前阶段的主动作

工具栏只显示一个主动作，`Enter` 执行同一动作：

Mask 编辑期间，AI 能力协商失败的恢复工具条会收起，保证高级工具和保存按钮可点击。切回选择工具后可继续重试 AI 能力协商。Mask 设置与高级菜单显示在选中对象浮卡上方，避免操作按钮被遮挡。

| 当前状态       | 主动作                                     | `Esc` / 次动作                       |
| -------------- | ------------------------------------------ | ------------------------------------ |
| 有效区域预览   | **应用区域预览**，只更新像素草稿           | 取消预览，保留此前笔画               |
| 有效实例预览   | **提交 N 个实例**，一次保存整批变更        | 取消实例预览，保留编辑会话           |
| 普通像素草稿   | **保存 Mask**                              | 选择保存后退出、丢弃或继续编辑       |
| 视频像素草稿   | **保存当前帧 Mask** / **保存当前帧关键帧** | 同一未保存保护                       |
| 无像素修改     | **已保存**，按钮禁用                       | 退出编辑                             |
| 计算或保存中   | 显示进度，禁止重复提交                     | 运算可取消；保存和刷新完成前不能退出 |
| 失败或预览失效 | 恢复编辑、刷新范围或取消失效预览           | 保留草稿，先处理当前错误             |

区域“应用”成功后，主动作才变成“保存”；应用前后的持久标注保持不变。空区域结果无论使用按钮还是
`Enter`，都会先询问是否清空。恢复编辑只恢复会话，之后仍需执行保存。图片保存成功后回到框工具，视频
保存成功后回到选择工具；“已保存”表示当前编辑会话没有像素修改。

视频保持帧会注明来源帧与保存目标帧。未修改时按 `Enter` 不会新建关键帧；保存笔迹后只在当前帧写入人工
关键帧，其它帧不变。菜单、类别浮层、输入框和组合输入期间，按键交由当前控件处理。
...
- 区域填充、组件 / 孔洞、切割 / 拆分 / 合并 / 非重叠、整图形态学与标注转换会明确禁用，
  不会偷偷回退到主线程扫描全图。
- 设备内存无法容纳当前可见分块时，画布保持只读预览并提示放大局部 ROI 或更换设备；
  已产生的脏分块与撤销历史不会因为后续准入失败而丢失；即使笔刷被禁用，已有草稿仍可保存。
...
在标注列表隐藏 Mask 后，图片和视频画布会隐藏对应的 Mask 图层；列表仍保留条目，便于恢复显示。视频 Mask 的当前帧范围和复审显示模式与其他视频标注一致。
...
切换任务、范围、状态、严重级别或规则时，清除旧问题选择与尚未提交的修复预览。新范围需要重新预览；迟到的旧预览不会成为新范围的可执行计划。
...
普通尺寸的套索和区域填充先生成橙色预览，并显示变化像素数，不会立刻覆盖当前 Mask。点击「应用区域预览」或按 `Enter`
后才写入草稿，且整次操作只占一个撤销步骤；点击「取消预览」或按 `Esc` 只丢弃预览。再次按
`Esc` 会经过未保存保护再退出编辑。分块模式的套索与视口 ROI 形态学直接写入草稿和撤销历史，完成后仍需保存。
...
这些操作都先显示变化像素、面积、组件数和孔洞数的前后对比。结果为空时，「应用区域预览」还会要求第二次
确认。复制命中组件或拆分全部组件会显示蓝色实例预览及来源 / 结果数量；实例预览不会用多次单对象保存
模拟提交。

### 实例拆分、复制、合并与严格非重叠

- **复制 / 拆分组件**：在当前 Mask 上生成蓝色实例预览，点击「提交 N 个实例」后才创建新对象。
...
Buffer 都会保留；网络或服务暂时错误可用原幂等键「重试实例提交」，范围或版本冲突只提供「刷新范围」重算。只有服务端确认整批成功后，
工作台才会清除草稿并刷新标注。
...
图片中已保存、未锁定且没有活动子对象的 Raster Mask 支持直线切割。先保存当前笔刷草稿，再在 Mask 编辑的高级菜单选择「直线切割为两个实例」，拖动两个不同的点定义有向直线，松手后查看两个结果和像素数量。点击「提交 2 个实例」或按 `Enter` 一次确认；「取消预览」或 `Esc` 只丢弃本地预览。

切割按整条直线两侧的像素中心分区，切线上的像素归拖动方向的左侧，不擦除切线像素。来源可以有孔洞或多个连通分量，两块必须都非空。像素较多的一块保留来源 ID；相等时左侧保留。新对象继承类别、业务属性和原父对象，使用人工来源；来源原有身份和属性保留。

成功后退出 Mask 编辑，一次 `Ctrl+Z` 恢复整个来源并停用新对象，一次 `Ctrl+Shift+Z` 恢复相同两个 ID 和全部像素。恢复要求两个对象都没有后续修改、锁定或活动子对象；冲突、版本缺失或过期时保持现状和历史位置。提交响应丢失时保留预览，点击重试会复用原请求，避免重复创建。

服务端保留切割所需 Mask 版本至原操作后 30 天，撤销和重做不续期，期间资源清理会保留这些引用。浏览器历史仍沿用当前标签页会话和 5 分钟恢复窗口；该窗口与服务端保留期独立。视频与大画布分块模式暂不提供直线切割。
...
在视频任务中按 `M` 或点击「单帧 Mask」工具，会创建只属于当前 `frame_index` 的
`video_mask`：

1. 没有选中当前帧 Mask 时，从空白画布开始；确认类别后进入人工标注列表。
2. 选中当前帧已有 Mask 并点击「编辑 Mask」时，会读取原 RLE 并带版本条件更新，不会创建轨迹。
3. 切换到其它帧后，该对象不再显示；在新帧落笔会创建新的单帧对象。
4. 单帧 Mask 可参与当前帧的选择、`Tab` 流转、改类和删除。
...
| 键                                  | 作用                                                   |
| ----------------------------------- | ------------------------------------------------------ |
| `M`                                 | 视频：切到单帧 Mask；图片：切到 Mask 工具              |
| `B`                                 | mask 工具激活时切笔刷（涂）                            |
| `E`                                 | mask 工具激活时切橡皮（擦）                            |
| `滚轮`                              | 调笔刷半径 ±2px（仅图片 / 视频画面内，clamp [1, 200]） |
| `Enter`                             | 执行工具栏当前主动作：应用区域、提交实例、保存或恢复   |
| `Esc`                               | 先取消当前预览；像素草稿经未保存保护退出；保存中不退出 |
...
进入 Mask 工具后，画布顶部居中会出现与其它工作台控件一致的紧凑浮动工具栏；按钮、文字和图标使用
统一尺寸，从左到右依次为：

- **主工具组**：笔刷、橡皮、套索添加和套索减去互斥；笔刷与橡皮分别对应 `B` 和 `E` 快捷键。
- **笔刷光标**：只在真实图片 / 视频画面内显示，离开画面会立即恢复为系统光标；画面边缘会裁掉
  超出范围的笔刷圈部分。
- **高级菜单**：选择区域添加 / 减去、组件 / 孔洞处理、面积阈值、形态学、圆形 / 方形笔头，以及
  4 / 8 邻域连接规则。
- **半径输入**：范围 `[1, 200]` px，默认 **16px**；可直接输入或把光标放在图片 / 视频画面内，用滚轮微调（±2px/格）。
- **操作预览**：像素操作完成后显示变化像素、面积、组件和孔洞的前后指标；
  多对象实例计划使用蓝色高亮并显示来源 / 结果数量。
- **状态文字**：明确显示加载中、就绪、未保存、保存中或失败；失败时可重试或恢复编辑。
- **撤销 / 重做**：工具栏按钮与快捷键共用同一操作历史。
- **主动作 / 次动作**：从当前阶段派生，名称和下方提示说明操作会更新草稿还是持久保存。不可操作时展示原因；无修改时不触发提交。
...
- **原生 Mask 编辑仍不可用**：确认项目没有主动关闭「原生 Mask 编辑」，并检查 `region` 工具是否启用。`RASTER_MASK_CREATE_ENABLED` 是部署紧急总闸，关闭时所有项目都只读。
...
- **Mask 与 SAM 候选重叠看不清**：普通 Mask 使用工作台设置的「通用 → 填充透明度」，选中与编辑中的 Mask 使用「通用 → 选中填充透明度」；SAM 是紫色候选，可用 `Tab` 切换当前候选
