...
4. **已落库 user polygon 精修**
   ...
   - 含内环或多个外环的复杂 polygon 不进入该流程，避免 points-only 编辑把完整几何降级
...
- **多连通 / 含孔的 mask 不再有损提交**：图片提交前会同时检查前景连通分量与封闭背景孔洞。任一存在时都会保留当前 Buffer 并阻止落库，避免静默丢弃其它分量或填平孔洞。可先擦除额外区域或填平孔洞后重试；视频路径保存逐像素 RLE，不受此限制。
- **复杂 polygon 保留查看、限制直接编辑**：含孔或多连通的 polygon 会用 even-odd 正确填充，并描出外环和孔洞边界。现有顶点编辑器只承载单个外环，因此复杂 geometry 仍可选择、改类、编辑属性或删除，但画布顶点编辑、整体拖动与 Mask 精修会被禁用，防止完整 geometry 降级成单 polygon。
