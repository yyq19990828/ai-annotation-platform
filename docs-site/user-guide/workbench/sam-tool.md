...
浏览器不保存原始 logits。每次追加笔迹后，先查看画布中的更新候选，再按 `Enter` 选类确认；结果会原位更新同一 annotation，不会创建重复对象。保存完成后可直接查看新轮廓，刷新后仍保留该结果。
...
点工具栏「AI」打开当前题 AI 面板，其中有一份**由所绑定后端 `/setup.params` 自动生成的参数表单**，每个字段下方带简短说明。面板可停靠、组成标签、浮动或隐藏，布局和显隐按账号及工作类型保存。常见字段及项目级默认值：
