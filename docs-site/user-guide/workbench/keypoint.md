...
2. 按当前关键点 schema 的节点顺序在图像或暂停的视频帧上依次单击落点。视频中切换帧会取消未完成的草稿。
3. 落满模板的全部节点后，在几何旁的类别浮层确认 `keypoint` 类别并提交实例。
...
- 视频单帧关键点只在自己的 `frame_index` 显示；已跳过的 `v=0` 节点不提供拖动手柄，`Alt` + 单击已落节点在可见 `2` 与遮挡 `1` 之间切换。
...
视频单帧关键点使用 `video_keypoint`，AAP JSON 与 Video JSON 会保留 `frame_index` 和完整 `{x,y,v}` 数组。逐帧检测、分割和跟踪格式没有对应表示，导出预检会明确报告不支持。
