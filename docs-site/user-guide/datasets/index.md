...
数据类型继续在列表上方一键切换，使用统一漏斗分组和选中态；搜索与筛选分别保留。 [筛选操作说明](/user-guide/reference/filtering)。

本章覆盖数据从**进入平台**到**导出成果**的完整链路：导入原始数据 → 关联项目生成任务 → 导入 / 清理外部预测 → 导出标注成果。各环节散落的入口在这里归一。

数据集管理页可按名称和数据类型筛选，停止输入后自动查询。条件随地址栏保存，支持刷新、分享与浏览器前进后退；改变条件会收起旧的展开行。从项目跳来的数据集定位链接仍会展开目标，项目向导已经选择的数据集不会被列表搜索清空。
