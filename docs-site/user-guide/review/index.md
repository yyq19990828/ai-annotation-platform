...
将鼠标移到工作台右下角，点击十字落点按钮，再单击视频画面中的问题位置。进入落点模式会暂停播放并保留当前源帧，不吸附到采样网格；如果画面还在准备，会先显示准备状态，实际帧绘制完成后才打开表单。表单显示捕获的源帧，提交时保存该帧与相对坐标。清空 x、y 两个坐标可以改为不绑定位置的任务级 Issue。也可在讨论面板的 Issue 页点击「记录任务级问题」，无需等待视频画面就绪，该入口不提供像素坐标。

创建时会一并记录当前画布视图和时间窗；若已选中正式对象，还会记录对象与当时的版本。勾选「记录源帧范围」可用一条问题描述一段视频，例如 F120–F160。起止帧从 0 开始，必须包含落点源帧，并位于视频范围内，不受采样网格影响。

保存后，图钉显示在对应源帧，时间轴同时出现问题标记。点击问题卡片、图钉或时间轴的问题标记，会在目标帧画面就绪后恢复对象、画布视图和时间窗。讨论面板可切换到「整个项目」，定位其它视频任务中的问题；未完成的绘制和 Mask 草稿仍会先走保护流程。刷新页面或改变画布尺寸后，视图会按当前画布大小恢复。

Issue 列表的状态筛选作用于服务端查询，切换范围或状态会从第一页加载。列表与图钉入口的“已加载”数量只统计已取得的记录，继续加载可查看后续问题。

对象已删除或版本已变化时，仍会定位原问题的帧和位置，并提示「对象已变化」。无权访问目标任务时保留当前位置。旧问题继续使用原来的帧与像素位置定位。

定位成功以实际画面就绪为准，准备状态和重试提示显示在画布内，可继续点击讨论面板中的其他问题。超时、取消或视频暂不可用时会保留原锚点，点击「重试」继续定位；快速点击多个问题只响应最后一次定位，切题会取消旧任务尚未完成的定位或创建。
