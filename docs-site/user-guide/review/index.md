...
审核权限来自你在**该项目中的 `reviewer` 项目角色**，而不是账号的全局角色；同一个账号可以在 A 项目标注、在 B 项目质检，无需切换账号。**提交者不能审核自己提交的内容，也不能审核自己参与编辑过的任务**（包括后来被删除 / 撤销的编辑、批量写入、导入、AI 接受与视频 / 场景写入），依据是任务上持久化的贡献者证据，而不是当前账号身份；历史遗留且证据不完整的任务在可信回填前会拒绝审核并提示，不会按旧全局角色放行。
