...
来自人员分析的指派标注员限制会显示为筛选摘要，点击旁边的 × 可移除；项目和批次选择仍是独立导航。 [筛选操作说明](/user-guide/reference/filtering)。

> 适用角色：审核员 / 项目管理员

审核权限来自你在**该项目中的 `reviewer` 项目角色**，而不是账号的全局角色；同一个账号可以在 A 项目标注、在 B 项目质检，无需切换账号。**提交者不能审核自己提交的内容，也不能审核自己参与编辑过的任务**（包括后来被删除 / 撤销的编辑、批量写入、导入、AI 接受与视频 / 场景写入），依据是任务上持久化的贡献者证据，而不是当前账号身份；历史遗留且证据不完整的任务在单独批准的修复完成前会拒绝审核并提示（平台不附带自动回填工具），不会按旧全局角色放行。
...
切换项目、批次、标注员或账号后，列表会清除原范围的勾选和未提交退回表单。已经发出的审核操作仍按原任务完成，结果不会清空新范围的选择。

「返回全部批次」仅在已选择项目、批次或标注员筛选时显示，点击会清除这些选择。没有待审核任务且未选择范围的首页不显示此按钮。
...
单条退回不会中断同批次其他任务的审核。原标注员可在任务列表打开退回项，查看原因，点击「接受退回开始重做」后修改并重新提交；页面会在服务端确认后显示提交、通过或退回结果。

选中批次后，标题区还有「整批退回」：点击后在弹窗中填写必填原因，批次整体打回重做——已送审 / 已通过的任务回到待重做，`review_feedback` 写入批次供标注员查看，批次变为 `rejected`。整批退回同样适用于只有部分任务送审、状态仍是「标注中」的批次；退回成功后左侧批次树与批次进度会立即刷新。整批通过 / 驳回（Projects 页面的多选批量操作）仍只适用于 `reviewing` 批次。
...
「补充说明」是**可选**自由文本，建议写清楚具体目标 / 帧号，例如 `trk_person_01 frame 128: 车辆消失后仍有插值框`（各入口的退回均为两步：先选类型，再填补充说明，任一步取消都不会提交）。退回被标注员跳过的任务时，第二步的补充说明会预填「标注员跳过：原因」，可直接改写，确认后随退回一并提交给指派方。
