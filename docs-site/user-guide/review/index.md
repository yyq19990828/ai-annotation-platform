...
定位成功以实际画面就绪为准。超时、取消或视频暂不可用时会保留原锚点，点击「重试」继续定位；快速点击多个问题只响应最后一次定位，切题会取消旧任务尚未完成的定位或创建。
...
待审核队列只显示当前可见项目中可以领取或已由本人领取的任务，其他审核员已领取的任务不会进入该队列（项目负责人和超管可管理项目任务）。通过率根据当前可见项目中本人审核过且仍处于通过或退回状态的任务计算，待审核任务不进入分母。
