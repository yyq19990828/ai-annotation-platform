...
- **头像**：三种选择——上传自己的图片、从内置像素头像中挑选、或恢复默认
  - 未设置头像时显示姓名首字母
  - 上传：PNG / JPEG / WebP，不超过 2 MB；服务端会按短边自动居中裁成方形并缩放到 256×256（不会拉伸变形），随后立即生效；再传一张会替换旧图
  - 内置头像：32 个像素小人（DiceBear Pixel Art，CC0 1.0，可自由使用），选择后立即生效，可随时更换
  - 「恢复默认」清除头像回到姓名首字母
  - 头像跟随账号跨设备同步；更换后顶栏与各处成员列表同步更新
- **显示名**：可修改；提交后立即生效
- **邮箱**：只读，不可在设置页自助修改
...
按 type 切换两个开关：**接收通知（in-app）** 与 **弹出提示（toast）**。

- 关闭「接收通知」后，新事件不进入站内通知中心，也不会推送在线会话；已存档通知及其未读状态不受影响。
- 「弹出提示」只控制可见页面中的瞬时提醒；需要先开启接收才能弹出。关闭接收只禁用、不重置弹出选择，重新开启接收时恢复原选择。
- 默认仅重要类型弹出提醒（任务退回、批次驳回 / 重开 / 管理员锁定、评论提及、问题回复、后台任务失败、导出失败）；普通完成事件与未知新类型默认不弹。

修改自动保存：保存期间该类型的两个开关暂时禁用并显示「保存中…」，失败时恢复上次保存值并提供本行重试，其它类型不受影响。设置随账号同步，适用于主界面及所有标注、审核工作台，也可以在 [工作台设置 → 通知](../workbench/settings#通知) 中修改同一份配置。所有已知 type 见 [通知中心](./notifications)。
