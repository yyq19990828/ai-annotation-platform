...
6. 如果目标在当前帧消失或被遮挡，可在画布浮动条点击「消失」/「遮挡」，右键轻点轨迹框打开菜单，或用 `O` / `Q` 快捷键。
7. 标完后点击「提交质检」。
...
## 画布浮动条、右键菜单与侧栏操作（v0.10.33）

选中轨迹后，画布右上角浮动条可直接操作当前轨迹：标记 / 恢复当前帧消失、标记 / 恢复当前帧遮挡、隐藏 / 显示轨迹、锁定 / 解锁轨迹，也保留改类、复制框、拆框和删除当前关键帧入口。

右键轻点当前帧的轨迹框会在光标处打开同一组常用操作：消失、遮挡、锁定、隐藏、AI 传播、改类、拆当前帧为独立框、删除当前关键帧和删除整条轨迹。右键按住拖拽仍然是平移画布；拖拽时不会弹出菜单。
