...
| `project_admin` | 项目管理员 | 创建/管理项目；列表中可见全部启用的标注员 / 质检员和超管，但仅可编辑自己项目内的标注员 / 质检员 |
...
列表可见范围与可操作范围是两回事：项目管理员在成员列表中还能看到尚未加入其项目的启用标注员 / 质检员（与成员分配入口一致）以及启用的超级管理员（便于联系核对），但这些人所在行的操作按钮为禁用态，提示「该用户不在你管理的项目内，仅可查看」或「仅可查看：超级管理员账号」；直接调用接口同样会被拒绝。已停用账号、其他项目管理员和观察者不在此扩大范围内。
