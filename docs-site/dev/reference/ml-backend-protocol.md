...
> - **多目标落库（平台侧）**：一次追踪可返回多实例（模式 a「自动发现」）。backend 每条结果带 `instance_id`（该 job 内**全局稳定**、跨窗一致，由 backend 负责保持）；与用户种子对应的那个实例标 `primary: true`。平台 `_partition_results_by_instance` 把结果按实例分组：**主实例**（primary 标记 / 无 `instance_id` 兜底）回填源 annotation；其余每个 `instance_id` 各落一条**新 annotation**（继承源 label，`source="ai_tracker"`，新 `track_id`）。无 `instance_id` 的单实例 backend 走原路径、零回归。若多实例流未标 primary，取字典序最小 `instance_id` 归主（确定性兜底）。
