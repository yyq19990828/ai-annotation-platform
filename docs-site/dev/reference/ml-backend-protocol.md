...
> **`type=video_tracker`**：由 `VideoTrackerJob` worker 使用，gsam2 backend 接通真实 `sam2_video`。平台会按 `VIDEO_TRACKER_WINDOW_SIZE_FRAMES` 把长区间分窗，多次调用项目绑定的 connected ML Backend。请求 `task.file_path` 是视频 signed URL；`context` 包含 `model_key`（`sam2_video` / `sam3_video` / `sam3_video_interactive`）、`job_id`、`dataset_item_id`、`annotation_id`、`from_frame`、`to_frame`、`direction`、`prompt`、`source_geometry` 和（PVS）`seeds`。其中 `sam3_video_interactive` 是 SAM3 的 **PVS 交互追踪**（点/框 seed + 跨帧 memory，SAM2 式，与 `sam2_video` 同为种子驱动、caller 指定 obj_id 支持多目标），区别于 `sam3_video`（multiplex 文本开集检测）；backend 用独立的 `build_sam3_video_predictor` 加载，`/setup.supported_trackers` 声明但**不**列入 `text_driven_trackers`。响应 `result[]` 每项为 `{ frame_index, geometry, confidence?, outside?, instance_id?, primary? }`；低于平台阈值的 `confidence` 会被写成 outside prediction range。`instance_id` / `primary` 用于多目标（见下「多目标落库」）：单目标 backend 可整体省略。
>
> 落地细节：
...
> - **种子输入 `context.seeds[]`（PVS 点/多目标）**：`sam3_video_interactive` 除 `source_geometry`（单框种子，与 `sam2_video` 等价）外，还接受 `context.seeds[]`——每条 `{obj_id?, bbox?:{x,y,w,h} | points?:[[x,y,label],...]}`，坐标归一化 [0,1]，点 `label` 1=正点 / 0=负点，缺 `obj_id` 按序补 1..N。backend `_seeds_from_video_ctx` 优先 `seeds[]`（含 points、多目标各自 obj_id）于 `source_geometry` 兜底；框种子仍可只发 `source_geometry`。**平台侧**：种子从 `job.prompt.seeds` 读出，经 `TrackerContext.seeds` → adapter `context.seeds` 透传，且**只在种子窗（首窗，含原始种子帧）下发**——points 锚在种子帧，后续窗靠 `source_geometry`（上一窗末帧框）续追、不重发点种子。点种子 → PVS 逐帧 memory 追随已真机验证。
> - **跨窗续追（平台侧）**：`video_tracker_runner.py` 窗 1 用原始 keyframe seed（或 `seeds[]` 点/框种子），后续窗用上一窗末帧（非 outside）geometry 作 `source_geometry` 续追，避免每窗从首帧框重新起追导致目标漂移。多目标时只用**主实例**的几何续种（非主实例不参与源 track 的跨窗种子）。
> - **多目标落库（平台侧）**：一次追踪可返回多实例（模式 a「自动发现」）。每条结果可带 `instance_id`（对象标识）与 `primary`（与用户种子对应的实例）。平台 `_partition_results_by_instance` 按实例分组：**主实例**（`primary` 标记 / 无 `instance_id` 兜底）回填源 annotation；其余每个 `instance_id` 各落一条**新 annotation**（继承源 label，`source="ai_tracker"`，新 `track_id`）。无 `instance_id` 的单实例 backend 走原路径、零回归；未标 `primary` 时取字典序最小 `instance_id` 归主（确定性兜底）。**跨窗身份**：落库层只消费一个**全局稳定的 `instance_id`**，具体怎么算按 tracker 模型分两条腿——**multiplex（text 自动发现）**：backend 每窗独立按 text 重检测（框种子不跨帧传播），平台在窗边界帧 IoU 关联出全局 id；**PVS（点/框 seed 交互追踪，SAM2 式 memory 传播）**：backend memory 跨帧追随、caller 指定 obj_id，`instance_id` 直接是该 obj_id。两条实现随 sam3 多目标消费落地（当前 `/predict` 单实例路径已可用）。
