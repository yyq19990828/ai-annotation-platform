...
- **滚动曲线**(自绘折线图 `AxisChart`,带自适应横纵坐标,保留最近 60 个 1s 采样点):GPU 利用率 % / 显存占用 % / 缓存命中率 %。
...
把 v0.15.12 的实时曲线能力从「单设备实时」推广到「项目生产趋势 + 团队绩效」,两个只读视图(非 web 仪表盘交互):

- **📊 看板**:`client.projects.stats()` 的可见项目聚合 —— 顶部标量(总量 / 完成 / AI 率 / 待审)+ 4 条最近 12 周折线图趋势(数据总量 / 完成量 / AI 率 / 待审,自绘 `AxisChart` 带横纵坐标)。统计是周级数据,`r` 手动刷新,不做高频轮询。
