...
当前成员列表包含负责人、零活动成员和停用账号。`include_historical` 可加入存在项目活动但没有当前成员关系的用户，并标记 `is_current_member=false`；区间内持久首审快照中的贡献者也可进入列表，即使原审计已归档。活动记录不能证明历史成员角色或加入日期。
...
每日提交与通过趋势沿用成员汇总的任务去重口径，将同一任务计入所选区间内首次发生的本地日期。缺失或没有有效贡献人的轮次快照不能用于成员归属，会增加未归属审核决定数，并将标注结果归属覆盖标记为不完整；不按当前负责人补造贡献。
...
历史视频任务若在查询区间内送审但缺少对应审计，提交指标会标记覆盖不完整。撤回或重开会清空当前提交时间；对于缺少持久首审资格、且无法证明区间历史完整的旧任务，仍保守保留覆盖缺口，不根据当前状态推断完整。审核视角的总体覆盖仅受审核证据影响，不继承标注提交与首审历史的缺口。

工作流与首审事实在 PostgreSQL 内按成员、日期和驳回原因分组，并在数据库内关联区间所需的审核轮次。返回行数随聚合分组增长，不随审计明细数量增长；Python 不加载全量工作流 ORM 或任务 ID 集合。空值指标以成员 UUID 稳定排序，避免分页重复或遗漏。
...
- 客户端事件 UUID 用于幂等入库；API 与 worker 按 ID 有序获取 PostgreSQL 事务锁，在锁内比较内容后插入。同内容重试保持幂等，冲突内容不能改写原记录；同步单事件冲突返回 409，混合批次仅丢弃冲突项。异步响应表示已入队，持久化时发现的冲突由 worker 丢弃并记录原因与事件 ID。
...
任务提交或完成审核后，最后区间保留 5 分钟收尾容差。更早的缓冲区间必须落在该操作人的服务端分派或认领起始时间（含 5 分钟入场容差）与本次状态转换之间。首次提交或跳过未指派任务时，可从本人尚未过期的任务锁保留起始时间；没有有效本人锁则以提交时刻为起始，不向历史任意扩展权限。
