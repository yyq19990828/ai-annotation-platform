...
前端项目设置的归属：这些 AI 字段统一在 `ML 模型` 页维护。该页上方是项目级 AI 预标注设置（选主后端即派生启用、去重阈值），下方是**对全局 backend 的启用 / 禁用清单**——列出全局注册表里全部 backend，逐个勾选启用并设项目级阈值 / 变体覆盖（主后端只能从已启用的里选）。backend 的物理注册与健康检查归超管在模型市场维护，项目侧不注册 backend。
