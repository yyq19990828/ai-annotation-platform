...
v0.9.21 后，视频 Stage 由 `WorkbenchStageHost` 分派到 `VideoWorkbench`。视频 annotation payload 细节收口到 `stages/video/useVideoAnnotationActions.ts`，Shell 不再直接拼 `video_track` / `video_bbox` payload。
...
- 选中轨迹但当前帧无可显示 bbox 时，stage 会用最近非 `absent` 关键帧渲染虚线参考框；拖动参考框或点击「复制到当前帧」会通过同一 `upsertKeyframe` 路径创建当前帧关键帧。
