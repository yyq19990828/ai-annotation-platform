...
精确帧 pipeline（实验开关，默认关闭）只在暂停态 + 浏览器支持时工作。BUG 报告附带的 `window.__videoWorkbenchDiagnostics` 快照里 `preciseFrame` 字段会给出 `state` / `source` / `fallbackReason`、当前绘制帧、阶段耗时和资源预算与计数，按以下顺序定位：
...
9. **关闭实验开关回退**：`localStorage.removeItem('video.experimental.webcodecs')` 刷新，原生 `<video>` / 位图接管，确认标注不受影响。
