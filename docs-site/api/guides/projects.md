...
预标注保留原项目/批次入口；显式 `task_ids` 必须非空且最多 200 项，不能用空数组表示整个项目。默认（`execution_scope: "bulk"`，数据管理批量路径）所选任务必须属于项目、满足给定批次、处于 pending，批次为 active 且未被管理锁定，任务没有编辑锁。工作台「当前题 AI」传 `execution_scope: "workbench"` 与单个 `task_ids`，按当前用户对该题的编辑语义校验：允许 in_progress 任务与 draft 批次，但批次管理员锁、他人编辑锁以及 review/completed 任务仍被拒绝，拒绝原因返回中文提示。
