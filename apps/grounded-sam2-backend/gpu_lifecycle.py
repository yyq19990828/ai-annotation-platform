"""Grounded-SAM2 managed GPU lifecycle state domain.

The shared protocol package owns only wire models.  This module deliberately keeps
Grounded-SAM2's image and video pools, borrower, active-operation, replay, and fencing state
...
from pool_domain import GroundedSam2Pools, PoolId
...
class GroundedSam2GpuLifecycle:
    """Single-process managed lifecycle guard for the Grounded-SAM2 image and video pools."""

    def __init__(
        self,
        pool: GroundedSam2Pools,
        *,
    ...
    async def legacy_unload(self) -> dict[str, Any]:
        async with self._lock:
            if self._gate is LifecycleGate.ENFORCE:
                raise LifecycleHTTPError(LifecycleErrorCode.ADMISSION_DENIED)
            image_snapshot = await self._pool.snapshot_for("image")
            if self._phase != "normal":
                return {
                    "ok": True,
                    "unloaded": False,
            ...
            if self._active or self._pool_is_active(image_snapshot):
                return {
                    "ok": True,
                    "unloaded": False,
                    "loaded": image_snapshot["current_size"] > 0,
    ...
    async def _execute_legacy_unload(self) -> dict[str, Any]:
        try:
            unloaded, loaded = await self._pool.unload_legacy_image()
        ...
        async with self._lock:
            ...
            self._generation_open = False
            self._unmanaged_tainted = True
        return {"ok": True, "unloaded": unloaded, "loaded": loaded}

    async def try_idle_unload(
        self,
        pool_id: PoolId,
        *,
        idle_before: float,
        ...
        async with self._lock:
            if self._phase != "normal" or self._active:
                return 0
            snapshot = await self._pool.snapshot_for(pool_id)
        ...
        try:
            count = await self._pool.unload_idle(
                pool_id,
                idle_before=idle_before,
            )
            aggregate = await self._pool.snapshot()
        ...
        async with self._lock:
            self._phase = "normal"
            if count:
                if managed and self._pool_is_trusted_empty(aggregate):
                    self._generation_open = False
                elif not managed:
                    self._generation = None
                    self._generation_open = False
    ...
    def _build_residency_locked(self, snapshot: dict[str, Any]) -> BackendResidency:
        ...
        has_unsettled_owner = bool(
            self._active
            or builders
            or borrowers
            or snapshot["waiters"]
            or snapshot["reserved_build_slots"]
            or snapshot.get("active_sessions", 0)
        )
        if gpu_loaded is False and has_unsettled_owner:
            gpu_loaded = None
        ...
        return BackendResidency(
            ...
            pools={
                pool_id: PoolResidency(
                    resident=pool_snapshot["gpu_resident"],
                    device=pool_snapshot["device"],
                    provider=None,
                )
                for pool_id, pool_snapshot in snapshot["pools"].items()
            },
    ...
    def _drain_response_locked(
        ...
        *,
        ...
        active_requests = len(self._active)
        ready = draining and active_requests == 0 and not self._pool_is_active(snapshot)
...
__all__ = ["GroundedSam2GpuLifecycle", "WorkloadOperation"]
