"""Managed RapidOCR lifecycle fencing, replay, and cleanup contract tests."""
...
def _inspect(_engine):
    return {
        "det": _Session().get_providers(),
        "cls": _Session().get_providers(),
        "rec": _Session().get_providers(),
    }


def _resolved(name: str = "default") -> ResolvedEngine:
...
def _domain(
    *,
    ...
    pool = EnginePool(
        3,
        build_engine or (lambda _resolved: _Engine()),
        _inspect,
...
async def test_fresh_boot_reports_trusted_legacy_empty_residency() -> None:
    ...
    assert residency.pools["engines"].resident is False
...
async def test_managed_unload_keeps_generation_tombstone_until_new_allocation() -> None:
    ...
    await pool.warmup(_resolved("detector"))
    await operation.close()
...
@pytest.mark.asyncio
async def test_idle_unload_respects_age_and_borrower_then_closes_generation() -> None:
    lifecycle, pool, key = _domain()
    await _enforce(lifecycle, key)
    operation = await lifecycle.begin_workload(
        AdmissionScope.WARMUP,
...
@pytest.mark.asyncio
async def test_failed_builder_stays_active_until_owner_task_is_done(
    ...
    def build(_resolved: ResolvedEngine) -> _Engine:
        build_started.set()
        assert allow_failure.wait(timeout=1.0)
        raise RuntimeError("build failed")

    lifecycle, pool, _key = _domain(build_engine=build)
    original_commit = pool._commit_failed_build  # noqa: SLF001

    async def delayed_commit(key, current_task) -> None:
        await original_commit(key, current_task)
        commit_visible.set()
        await allow_commit_return.wait()

    monkeypatch.setattr(pool, "_commit_failed_build", delayed_commit)
    operation = await lifecycle.begin_workload(
        AdmissionScope.WARMUP,
        generation_header=None,
        token=None,
    )
    target = _resolved("failed")
    waiter = asyncio.create_task(pool.warmup(target))
    assert await asyncio.to_thread(build_started.wait, 1.0)
    allow_failure.set()
    await asyncio.wait_for(commit_visible.wait(), timeout=1.0)

    waiter.cancel()
    with pytest.raises(asyncio.CancelledError):
        await waiter
    builder = pool.builder_for_now(target)
    assert builder is not None
    assert not builder.done()
    operation.track_future(builder)
    await operation.close()

    residency = await lifecycle.residency()
    assert residency.active_requests == 1
    assert residency.builders == 1
    assert residency.state.value == "unknown"
...
async def test_shutdown_waits_for_active_operation_and_pool_borrower() -> None:
    ...
    async def use() -> None:
        async with pool.borrow(_resolved("detector")):
            entered.set()
            await release.wait()
