...
async def _create_dataset(httpx_client, token: str, name: str, data_type: str = "image") -> dict:
...
async def test_upload_zip_preserves_subdirectories(
    ...
    ds = await _create_dataset(httpx_client, token, f"pc-d1-{uuid.uuid4().hex[:6]}", "point_cloud")
    ds_id = ds["id"]
    ds_name = ds["name"]
...
async def test_upload_zip_zipslip_rejected(
    ...
    ds = await _create_dataset(httpx_client, token, f"pc-slip-{uuid.uuid4().hex[:6]}", "point_cloud")
    ds_id = ds["id"]
    ds_name = ds["name"]
...
async def test_upload_zip_cross_dir_same_name_dedup_by_hash(
    ...
    # ── 情况 A：内容不同，两条都入库 ──
    ds_a = await _create_dataset(httpx_client, token, f"dedup-a-{uuid.uuid4().hex[:6]}", "image")
    ds_a_id = ds_a["id"]
    ds_a_name = ds_a["name"]
    ...
    # ── 情况 B：内容完全相同，只入 1 条 ──
    ds_b = await _create_dataset(httpx_client, token, f"dedup-b-{uuid.uuid4().hex[:6]}", "image")
    ds_b_id = ds_b["id"]
