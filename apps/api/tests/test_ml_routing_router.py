...
async def _seed_pool_with_two_members(
    ...
    caps = {"capabilities": {"model_ids": ["sam3"], "supported_trackers": ["sam3_video"]}}
    r1 = MLBackendRegistry(
        name="sam3-a", url=f"http://sam3-a-{uuid.uuid4().hex[:6]}:9999",
        state="connected", is_interactive=True, health_meta=caps, source="manual",
        last_checked_at=datetime.now(UTC),
    )
    r2 = MLBackendRegistry(
        name="sam3-b", url=f"http://sam3-b-{uuid.uuid4().hex[:6]}:9999",
        state="connected", is_interactive=True, health_meta=caps, source="manual",
        last_checked_at=datetime.now(UTC),
    )
    db.add_all([r1, r2])
    ...
    pool = MLBackendServicePool(
        name="sam3-pool", enabled=True, routing_policy="smooth_weighted_round_robin",
        legacy_instance_id=r1.id, routing_generation=1, capability_fingerprint=fp,
        capability_snapshot=None,
    )
    db.add(pool)
    await db.flush()
    db.add_all([
        MLBackendPoolMember(pool_id=pool.id, registry_id=r1.id, traffic_state="active", weight=1),
        MLBackendPoolMember(pool_id=pool.id, registry_id=r2.id, traffic_state="active", weight=1),
    ])
...
@pytest.mark.asyncio
async def test_off_mode_returns_legacy_instance_without_lease(db_session, super_admin) -> None:
    ...
    sel = await router.acquire(pool.id, owner="test", operation="predict", project_id=proj.id)
...
@pytest.mark.asyncio
async def test_observe_returns_legacy_but_records_would_select(db_session, super_admin) -> None:
...
@pytest.mark.asyncio
async def test_observe_does_not_break_when_ledger_unavailable(db_session, super_admin) -> None:
...
@pytest.mark.asyncio
async def test_unhealthy_member_excluded_from_would_select(db_session, super_admin) -> None:
    ...
    stale = datetime.now(UTC) - timedelta(seconds=settings.ml_backend_router_health_max_age_seconds + 10)
