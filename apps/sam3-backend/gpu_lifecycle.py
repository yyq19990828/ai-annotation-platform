"""SAM3 managed GPU lifecycle state domain.

The shared protocol package owns only wire models.  This module deliberately keeps
SAM3's image, multiplex video, and PVS video pools, borrower, active-operation, replay, and fencing state
...
from pool_domain import Sam3Pools, PoolId
...
class Sam3GpuLifecycle:
    """Single-process managed lifecycle guard for the SAM3 image, multiplex video, and PVS video pools."""
    ...
    async def legacy_unload(self) -> dict[str, Any]:
        async with self._lock:
            ...
            snapshot = await self._pool.snapshot()
            if self._phase != "normal":
                return self._legacy_unload_response(snapshot, unloaded=False)
            if self._active or self._pool_is_active(snapshot):
                return self._legacy_unload_response(snapshot, unloaded=False)
            self._phase = "unloading"
    ...
    async def _execute_legacy_unload(self) -> dict[str, Any]:
        try:
            unloaded, loaded, video_loaded = await self._pool.unload_legacy_all()
    ...
    def _legacy_unload_response(
        ...
        *,
        ...
        return {
            "ok": True,
            "unloaded": unloaded,
            "loaded": pools["image"]["current_size"] > 0,
            "video_loaded": any(
                pools[pool_id]["current_size"] > 0
...
__all__ = ["Sam3GpuLifecycle", "WorkloadOperation"]
