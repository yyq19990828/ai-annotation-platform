...
const assetSpecs = [
  ...
  defineAsset({
    ...
    objective:
      "展示同一条左侧公交车轨迹纯几何复制到 F30 后逐渐偏离真实目标，撤销后用真实 SAM3 作业延展、跨帧复核候选并回填原轨迹。",
    duration: { minSeconds: 18, targetSeconds: 28, maxSeconds: 36 },
    shots: [
      "从 F0 选中与完整左侧公交车对齐的单条 bus 轨迹，同时展示“延展此轨迹”与“复制后续”。",
      "选择纯几何复制 F0→F30，连续拖动时间轴，在 F30 展示固定框与已移动公交车的明显偏离。",
      "一次撤销恢复单个人工源关键帧，返回 F0 后从同一轨迹发起真实 SAM3 延展作业。",
      "在 F0–F30 候选审阅态中连续拖动时间轴，展示 AI 框跟随同一辆公交车的位移与尺寸变化。",
      "仅采纳 F1–F30 候选，保留人工 F0，忽略剩余种子帧候选并展示回填后的 F30 轨迹。",
    ],
    editingNotes: [
      "对比必须使用同一条公交车轨迹和同一 F0→F30 范围，不能用不同目标制造伪差异。",
      "纯几何段必须显示 F30 的实际偏离；AI 段必须显示候选跟随、审阅决策与最终回填，不能只录两个入口按钮。",
