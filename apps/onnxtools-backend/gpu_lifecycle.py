"""ONNXTools managed GPU lifecycle state domain.

The shared protocol package owns only wire models.  This module deliberately keeps
ONNXTools' handle pool, borrower, active-operation, replay, and fencing state local to the single
uvicorn worker that owns the model pool.
"""
...
from handle_pool import HANDLE_ORDER, HandlePool
...
class OnnxToolsGpuLifecycle:
    """Single-process managed lifecycle guard for the ONNXTools handle pool."""
    ...
    def _build_residency_locked(self, snapshot: dict[str, Any]) -> BackendResidency:
        ...
        return BackendResidency(
            ...
            pools={
                name: PoolResidency(
                    resident=snapshot["handles"][name]["resident"],
                    device=snapshot["handles"][name]["device"],
                    provider=snapshot["handles"][name]["provider"],
                )
...
__all__ = ["OnnxToolsGpuLifecycle", "WorkloadOperation"]
