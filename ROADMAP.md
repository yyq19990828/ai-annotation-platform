...
- **跨 backend 显存互斥编排（单卡与多卡共享布局）**（**P2**）：平台可同时注册多个 GPU backend，但当前还没有跨进程显存准入。单卡上的多个 backend 会直接竞争；多卡部署也不能整机绕过治理——默认布局的卡 0 仍由 Grounded-SAM2、YOLO、ONNXTools、RapidOCR 共享，只有实际独占的卡才自然走快路径。**现状**：P0 已完成设备诊断地基与逐 `gpu_resource_id` 的 [ADR-0049](docs/adr/archive/0049-cross-backend-gpu-memory-arbitration.md)；共享 wire 及 YOLO、ONNXTools、RapidOCR、Grounded-SAM2、SAM3 的受管代码纵切已完成，其中 ONNXTools 仍等待业务模型就绪后的真实 GPU 回落验收。P2a 已落地强类型逐卡 claim、物理资源身份、预算、配置诊断与 desired/effective mode 分离；运行时 effective 仍为 `off`。**做法**：同卡按保守静态预算共驻，容量不足时保护活跃请求并按优先级加权 LRU 驱逐，不同卡的账本、锁和队列相互独立。下一步按[实施计划](docs/plans/2026-07-14-v0.22.4-cross-backend-gpu-memory-arbitration.md)完成 P2b observe 影子派发与管理展示，再于 P3+ 引入 Redis request lease、准入、驱逐和 fail-closed enforce。**触发**：单卡多 backend 部署成为常规用法，或任一物理卡出现多个 backend 共用、显存竞争误诊或部署期预算超额。
