...
- **编排「初始输入节点」重构（方向 B/C）**（**P3**，crops 立项触发）：v0.21.0 编排选择器把源阶段（`ROOT_SID`）**同时**承担「喂什么数据进来」与「第一个跑什么模型」两件事——今天能糊在一起，只因每个源恰好都是「吃整图的检测模型」。三种非平凡源扩展（video / crops / scene）都在逼近同一个正确终态：**把「输入」与「第一模型」解耦成独立的初始输入节点**（源自 v0.21.1 detect-track plan 讨论；节点 UI 去硬编码 = 方向 A 已并入 [v0.21.1 plan 前置](docs/plans/2026-07-01-v0.21.1-detection-tracking-source-stage.md)）：
  - **终态形状**：加一个**初始输入节点**声明「数据源 + 数据类型（image/video/crops/scene）+ 执行单位（task/scene）」——「源类型」这个维度的真正归宿。原「第一阶段检测」退化成有父的普通 stage，与下游一样走 `deriveDownstreamShape`（父=输入节点），画布不再 special-case 源 vs 阶段，`deriveSourceShape`（方向 A 的过渡产物）随之上移/退役。
  - **方向 B · 矩形标注框（crops）作为源 → 逼出输入节点的那个源**：crops 是「以项目里已有的矩形标注为父框来源」、**第一个算子不是模型**（零推理），故「源=第一阶段模型」根本表达不了它。这是**硬需要**输入节点的源（video 的 tracker 仍是模型、不需要；见 v0.21.1）。`usePipelineComposer` 的输入节点 payload 要能表达「无模型输入」（无 `ml_backend_id`/`model_id`，换 `source: {kind:"annotations", annotation_type:"rectangle"}`），backend 源阶段增「读已有标注而非调 backend」分支。因结构机已抽出共用，改一处两页同吃。**触发**：crops 源诉求出现（rule-of-three：客户明确要「用已有框跑下游」）——**此时一并落输入节点重构**，不要退而在模型节点上挂「源种类下拉」（那是把混淆糊回去）。
  - **方向 C · 图片序列（scene 抽帧）作为源**：见 v0.21.1 plan「不在范围」——打破 pipeline per-task 独立执行、逼执行单位从 task 升到 scene，**最贵的一块**。在输入节点模型下，「执行单位（per-task / per-scene / per-video）」是输入节点的一个字段，让用户看清逐题跑还是整场景聚合跑；方向 A 加「源类型徽标」时已预留节点头第二行槽位承接此信息。**触发**：scene 跨帧聚合标注单独立项（计划已判「最贵、单独立项」）。
