...
- Scene Track、成员包络/显式存在模式、多区间、时间角色、缺席/恢复/终止与可逆命令账本已进入 Scene 时间轴和跨帧任务中心。下一步让 3D Quality 引用稳定的 `scene_track_id + revision + interval`，并以同一命令 locator 提供断点、类别漂移和时序跳变修复；质量链闭环后再接入持久化多相机人工成员。
