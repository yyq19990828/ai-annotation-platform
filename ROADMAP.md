...
- **CSP `style-src` nonce 收紧**（P3，v0.10.11 已落基建试点：`BatchesSection.tsx` 17 处 inline → CSS modules + 文件级 `no-restricted-syntax` lint guard + [迁移指南](docs-site/dev/how-to/migrate-inline-style-to-css-modules.md)；v0.10.12 已清空 `apps/web/src/**/*.tsx` JSX `style=` / `<style>`，`no-restricted-syntax` guard 已扩到全站 TSX；待单独 PR 从 `security_headers.py` 摘 `'unsafe-inline'` 并补齐 style 标签 nonce 注入；script-src 已 v0.9.11 收紧）
...
| **P3** | CSP `style-src` nonce 收紧 sections 群续推 | v0.10.11 已落 `BatchesSection.tsx` 试点 + lint guard + 迁移指南；v0.10.12 已清空 `apps/web/src/**/*.tsx` JSX inline style 并把 guard 扩到全站 TSX；下一步单独 PR 摘 `'unsafe-inline'` | [0010](docs/adr/0010-security-headers-middleware.md) |
