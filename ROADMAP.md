...
- **巨石文件拆分 Epic 未收尾项**（计划：[Epic](docs/plans/2026-06-17-v0.16.x-large-file-component-split-epic.md) / [缓拆·补测试再拆](docs/plans/2026-06-17-v0.16.x-deferred-split-test-backfill.md) / [点云 E2E 基线](docs/plans/2026-06-17-v0.16.x-pointcloud-e2e-baseline-for-3d-split.md)）：0.16.9–0.16.13 已落主体（后端 router/service + 前端工作台 hook/纯函数拆分）与「补测试再拆」三批。**剩余三组**：
  - **3D 整簇缓拆（前置：护栏未齐，勿强拆）**：`usePsrEditor` / `usePointMask` / `usePointCloudSelection` 因共享 `sceneRef` + `form` + 合并键盘 handler 职责纠缠，已诚实缓拆。**触发**：先补下条的剩余护栏刀，覆盖该簇对应共享面后再逐个拆（拆边缘不碰 `form`/`scene` 的小块如 `usePsrPatchPipeline` 已在第 3 批完成）。
  - **点云 Playwright E2E 护栏剩余刀**（均需先扩 `seed/lidar`）：② gizmo W/E/R canvas 拖拽落库、④ point-mask polygon/lasso 绘制（需补 `point_mask` tool binding）、⑦ 跨帧 Shift+→（需补 scene/frame_index）、⑧ 相机面板拖动/折叠（需补 camera link + calibration）；usePsrEditor 的一键操作(归零/snap)/三视图浮窗 commit 同属。**已绿**：headless WebGL 冒烟 + 点选/数值编辑落库 + B 放置/Delete 删除（4 spec，SwiftShader 软渲染，`--repeat-each=3` 稳定）。
  - **`useStageViewport`（ImageStage paint 时序，翻页首帧 jank 修法核心）**：render-time setState + 双 `useLayoutEffect` 顺序 jsdom 测不到，需 Playwright 视觉基线 + 手动翻页验证，同属缓拆。
...
| **P3** | 3D 工作台整簇拆分 + 点云 E2E 护栏剩余刀（§B 测试/开发体验） | 巨石拆分主体已落 0.16.9–0.16.13;`usePsrEditor`/`usePointMask`/`usePointCloudSelection` 共享 scene/form/键盘 handler 缓拆,**先补 gizmo/point-mask/跨帧/相机 的 Playwright 护栏(需扩 seed)再拆**;`useStageViewport` 需视觉基线 | — |
