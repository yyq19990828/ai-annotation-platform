...
- **Predictions Import / AAP JSON 后续延伸**（按客户反馈触发）：
  ...
  - **COCO importer image_size_hint 参数化**（**P3**）：[`import_coco`](apps/api/app/services/predictions_import.py) 强制 `images[i].width/height` 非零，缺失整条进 errors[]。UI Wizard 加全局尺寸 hint fallback。触发：客户上传裸 annotations。
  - **AAP JSON 单 prediction 多 shape**（**P3**）：当前 `predictions[i]` 对应一条单 shape 行，与 ML backend 协议（N shape）不一致。envelope 加可选 `shapes[]`（与 flat 二选一）。触发：客户希望一次推理所有框作为同一 prediction 单元。
