...
- **ML 后端 GPU 失效 → CPU fallback 健壮性审计（剩余四镜像，P2）**：以 yolo-backend 的真实显存分配探测、失败后 latch CPU、显式 device 贯通和 `/health.provisioning.effective_device` 为参照，补齐以下镜像：
  ...
  - **统一交付**：抽一个共享探测 helper（候选落 `aap_backend_runtime`，五镜像复用）——`effective_device()` 做真实分配探测 + 失败 latch CPU；torch 系（yolo/gsam2/sam3）显式把 device 贯通到推理调用，onnxruntime 系（rapidocr/onnxtools）显式声明 provider 优先级并观测实际生效 provider；各 `/health` 统一暴露 `effective_device` / 实际 provider，供「GPU 静默退回 CPU」告警。**触发**：现在可做（yolo 已开路，四镜像照抄）。计划见 [ML backend CPU fallback 审计计划](docs/plans/2026-07-13-ml-backend-cpu-fallback-audit.md)（含 `effective_device` 平台放行 WS4，为 [ADR-0049](docs/adr/0049-cross-backend-gpu-memory-arbitration.md) 的 L0 观测地基）。
