...
- **删除已预标注 / 进行中 / 审核中批次时 task 状态的去向（待商议）**（**P3**）：当前 `BatchService.delete()` 是「解绑」语义——批次内 task 的 `batch_id` 置 NULL（老项目回收到 B-DEFAULT），但 `status` / `is_labeled` / `annotations` / AI 预标记录全部不动（配合 `Task.batch_id` 的 `ondelete=SET NULL`，v0.6.8 B-14 有意为之，避免删完批次死锁）。副作用按 task 状态分两档：
  - **`in_progress` / `pending`**：变「未归类任务」，再被 split 重分包时混进新 draft 批次，状态停留在旧批次时代。
  - **`review` / `completed` / `rejected`（已送审及之后，危害更大）**：① 孤儿化后审核**半残**——`review` 只能在 reviewer 全局队列（`dashboard.py` 按 `status==review` 查、不 join batch）看到，批次审核树看不到；标注员 / 非特权用户列表强制 `JOIN TaskBatch`（`tasks.py:192-200`），孤儿 task 对他们**完全不可见**，故 `rejected` 的 task 一旦孤儿化原标注员再也看不到、无法返工。② 被 split 重分包时 `_splittable_task_ids`（`batch.py:198-215`）**不按 status 过滤**，会把 review/completed/rejected 孤儿塞进新建 **draft** 批次 → `recalculate_counters` 计数污染（draft 批次冒出 completed/review 计数）+ 自动推进 `annotating→reviewing` 条件（无 pending/in_progress/rejected）与计数错乱。③ `export_artifacts.batch_id` 是软引用（无 FK），删后变失效引用。
  - **`pre_annotated`（AI 预标过的 task，确认 bug，可立即修）**：删批次时 `delete()`（`batch.py:361-387`）**只置 `batch_id=NULL`，不清预测**——`predictions` / `source='prediction_based'` annotation / `task.total_predictions` / `is_labeled` 全部残留。预标过但未人工动的 task 仍是 `status='pending'`，重分包后被 split 捞进新批次；再次预标时 `_run_batch` 只按 `status=='pending'` 过滤（`tasks.py:136`）、不查「已预标」，`create_from_ml_result` 无条件 INSERT（`prediction.py:276-285`，`predictions`/`annotations` 均无 `(task_id, source)` 唯一约束）→ **第二份预测叠加到残留的第一份上，同一对象重复框标**。这正是 B-33（v0.10.17）修复时点名的场景（`batch.py:854-857`：「再次跑预标时新 prediction 会与残留的 prediction_based annotation 叠加」），但 B-33 只修了 `reset_to_draft` 路径，`delete()` 仍漏。**修法明确**：`delete()` 复用 `reset_to_draft` 的预测级联清理（删 predictions/metas/jobs、软删 prediction_based annotation、归零 `total_predictions`、重算 `is_labeled`）后再解绑。
  - **待商议**：是否对「非 draft 批次」删除增加确认弹窗（说明将解绑 N 个 task 及其状态去向）；或提供「解绑 / 重置到 pending」两种行为选项（重置可复用 `reset_to_draft` 级联清理）；或让 `_splittable_task_ids` 只捞 `pending` 状态、避免重分包污染。触发：客户反馈删批次后未归类任务状态「不对劲」或孤儿 task 无法继续审核 / 返工。
