...
- **巨石文件拆分 Epic — 已结项（2026-06-22 有人值守动手收尾）**（计划：[Epic](docs/plans/2026-06-17-v0.16.x-large-file-component-split-epic.md) / [缓拆·补测试再拆](docs/plans/2026-06-17-v0.16.x-deferred-split-test-backfill.md) / [点云 E2E 基线](docs/plans/2026-06-17-v0.16.x-pointcloud-e2e-baseline-for-3d-split.md)）：0.16.9–0.16.13 已落**全部安全可拆部分** —— 后端 `tasks.py`/`dashboard.py` 拆包 + service 抽取、Tier 2 三个 ⭐⭐⭐（`media.py`/`dashboard.py`/`CapabilityCatalogPanel.tsx`）、前端工作台 hook/纯函数拆分、「补测试再拆」第 1 批后端 3 刀 + 第 2 批前端 7 刀 + 第 3 批 B 类可守护者（`usePredictionPropagation`/`usePsrPatchPipeline`/`useAiPopoverFrame`）、点云 E2E 基线 P0/P1/P2 核心 4 spec。**2026-06-22 收尾**：补全点云护栏网(gizmo W 拖拽/Q 贴合/point-mask polygon,7→8 spec)→ 抽出 `useScene`(真边界)→ 动手核实其余三组为假边界 → 顺手修了三视图 pointer-events 预存 bug。下方原「残留三组」结论更新如下:
  - **3D 整簇 —— ✅ 已抽真边界 + ✅ 证实其余为假边界**：① **`useScene`(`usePointCloudScene`)已抽出**(cut-1,commit `a7bc96bf`,−107 行;Three.js 场景生命周期整簇,唯一耦合点 gizmo→form→PATCH 走 `onTransformCommit` 回调注入,`sceneRef` 壳层持有作参数,35 处调用点零改动)——前队漏掉的真边界。② `usePsrEditor`/`usePointMask`/`usePointCloudSelection` **动手核实=假边界,定论不抽**:`form` 是选中框 PSR 的显示镜像、被 7 个操作写,抽 hook 只加 `syncDisplay` 间接不解耦(违反 Simplicity First);`usePointMask` 交互与 box 放置/选择共享同一 viewport drag + 键盘 effect,劈开=改行为;`usePointCloudSelection` 选中态本在壳层 props,无本地状态可抽。**结论从「缓拆(没敢拆)」升级为「拆过·证明假边界」,3D 整簇收口。**
  - **点云 E2E 护栏**：gizmo W 拖拽落库 / Q 一键贴合落库 / point-mask polygon(P→多边形→Enter)已补(+seed `point_mask_3d` binding + `pointmask-mode-select` testid),护栏网 **8 spec**(冒烟+点选+cx编辑+Q贴合+gizmo+B放置+Delete+三视图 overlay 可交互),`--repeat-each=3` 稳定。剩 跨帧 Shift+→ / 相机面板拖动两刀守护已抽稳件(`usePredictionPropagation`/`useCameraPanels`),非必要不补。
  - **`useStageViewport`**（ImageStage paint 时序,翻页首帧 jank 修法核心,**与 3D 无关、属图片工作台**）：render-time setState + 双 `useLayoutEffect` 顺序 jsdom 测不到。**触发**：先立 Playwright 视觉回归基线（Epic §7 非目标,需另立项）再拆。此项独立于已收口的 3D 整簇,保留为图片侧触发项。
...
| **P3** | `useStageViewport`(图片工作台 paint 时序)拆分 | 巨石拆分 Epic 已**结项**(2026-06-22):3D 整簇收口 —— `useScene` 真边界已抽,`usePsrEditor`/`usePointMask`/`usePointCloudSelection` 动手证实为假边界(显示镜像/共享 handler/状态在壳层)不抽。仅剩 `useStageViewport`(图片侧,render-time setState + 双 useLayoutEffect 顺序,需先立 Playwright 视觉基线另立项)独立保留 | — |
