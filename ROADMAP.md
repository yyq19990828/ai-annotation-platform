...
- **跨 backend 显存互斥编排（单卡与多卡共享布局）**（**P2**）：平台可同时注册多个 GPU backend，但当前只有容器级静态绑卡，没有跨进程显存准入。单卡上的多个 backend 会直接竞争；多卡部署也不能整机绕过治理——默认布局的卡 0 仍由 Grounded-SAM2、YOLO、ONNXTools、RapidOCR 共享，只有实际独占的卡才自然走快路径。**现状**：P0 已完成设备诊断地基纠偏并接受逐 `gpu_resource_id` 治理的 [ADR-0049](docs/adr/archive/0049-cross-backend-gpu-memory-arbitration.md)；P1a 已实现共享 lifecycle wire、canonical fencing 字段和 EdDSA 公钥验签契约，但五个 backend 尚未完成受管 full-pool unload 与 `residency.gpu_loaded`，物理资源身份、Redis request lease、预算、LRU 驱逐与 fail-closed enforce 也未实现，`enforce` 保持关闭。**做法**：同卡按保守静态预算共驻，容量不足时保护活跃请求并按优先级加权 LRU 驱逐，不同卡的账本、锁和队列相互独立。下一步按[实施计划](docs/plans/2026-07-14-v0.22.4-cross-backend-gpu-memory-arbitration.md)以 YOLO 完成首个受管 backend 纵切。**触发**：单卡多 backend 部署成为常规用法，或任一物理卡出现多个 backend 共用、显存竞争误诊或部署期预算超额。
