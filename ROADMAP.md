...
- **✅ 回归验证全部通过（backend 前向 + 前端-API link E2E）：`sam3_video` 文本视频追踪**（随 **v0.21.20** 已发版）：v0.21.19（sam3 文本追踪 backend：协议 text 贯通 + 前端 text UI + 能力协商 + sam3-backend `sam3.1_multiplex` video predictor）与 v0.21.20（多几何 track：polygon/polyline schema·弧长插值·画布绘制工具·导出降级·SAM2 mask→polygon 回填）两 epic 均已交付。**2026-07-10 在 RTX 3090(24GB) 完成 backend 前向回归**：warmup 冷载 sam3.1_multiplex ~20.7s / ~3.8GB；`/predict video_tracker`（tracking_car.mp4, text=car）前向峰值 **~17.5GB**——正解释本机 4060(8GB) 前向激活 OOM 的根因；逐帧输出键 `out_obj_ids`(N,)int64 / `out_binary_masks`(N,H,W)bool / `out_probs`(N,)float32 与解析假设全部吻合、归一化坐标正确（含此前修掉的 vendor `init_state` kwarg 真 bug）。**回归中发现并修复一枚真 bug**：`_pick_target_obj` 的无种子 / 空 mask 兜底误读 `output_probs`（真键是 `out_probs`）→ probs 兜底静默失效、退化成取多目标里第一个而非最高分（低危：平台主路径带 seed 走 IoU 匹配不受影响，仅纯文本无种子直检兜底生效）；已修 + 单元验证 + 真模型验证（无种子从 12/16 帧提升到 16/16 帧、挑中更显著目标）。**2026-07-10 前端-API link E2E 亦通过**：登录会话经 `POST .../tracks/{id}:propagate`（model_key=sam3_video, text=car）→ Celery runner → adapter → sam3-backend multiplex → 回填，job `completed`、种子轨迹从 1 关键帧铺满到 21 帧（F0-F20，frame0=manual、其余 prediction、bbox 随车移动、归一化正确），UI 右栏「21 关键帧 · F0-F20」如实显示；`sam2_video` seed-bbox 链零回归。**本条收口**：sam3_video 图像 + 视频在 3090 上均正常（图像 sam3.pt / 视频 sam3.1_multiplex 并存）。**显存画像（实测窗口缩放）**：视频前向峰值随窗口帧数近似线性增长——基线权重 ~5.4GB，3 帧 9.4GB / 16 帧 18.9GB / 41 帧即 OOM（顶满 24GB），约 0.85–1.3 GB/帧；主因是 tracker 把整窗每帧的 1008² backbone 特征 + 每目标 memory 常驻 GPU 做时序 memory-attention，再被 multiplex（≤16 目标槽）放大，而非权重本身。**⚠ 由此暴露一个隐患**：`SAM3_VIDEO_MAX_WINDOW_FRAMES=300` 默认对单张 24GB 卡**远超安全线**（实测 ~35 帧即触顶 OOM），应按卡显存下调窗口上限或改分块传播，否则大窗 propagate 请求会 500 OOM——待跟进。
