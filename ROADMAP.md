...
- Scene Track 与 3D Quality 已共享 `scene_track_id + revision + interval` 权威合同，并打通冻结输入、异步规则、时间轴标记、工作台定位、人工处置和讨论。下一步优先用真实质检处置数据标定规则阈值与误报率，再为持久化多相机人工成员和投影残差规则冻结真值冲突、标定版本与导出语义。
