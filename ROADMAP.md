...
- **跨 backend 显存互斥编排（单卡与多卡共享布局）**（**P2**）：平台已按稳定 `gpu_resource_id` 完成逐卡静态预算、Redis 原子准入、FIFO、空闲/忙碌驱逐、generation fencing、持久 rollout 与安全回滚；同卡按保守预算共驻，容量不足时保护活跃请求并按优先级加权 LRU 驱逐，不同卡的账本、锁和队列相互独立。五个 GPU backend 的受管生命周期、部署级能力门槛和真实派发入口均已接通，ONNXTools 参考制品也已完成四 session CUDA 推理与两轮全池显存回落验收。本机单卡、双卡并行、卡间隔离、容量拒绝和 busy victim 证据均已通过重算；真实第二宿主同号卡验收尚未完成，因此 release latch 与生产 effective `enforce` 继续关闭。完整状态与验收边界见[实施计划](docs/plans/2026-07-14-v0.22.4-cross-backend-gpu-memory-arbitration.md)。
