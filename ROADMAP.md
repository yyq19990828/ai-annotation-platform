...
- **CSP `style-src` nonce 收紧**（P3，v0.10.11 已落基建试点：`BatchesSection.tsx` 17 处 inline → CSS modules + 文件级 `no-restricted-syntax` lint guard + [迁移指南](docs-site/dev/how-to/migrate-inline-style-to-css-modules.md)；v0.10.12 已清空 `pages/Projects/sections/*.tsx`、`pages/Workbench/**/*.tsx`、`pages/Dashboard/**/*.tsx`、`components/ui/*.tsx` 组件自身、`components/shell/**/*.tsx`、`components/bugreport/**/*.tsx` 与部分 `AIPreAnnotate` jobs/history 文件 JSX `style=`；lint guard 已覆盖上述清零批次；跨全站剩 1392 处 JSX `style=`（`rg style\\s*=` 口径）；待全站迁完后单独 PR 从 `security_headers.py` 摘 `'unsafe-inline'`；script-src 已 v0.9.11 收紧）
...
| **P3** | CSP `style-src` nonce 收紧 sections 群续推 | v0.10.11 已落 `BatchesSection.tsx` 试点 + lint guard + 迁移指南；v0.10.12 已清空 Projects sections / Workbench / Dashboard / 基础 UI / shell / bugreport 与 AIPreAnnotate jobs/history；接下来迁调用方 style prop / 非 ui components 剩余子目录 / AIPreAnnotate 剩余 123 处，全部迁完后摘 `'unsafe-inline'` | [0010](docs/adr/0010-security-headers-middleware.md) |
