...
1. 计划 §10 共 **22 条**标准，在 `e91ac8dfd` 上按现有证据的草稿判定为：**满足 17 条**（有 EXEC/STATIC 证据）、**部分满足 4 条**（CI E2E 的门禁落点依赖 P9/远程 CI）、**归 P9 1 条**（默认 7/9 策略替换）。
2. 这些判定是**证据映射与状态校准**，不是 P10 收口。真正的有限缺口只有（均为已证实项，非猜测）：P9 的影子对比与门禁切换（他处进行中）、候选 SHA 的远程 CI（本仓无 push 授权）、P10 终验需要在新候选上复跑整套受影响验证并在 P9 的 `43` 就绪后重建文档。
...
| 测试文件清单                           | 1042（P0 机器可读清单）                                                                         | **1134 可执行 + 4 测试支撑 = 1138**；TSV 记录 1136                          | [26§3/§9/§10] + 本次 `git ls-files` 发现（见 §5 缺口 1）    |
| 前端单测                               | 未在基线执行                                                                                    | **562 文件 / 5626 用例**（P1 曾 5580）                                      | [41] EXEC                                                   |
| 后端测试                               | 未在基线执行                                                                                    | **4428 tests / 0 fail / 15 skipped**（`9e34dfafb`）                         | [40] EXEC；`apps/api` 在 `9e34dfafb..e91` 字节不变（REUSE） |
...
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ------ | ----------- |
| 全仓台账覆盖所有受管目录，非热点也有结论      | `docs/research/26` §2（5873 文件分类）；`docs/research/data/26-…inventory.tsv`                                                                                                         | 双向比对 0 缺口（[26§9/§10]）；TSV 为 P6 口径，缺 2 个 P8 文件（§5 缺口 1）                                                | STATIC | 满足(draft) |
...
| 后端直接用现行模型，不全局替换 ORM 构造器     | `apps/api/tests/conftest.py`、`factory.py`                                                                                                        | 43 处旧 kwargs 迁 `tool_bindings`、shim 删除（[29]）；4428 tests 通过（[40]）     | EXEC   | 满足(draft) |
...
| 引用/命令/架构文档/共享指引同步，文档构建通过                | `docs-site/dev/concepts/repository-map.md`；`docs/research/README.md`                           | docs 构建在前端通道通过（34.24s，[41]）；P9 改动后需最终重建（⚠ 见 §5）            | EXEC   | 部分        |
...
- **清单（inventory）**：机器可读 TSV，P0 1042 → P6 收口 1136；当前发现 1138（1134 可执行 + 4 支撑），TSV 缺 2 个 P8 文件（§5 缺口 1）。
...
- **REMOTE-CI（远程实际运行）**：**候选 SHA 无远程 CI 结果**（无 push）。P0 校准的 60 次运行（49/6/4/1）是**基线历史**，不是候选证据（[26§5]）。

## 5. 有限缺口（已证实项，非猜测）

1. **TSV 未随 P8 回填**：`docs/research/data/26-…inventory.tsv` 停在 P6 口径（1132 可执行 + 4 支撑 = 1136），而 `e91ac8dfd` 的确定性发现为 **1134 可执行 + 4 支撑 = 1138**；缺 `scripts/audit-e2e-requirements.test.mjs`、`scripts/summarize-e2e-results.test.mjs` 两行，且 `status` 列（ci-wired 1031 / script-wired 15 / not-wired 90）为 P6 静态口径，未反映 P8 的 `ml-cpu-test.yml` caller 与 node 测试接线。数据文件不在本轮 ownership（本轮只改 26/27/README/45）；**需在 P10 终验回填**。
2. **P9 未完成**：默认 7/9 策略替换、影子-旧策略同 SHA 对照、非零 flaky 门禁、候选发布入口均属 P9（其 `43`/`testing.md`/CI 未纳入本轮，未检查）；这是 P9 待补项。
3. **候选远程 CI 缺失**：本仓无 push 授权，所有证据为本地执行；`ci.yml` 各 job（pytest / vitest coverage / SDK / examples / docs / ML CPU / E2E 矩阵）对 `e91` 无远程结果。
4. **P10 终验未做**：需要在新候选上复跑整套受影响验证（后端全量、前端 quality、8 套 ML CPU、迁移四阶段、E2E 全矩阵 + visual/stress）并产出 before/after 记录；本矩阵只做映射，未执行这些复跑。
5. **最终 docs 构建待 P9 就绪**：`pnpm docs:build` 已在 `e91` 的前端通道通过（34.24s，[41]）；P9 修改 `43`/`testing.md`/CI 后需再构建一次，属 P10 终验。
6. **集成同跑缺失**：P10 三个验证通道在不同根上运行（后端 `9e34dfafb`、前端 `c62aa8364`、渲染 `e41e4973c`）；`e91` 作为整体候选尚未被各通道在同一 SHA 上共同执行。已按 REUSE 规则证明各通道相关子树在 `e91` 字节不变（§1/§4），但“同候选同跑”本身仍是 P10 待办。
...
- 阶段状态与确定性快照见 [26§10] 与 [27]；本文件不复制其历史结论，只做 §10 映射与状态校准。
