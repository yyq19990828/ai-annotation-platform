...
| 37   | [仓库优化 P8 CPU 测试 CI](./37-repository-optimization-p8-cpu-ci.md)                   | 可复用 `ml-cpu-test.yml` + `scripts/run-ml-cpu-tests.sh`（8 套件、每套件独立 venv；gs2/sam3 装 CPU torch，yolo 不用 torch/ultralytics）；P8 阶段产物，PR caller 未收口                                                                                                        |
...
> 注：P8 的 CI 选择与报告证据（`36-repository-optimization-p8-ci.md`）及其工作流修正仍在 P8 分支、尚未并入本树，故本索引未列出；待 P8 收口后补入。
