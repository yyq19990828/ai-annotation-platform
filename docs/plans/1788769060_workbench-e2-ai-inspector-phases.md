...
硬依赖：[E1](1788769060_workbench-e1-ai-toolbar-layers.md)
...
| 编号 | 操作与通过条件                                                                               | 当前结果 |
| ---- | -------------------------------------------------------------------------------------------- | -------- |
| E2-1 | 从无结果运行当前题，观察 idle→running→review；每阶段一个主动作，运行摘要固定为该次请求输入。 | 未执行   |
| E2-2 | 审阅时修改候选属性、隐藏再打开面板；草稿保留，接受后刷新检查属性落库。                       | 未执行   |
| E2-3 | 运行中停靠、浮动、隐藏面板；作业继续且业务实例数不增加。                                     | 未执行   |
| E2-4 | 制造失败后重试，再验证取消/切题后的迟到结果不打开新题审阅。                                  | 未执行   |
| E2-5 | 视频重复运行与审阅，单帧预测和追踪作业仍使用原执行器和各自范围。                             | 未执行   |
