...
> Status: local implementation in progress; Increment A, B1 foundations and B5 SDK/docs accepted; B2–B4 remain in review.
...
- B2 first checkpoint `c81eeb5f2` is provisionally integrated as `9d7ce03ef`: core task list, lifecycle, review, annotation and lock routes now consume project access. The main coordinator passed 64 isolated API tests, closing all three B1 workflow carry-forwards. A new coordinator-owned HTTP acceptance test also passed: one employee acquires a lock, saves and submits A's annotation, claims and approves B's review, is denied the opposite annotation/review actions and cannot read or write C despite an explicit task assignment. The final account identity remains `employee`, and A's submission retains contributor evidence. Remaining multimodal/service/batch/async/frontend work is not yet accepted; this checkpoint is not deployable on its own.
- Coordinator HTTP review-negative acceptance also passed: employee reviewer, project owner and super administrator are each denied claim/approve/reject for their own contribution (`403 self_review_denied`) or unknown evidence (`409 review_contributors_unknown`), with review state and claim fields unchanged. Six scenarios were verified in two focused runs (four ordinary/owner cases, then two super-admin cases); this does not yet qualify every multimodal review-edit producer.
- B2 remaining entrypoint wiring is split into Task `task_f6fad611af7b`, Dispatch `ctx_9cef940ee05b`, in direct child `worktree-agent-roles-b2-entry` from current parent `034c7057b`, with the same verified DeepSeek Flash/high model. It owns only ten reserved API modules and its new entrypoint regression file. The original B2 worker acknowledged the boundary and retains services, shared test fixtures and existing workflow tests. B2 stays active until both lanes are accepted. B2 checkpoints through `034c7057b`, the initial B3 aggregate/delivery candidate and B5 SDK/docs candidate are integration work in progress, not accepted feature increments; locked review-evidence, batch decisions, ongoing socket/notification revocation and documentation corrections remain required.
- Main-thread review reproduced and rejected a top-level annotation mutation bypass: employee-reviewer, employee-viewer and platform-viewer accounts could bulk-edit visible annotation-phase content. Worker correction `c61a0eef0`, integrated as `9ef827c15`, now checks the write capability; all three coordinator-owned negative regressions pass, with 31 targeted API tests passing in the same run. Fresh locked-state revalidation and direct-service/batch mutation review are still outstanding and are not implied by this result. The integrated B5 SDK follow-up separately passed all 322 SDK tests in the main checkout; final documentation/socket contract reconciliation remains pending.
- B5 scoped acceptance completed through `81e556551`: 322 SDK tests and ADR/navigation/frontmatter/snippet/freshness/changelog checks passed in the main checkout. The worker reported success, was released, and its clean integrated child was removed with its branch preserved. Final code/docs reconciliation still belongs to B6; B5 acceptance does not certify the unfinished B2–B4 implementation. The integrated frontend candidate passed main typecheck after regenerating its stale ignored client, but revocation, per-operation offline access, socket identity and role-preview races remain under review.
- B2 entry candidate `b6c0a6f80`, integrated as `c947507ab`, passed all 10 new entrypoint HTTP tests. Long-compute final-write freshness remains required. B2 visibility signatures `f881cf814`, integrated as `b02960cef`, unblock the entry worker's task-view role plumbing. The original B2 agent ended a turn with work outstanding; the old attempt was explicitly fenced after inspection, then retried as `ctx_57f87f9f0e2e` in the same session/terminal/worktree without a duplicate editor.
- After an explicit no-dirty-tests ownership acknowledgment, B2 fixture migration was split into Task `task_5624b7a468fd`, Dispatch `ctx_9513b58335d8`, in direct child `worktree-agent-roles-b2-fixtures`, based on parent `2151e9091`. Its terminal confirms OpenCode DeepSeek V4.1 Flash/high. It owns shared API fixtures and existing workflow tests; original B2 retains services/implementation, B3 retains its aggregate/delivery/worker tests, and coordinator/entry regression files remain reserved. B2 requires acceptance of all three lanes. Database, migration and runtime execution remain exclusive to the coordinator.
- B3 second follow-up through `5213f689f` yielded 106 passing and four failing tests in main: one expired test ORM field, one shared legacy fixture, and two legacy-video fixture cases. It is not accepted. Coordinator regression `72ff86004` additionally reproduces three notification REST disclosures when an async-job notification omits project scope or supplies a different accessible project; the actual job project must determine list/count/delivery authority. These failing regressions are tracked corrections, not expected failures or waived acceptance.
- Coordinator migration acceptance `2151e9091` passed in the disposable test database: two consecutive conversions preserve existing cross-project memberships, batch/task assignments, unknown contributor evidence, event history and disabled identities; absent memberships remain absent. Destructive downgrade still refuses. This is not a production snapshot-restore rehearsal, deployment, release, browser result or remote CI result; R/C remain separately gated.
