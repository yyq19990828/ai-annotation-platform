...
| 编号 | 操作与通过条件                                                                  | 当前结果 |
| ---- | ------------------------------------------------------------------------------- | -------- |
| H1-1 | 在图中央 Alt 向四个方向拖框，保存后的中心均等于 pointerdown；普通角点创建不变。 | 未执行   |
| H1-2 | 靠近图像四边拖出界，半径对称限制，中心不漂移；零移动不创建对象。                | 未执行   |
| H1-3 | 通过工具中心模式完成同样操作；拖动中松 Alt 不改变已经锁存的创建方式。           | 未执行   |
| H1-4 | 回归已有 Alt 中心缩放和 Ctrl/Cmd 选 SAM 候选；C 已交付时再验证连续中心创建。    | 未执行   |
