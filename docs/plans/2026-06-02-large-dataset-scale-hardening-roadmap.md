...
| **v0.12.0** | 建任务异步化 + 未归类池规模化（B4/B5） | 中（Celery 任务 + 前端进度 + 行为变化） | ⏳ 挂触发 | 单次 link 的 dataset > ~3 万 items，或真实用户反馈导入卡顿/超时 |
| **v0.12.1** | 导出流式化（B6） | 中（导出链路重写） | ⏳ 挂触发 | 单 project/batch 导出 task > ~3 万，或 worker OOM / 导出超时实际发生 |
