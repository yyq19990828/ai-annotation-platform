...
- 根目录中的 `draft / pending approval / approved` 才能进入实施评审；backlog 文件不能作为开工依据。发版里程碑与版本归属由维护者确定。
...
1. 确认当前 `CHANGELOG.md`、活跃 Epic 与已实施计划，重新核对范围和依赖；仅记录维护者已确认的发版里程碑，不自行分配版本，也不从草案日期或时间戳推导版本。
...
7. 将文件移动到 `docs/plans/` 根目录，遵循[命名规范](../README.md#命名规范)：已有 `<unix-seconds>_<topic>.md` 文件保留原名；旧日期或版本前缀的草案改用转定稿时的 10 位 Unix 秒级时间戳，移除版本号，并同步修正所有 Markdown 引用。状态改为待批准或已批准；实施仍需获得授权。
