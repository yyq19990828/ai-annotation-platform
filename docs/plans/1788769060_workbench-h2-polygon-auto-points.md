...
| 编号 | 操作与通过条件                                                                                  | 当前结果 |
| ---- | ----------------------------------------------------------------------------------------------- | -------- |
| H2-1 | 不同画布缩放下按相同屏幕路径 Shift 拖动，约 8 CSS px 点距；不同输入速度不改变固定距离采样语义。 | 未执行   |
| H2-2 | 松开 Shift 后补单点，Backspace 撤一点、Enter 闭合；刷新检查最终轮廓。                           | 未执行   |
| H2-3 | 零移动和重复样本不膨胀顶点；Space 平移不会写入草稿。                                            | 未执行   |
| H2-4 | 用受控长路径到达 20,000 点，页面显示暂停采样且保留草稿，可撤销/取消，不自动保存或截断。         | 未执行   |
