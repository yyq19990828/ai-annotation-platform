...
| 编号 | 操作与通过条件                                                                                     | 当前结果 |
| ---- | -------------------------------------------------------------------------------------------------- | -------- |
| E1-1 | 在图片/视频进入 Smart Point、Box、Exemplar、Scribble；高级区折叠时仍能填写当前必需输入并完成提示。 | 未执行   |
| E1-2 | 展开高级区切后端、模型和 variant 再折叠；选值保持，实际请求使用该配置，折叠不重新发起推理。        | 未执行   |
| E1-3 | 切换多实例候选、接受与取消；主层序号/总数跟随真实会话，刷新核对原生 Mask 像素。                    | 未执行   |
| E1-4 | 分别制造能力协商失败和本次推理失败；错误来源正确，必要配置入口可达。                               | 未执行   |
