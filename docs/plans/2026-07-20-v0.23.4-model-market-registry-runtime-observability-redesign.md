...
> Status: draft, ready for review; implementation blocked on the v0.23.3 release baseline · 2026-07-20
>
...
| 证据 | 当前值 | 解锁要求 |
|---|---|---|
| `V0_23_3_RELEASE_COMMIT` / tag | 待 v0.23.3 Outcome | 填入可检出的 release commit / tag |
| 部署镜像 | 待 v0.23.3 Outcome | 填入 image digest 与对应运行态版本 |
| ADR-0050 | 待 v0.23.3 Outcome | 填入已接受 ADR 路径和状态 |
| schema / migration | 待 v0.23.3 Outcome | 填入模型、迁移、singleton backfill 和双 ID 证据 |
| topology / runtime endpoints | 待 v0.23.3 Outcome | 填入 OpenAPI 路径、schema version 与实现入口 |
| 共享 router metrics | 待 v0.23.3 Outcome | 填入 Redis / 指标实现、采集窗口和 owner |
| 项目绑定和双 ID 溯源 | 待 v0.23.3 Outcome | 填入迁移、调用链和数据落点 |
| drain / resume 合同 | 待 v0.23.3 Outcome | 填入端点、状态机、审计和幂等测试 |
| 负载均衡测试与灰度 | 待 v0.23.3 Outcome | 填入命令、日期、环境、off → observe → enforce 与 0 失败结果 |

只有每行都从“待 v0.23.3 Outcome”改为可复核证据，并由 v0.23.3 实现 owner 与本计划 owner 完成评审，才能把页头的 `implementation blocked` 改为 `implementation ready`。解锁提交记录最终字段名、端点和兼容差异；任何生产 UI PR 必须以 v0.23.3 release commit 为祖先。
