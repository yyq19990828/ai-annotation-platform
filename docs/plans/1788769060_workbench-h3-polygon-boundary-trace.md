...
| 编号 | 操作与通过条件                                                                  | 当前结果 |
| ---- | ------------------------------------------------------------------------------- | -------- |
| H3-1 | 在凹 Polygon 同一边界选起止点，默认较短弧；切换另一条路径、确认，草稿准确追加。 | 未执行   |
| H3-2 | 跨首尾顶点选择路径，没有重复端点；Backspace 逐点撤，取消预览不改来源对象。      | 未执行   |
| H3-3 | 预览打开后通过第二测试会话修改/删除来源，再确认时拒绝失效预览。                 | 未执行   |
| H3-4 | 含孔、multi_polygon、同点起止等情况显示原因，不能降级为单环保存。               | 未执行   |
