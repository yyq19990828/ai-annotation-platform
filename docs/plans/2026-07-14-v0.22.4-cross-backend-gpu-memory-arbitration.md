...
- [ ] P5c-IV-A-II 建立非生产验收 runner、机器可读证据格式、稳定显存采样阈值与软件故障注入。
...
A-II 验收 runner、B 本机单卡/双卡实测与 C 真实跨宿主验收仍未完成。
