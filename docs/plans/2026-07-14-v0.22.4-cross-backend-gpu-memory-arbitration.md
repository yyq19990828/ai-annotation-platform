...
剩余工作：P4b-II 接入 runtime authority，把 Redis admission/lease/heartbeat/
...
effective enforce / gate promotion 前还必须关闭 backend 重启降回 legacy 后的旁路：只有 generation/token 两个 header 都缺失时才允许
legacy workload；任一受管 header 出现但验签、boot、scope 或 generation 不匹配都必须 fail-closed，不能降级成
unmanaged 请求继续加载显存。
