...
> 版本：v0.22.4 · 状态：P6-I–P6-III 与其余 P0–P5c-IV-B 计划项已完成；P1c ONNXTools 真实 GPU 回落验收与 P5c-IV-C 真实第二宿主验收待完成，生产 release latch 继续关闭 · 2026-07-16
>
...
- 镜像内验收器已补齐：强制核对检测/属性模型的批准 SHA-256 与审批引用，执行真实
  ...
  JSON 证据。当前主机没有完全一致或已明确批准等价的属性模型，因此本项仍待执行，
  不变更 capability 默认值与发布闩锁。
...
demotion 接入多卡 health worker。当前 latch
默认关闭，因此没有提前改变生产 effective mode。
ONNXTools 仍保持 non-evictable，
平台 enforce 继续关闭。
