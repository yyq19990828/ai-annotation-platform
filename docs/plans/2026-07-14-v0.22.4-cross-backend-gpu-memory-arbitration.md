...
> 版本：v0.22.4 · 状态：P0、P1a、P1b、P1d、P1e 已完成 · P1c ONNXTools 代码纵切已实现，真实 GPU 回落验证待模型就绪 · SAM3 与仲裁主体待实施 · 2026-07-15
>
...
P1a 已实现共享 wire 与验签契约，P1b/P1c/P1d/P1e 已分别完成 YOLO、ONNXTools、RapidOCR 与
Grounded-SAM2 受管纵切。ONNXTools 仍需在模型就绪后完成真实 GPU 回落验证；RapidOCR 与
Grounded-SAM2 已按各自参考镜像、模型摘要与物理 GPU 冻结 load/LRU/unload 证据，部署门槛继续默认
opt-in。SAM3 的 image/multiplex/PVS 多池 residency 与全量 unload 尚待实现；
Redis 账本、静态 claim、observe/enforce、驱逐与多卡灰度也未开始。下一步处理 SAM3 多池聚合；
