...
P3c-2b-II-B2b 仍须在消费时严格校验
`probe_started_at > token_expiry_high_water`、证据 TTL、当前 fence/membership 与完整 residency，并在相同
membership→fence 锁内调用上述 commit；任何 HTTP 探活都必须发生在锁外。P3c-2b-III 仍需完成
...
Redis 两阶段 proof reset、prepared 全操作冻结、重启恢复 context 与 exact commit 重试复验；P3c-2b-II-B2b/III
继续负责 token horizon 的锁内消费、两阶段 tombstone GC、逐资源观测与
...
- [ ] P3c-2b-II-B2b 接通 token horizon 的锁内证明消费、严格 residency 解析与数据库锁内 Redis commit。
...
两阶段 proof reset；仍需 P3c-2b-II-B2b/III 的 token horizon 锁内消费、两阶段 GC、观测与 worker 才能形成
