...
> 版本：v0.22.4 · 状态：P0、P1a、P1b 已完成 · P1c ONNXTools 代码纵切已实现，真实 GPU 回落验证待模型就绪 · 其余 backend 与仲裁主体待实施 · 2026-07-14
>
...
P1a 已实现共享 wire 与验签契约，P1b 已完成 YOLO 首个受管纵切，P1c 已完成 ONNXTools 代码纵切。
ONNXTools 仍需在模型就绪后完成真实 GPU 回落验证；Grounded-SAM2、SAM3、RapidOCR 的受管 residency / unload
仍待实现，Redis 账本、静态 claim、observe/enforce、驱逐与多卡灰度也未开始。下一步先完成 RapidOCR 池所有权
与 lifecycle，再处理 Grounded-SAM2 / SAM3 多池聚合；在 P1 全部验收前继续保持平台 enforce 关闭。
