...
- [ ] P5c-III 在 Redis transition owner 上原子冻结 cancel/unload 唯一分支，再接通 cancellation-safe 的 lease-zero + backend drain-health 双域有界等待、真实 RESUME 与 owner 收尾。
...
busy victim 有界等待与实物多卡灰度仍属于后续阶段。P4a-I 已交付统一派发接缝；P4a-II-A 已建立仅由 API/通用 worker/GPU
...
固定 ticket TTL、超时/取消精确清票和完整资源分片保证等待有界且不跨卡串行。busy victim drain/cancel
尚未接入 authority；P5c-I-A 只先交付了保留旧 lease 的 Draining begin、零 lease Unloading、
...
五 backend 原 operation 绑定。真实 RESUME、Redis cancel/unload branch fence、authority 双域有界等待和
实物多卡验收仍未开启。
