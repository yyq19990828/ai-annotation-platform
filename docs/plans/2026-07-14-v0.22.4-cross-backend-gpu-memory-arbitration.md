...
> 版本：v0.22.4 · 状态：P0、P1a、P1b、P1d、P1e、P1f 已完成 · P1c ONNXTools 代码纵切已实现，真实 GPU 回落验证待模型就绪 · P2a/P2b 静态 claim、配置诊断、管理端与 observe 影子派发已完成 · P3a/P3b durable fence 与 Redis 原子账本、P3c-1 Redis 安全重建原语已完成 · P3c-2 权威 membership/tombstone 与 repair worker、P4–P6 待实施 · 2026-07-15
>
...
也不能替代显式、有界的 key domain。现有 `gpu_backend_fences.backend_registry_id` 使用级联删除，不能承担
tombstone 职责；P3c-2 必须增加独立 durable membership/tombstone、规定“先持久化 membership，再允许创建
Redis child”的事务顺序，并由 bootstrap/repair worker 组装该闭包。此前 `enforce` 继续保持 off。
...
P3c-2 必须交付独立 durable membership/tombstone 与 bootstrap/repair worker，确保传入当前及 retired backend
的封闭域。该前置未完成前不允许开启 `enforce`。
...
- [ ] P3c-2 增加独立 durable membership/tombstone，接通 PostgreSQL claim、fencing high-water、live health 与周期 bootstrap/repair worker。
...
有界、fail-closed 的 Redis 安全重建原语。可信 bootstrap/reconcile 仍需 P3c-2 的 durable
membership/tombstone 与 worker 才能形成闭环；enforce、真正驱逐与多卡灰度仍属于后续阶段。
