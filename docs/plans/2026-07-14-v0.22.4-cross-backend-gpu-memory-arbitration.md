...
P6-II-A 已把 health repair 接到同一持久转换。release latch 关闭时，即使 desired 为
`enforce`，worker 也不创建 Redis store、不读 signer 且不提升 Backend gate；开启后则先以 exact
transition 进入 `promoting`，只在 membership ACK 和 Redis proof repair 同时返回可信 `ready=true`
后提交 `enforcing`。首次 promotion 不就绪落为 `blocked/off`；超时或 worker 取消会尝试以原
transition 落 `blocked`，回执不可用时保留 `promoting` 也仍然阻断派发，下一轮可幂等重放。
...
- [x] P6-II-A 将 health worker promotion、Backend gate ACK 与 Redis ready 证明接入 exact rollout 提交。
...
P6-II-A 已让 health worker 在 Backend gate 与 Redis ready 证明后精确提交 enforcing；
P6-II-B 已让管理面联合展示持久 rollout/effective 与 Redis runtime 真值。当前 latch
