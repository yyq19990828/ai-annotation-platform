...
> 版本：v0.22.4 · 状态：P0–P6 除 P5c-IV-C 真实第二宿主验收外均已完成；ONNXTools 四 session 真实 GPU 回落证据已冻结，生产 release latch 继续关闭 · 2026-07-16
>
...
验收为准，不能把本次共享卡样本冒充新的制品 gate。P5c-IV-C 仍需要真实第二宿主，当前单机
双卡环境不能替代；因此生产 effective `enforce` 继续关闭。
...
off 执行全新 promotion。生产 release latch 仍因外部实物验收门禁保持关闭。
...
- [ ] P5c-IV-C 完成真实第二宿主同号卡的资源域隔离、并行与单侧故障验收。
...
- [ ] 完成 P5c-IV-C 真实第二宿主 exit criteria；该外部实物门禁关闭前保持生产 `enforce=off`。
...
卡间无污染和双卡真实并行均由原始快照重算通过。C 仍缺真实第二宿主，不能由本机双卡替代。
...
P5c-IV-C 真实第二宿主证据尚缺，因此 release latch 与生产 effective mode 继续关闭。
