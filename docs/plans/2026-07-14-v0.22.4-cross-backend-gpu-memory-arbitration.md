...
> 版本：v0.22.4 · 状态：P0、P1a、P1b、P1d、P1e、P1f 已完成 · P1c ONNXTools 代码纵切已实现，真实 GPU 回落验证待模型就绪 · P2a 强类型静态 claim 与配置诊断已完成，P2b observe 派发与管理端待实施 · P3–P6 待实施 · 2026-07-15
>
...
P2b 待完成：管理端资源编辑与驻留展示，以及真实派发点的 would-admit /
would-evict / would-reject 影子决策与 unmanaged taint。该子阶段验收前 P2 整体仍未通过。
...
- [ ] 完成 ONNXTools 真实 GPU 回落验收与 P2–P6 exit criteria；任何 P1 blocker 未关闭时保持 `enforce=off`。
...
P2a 已交付强类型静态 claim、逐卡资源配置、静态诊断和 health 完整传输；这些路径
只读配置且默认 off，尚未执行准入或驱逐；desired 与 effective 已分离，未就绪的
observe/enforce 不会被误报为已生效。P2b 仍需完成 observe 影子派发、unmanaged
taint 和管理端展示；Redis 账本、lease、enforce、驱逐与多卡灰度属于 P3–P6。ONNXTools
仍保持 non-evictable，平台 enforce 继续关闭。
