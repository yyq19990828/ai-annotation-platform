...
P3c-2b-II-B2 仍须在消费时严格校验
`probe_started_at > token_expiry_high_water`、证据 TTL、当前 fence/membership 与完整 residency，并实现
partial corruption 的 proof-backed 原子 reset。P3c-2b-III 仍需完成
...
generation-null Unknown，P3c-2b-II-B1 已交付 challenge-bound live-health 证据候选；P3c-2b-II-B2/III
继续负责 token horizon 的锁内消费、proof-backed reset、两阶段 tombstone GC、逐资源观测与
...
- [ ] P3c-2b-II-B2 接通 token horizon 的锁内证明消费与 partial corruption proof-backed reset。
...
P3c-2b-II-B1 已提供 challenge-bound live-health 证据候选；仍需 P3c-2b-II-B2/III 的 token horizon 锁内
消费、proof-backed reset、两阶段 GC、观测与 worker 才能形成
