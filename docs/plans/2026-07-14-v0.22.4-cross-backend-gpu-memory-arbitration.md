...
> 版本：v0.22.4 · 状态：P0、P1a、P1b、P1d 已完成 · P1c ONNXTools 代码纵切已实现，真实 GPU 回落验证待模型就绪 · Grounded-SAM2、SAM3 与仲裁主体待实施 · 2026-07-14
>
...
- YOLO ModelPool 实现 build slot 预留、同 key single-flight、borrower、逐 entry use lock、先淘汰后构建、
  cancellation-safe builder 跟踪与可信/unknown residency；LRU 和 idle/manual unload 不再触碰活跃模型。
...
- 固定 `pipeline/detector/va` 三句柄池实现同 key single-flight、builder slot、waiter、borrower、逐句柄
  use lock、取消安全 cleanup 与原子 snapshot；三句柄最多持四个业务 ORT session。
...
P1a 已实现共享 wire 与验签契约，P1b/P1c/P1d 已分别完成 YOLO、ONNXTools 与 RapidOCR
受管纵切。ONNXTools 仍需在模型就绪后完成真实 GPU 回落验证；RapidOCR 已按当前镜像、
上游版本、模型摘要与物理 GPU 冻结 load/LRU/unload 和 CPU 路径证据，部署门槛继续默认
opt-in。Grounded-SAM2 与 SAM3 的受管 residency/unload 仍待实现，Redis 账本、静态 claim、
observe/enforce、驱逐与多卡灰度也未开始。下一步处理 Grounded-SAM2 / SAM3 多池聚合；
在 P1 全部验收前继续保持平台 enforce 关闭。
