...
剩余工作：P4b-II-B2a-II 接入 Resident-only runtime authority，把 Redis admission/lease/heartbeat/
uncertain/release 接到该 context；统一批量失败记录；手工 unload 成功后立即对账，不等 60 秒。首个 enforce 子阶段
...
identity 与 exact control high-water，只在该证明成立后才由 P3 repair 恢复 Redis ready。runtime authority、
Redis admission/lease、业务 token 与驱逐仍未接通。
