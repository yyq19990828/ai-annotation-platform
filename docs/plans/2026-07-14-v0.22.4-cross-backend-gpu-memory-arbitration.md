...
timeout。当前 authority 对 cooldown 阻断仍以结构化 503 fail-fast，保持 exact card ticket 到期限内
有界等待属于 P5b-II；busy drain/cancel、实物多卡验收与生产 effective enforce 仍关闭。
...
- [x] P5b-I 将 allocation 升级为 Redis v3 cooldown schema，以 Redis TIME 写入新 residency 保护窗口，在 Python/Lua 双层选择中原子阻断未到期 victim，并让升级时遗留的 v2 prepared reset 仅通过原上下文 COMMIT 安全收敛；authority 有界等待留给 P5b-II。
...
cooldown 的 exact card ticket 有界等待、busy victim drain/cancel 与实物多卡验收仍未开启。
