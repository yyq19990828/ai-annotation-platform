...
> 版本：v0.22.4 · 状态：P0–P6 全部完成；ONNXTools 四 session 真实 GPU 回落与 P5c-IV-C 第二宿主证据均已冻结，外部发布门禁已关闭；release latch 保持默认关闭并由运维逐资源启用 · 2026-07-17
>
...
重写当前 v3 账本。Unknown、不完整证明和超额承诺只能提交 not-ready；响应丢失的 exact commit 重试会重新校验
...
删除而又没有 Redis receipt 时安全保留墓碑。PostgreSQL 无法独立读取 Redis，transaction-local receipt
因此属于受信 collector 声明。collector 已拆到独立 `gpu.control` 单并发进程，
数据库 URL 仅以该进程可见的只读文件投递；每轮 repair 都通过普通应用与 collector 两条连接核对实际角色和
有效权限，要求角色不同、普通角色无 membership/fence DELETE、collector 仅有三张 GPU 真值表的读、
membership/fence 行锁所需单列 UPDATE 和 tombstone/fence DELETE。验证失败先把资源闩为 not-ready。
孤立 fence 另由数据库触发器要求
exact receipt、同事务资源 advisory lock 且 registry/membership 均已消失，不再接受同角色 worker 信任例外。

每分钟健康扫描与 repair 分别投递到 `default` 和 `gpu.control`，两条 beat 消息均在 55 秒过期并使用不同的
防重入锁，避免长健康扫描饿死控制面；修复批次总时限 50 秒。多卡最多四路并行，并按本轮波次数量均分 45 秒
工作预算，避免固定顺序让后排卡跨轮饥饿；管理 API
同样最多四路读取逐卡快照。
...
P0 已验收：D1 不再误报 CPU / CUDA，SAM3 GPU-only 能力明确，契约测试覆盖真实新增行为；D2–D14、不变量、模式、错误契约和阶段门禁已由 ADR-0049 冻结。ONNXTools 已完成参考制品实卡验证；各部署仍须匹配该证据或重新验收后才可开启受管能力。
...
- Dockerfile 固定已审计的 onnxtools commit，compose/env、协议文档、README 与契约测试同步。部署级默认仍为
  `ONNXTOOLS_MANAGED_LIFECYCLE_VERIFIED=0`；只有匹配参考证据或重新完成同等验收的部署才能设为 `1`，
  未验证部署的 `/setup` 不宣告 capability、mode 拒绝切入 enforce、`evictable=false`。
- 镜像内验收器已补齐：强制核对检测/属性模型的批准 SHA-256 与审批引用，执行真实
  业务推理、四 session CUDA provider 检查以及两轮 full-pool unload/显存回落，并输出可复核
  JSON 证据。参考验收已在 RTX 3090 `GPU-ac5e7f45-7a36-c3f8-1f65-a54f28bfcd31` 上通过：镜像
  `sha256:3f206c5aca2991d8d4880151aa58b647cb0086e56a92d9e965c6d765f5fe5d2d`，检测模型摘要
  `3773790750308d24824bbb4a22cd1601bceb90a61bf6939216dc9f3c44ca43d2`，属性模型摘要
  `b8d188ffc6f1bbdb49d91f8f29ad99dca08e4fbf0a0e4cb5d21c7acca41197ee`。代表车辆图依次命中
  pipeline/detector/va 三条业务路由，四个 session 的 primary provider 均为 CUDA；上下文基线稳定为
  284 MiB，两轮加载分别稳定为 1442/880 MiB，卸载后均稳定为 304 MiB，最终三池为空、
  `gpu_loaded=false`，满足稳定性和至少 90% 工作集回收门槛。
...
receipt、数据库 exact tombstone/fence finalize、逐资源观测与防重入 bootstrap/repair worker；worker
只服务 desired-enforce 资源，off/observe 不访问仲裁 Redis。P3c-2b-IV 已把 repair/GC 隔离到独立
`gpu.control` 进程与数据库角色，普通应用撤销 membership/fence DELETE，运行时每轮复验有效权限，孤立 fence
删除也由 receipt/资源锁触发器封闭。P3 已形成 fail-closed 闭环，但 P1/P4–P6 的全部发布门禁仍未关闭，因此
不允许开启 effective `enforce`。
...
P4b-II-C2a 已建立 cold generation 的持久授权原语。cold runtime subject 只接受 exact active
membership、完整受管 capability、新鲜 challenge proof、enforce gate 与绑定 identity，并要求
`gpu_loaded=false`、全 pool 显式为空、active/builder/borrower 全零、非 draining/evictable；已有
generation tombstone 不得超过 durable high-water。平台在 membership→fence 短事务内二次复验后
原子推进 generation 与保守 token horizon，不存在可被 proof recovery 观测的中间窗口；
后续 token 实际过期时间还必须取该上界与 Redis lease hard deadline 的较早值。失败代际
与其保守时域都不回滚。该阶段尚未调用 Redis admission、签发 token、发送 backend HTTP 或选择 victim。

P4b-II-C2b-I 已以现有逐卡 transition owner 建立 cold intent。只有 ready 卡的 exact active membership 下
绑定预期新 generation 的 owner 才能在 missing、Unloaded 或 CpuFallback 且零 lease 时取得短卡锁；
generation 持久后还必须以只能续期、绝不新建 intent 的 exact revalidate 动作重验
ready、membership 与 idle target。Redis admission 只允许同 owner/generation 在同一 Lua
原子创建 `Reserving` allocation 与 reservation lease，并同时消费 intent。过期 owner 不得消费
接管者的 intent；首次 admission 响应丢失后，exact lease/owner/generation 重试仍能只读确认已有
reservation。不同完整 `gpu_resource_id` 各自持有 transition key，不串卡。该阶段尚未将
cold authority 注入生产派发。

P4b-II-C2b-II-A 已将 cold admission 接入既有生产 authority。Resident subject 不成立时才读取
strict cold subject，并固定按 `Redis ping → cold intent → durable generation/horizon → exact revalidate
→ Reserving + reservation lease → token → first heartbeat → Loading` 执行。admission 响应丢失只用
exact lease/owner/generation 重试确认；签名、heartbeat 或 Loading 前后的未暴露失败先以 owner CAS
回滚 Unloaded，CAS 不确定则保留 uncertain lease。hard TTL 在任何私钥、Redis 或持久写入前先验证
正值、heartbeat 下界与 Redis 上限；token 过期取 durable horizon 与 lease hard deadline 较早值。token 一旦暴露，本子阶段无论 HTTP 状态均
先收敛为 Unknown + uncertain，不把完整 HTTP 响应当作驻留证明。下一子阶段再以响应后新 challenge
health 证明将 Loading 立即分类为 Resident、CpuFallback、Unloaded 或 Unknown。

P4b-II-C2b-II-B 已完成响应后立即终态对账。cold token 暴露后只有收到完整 HTTP 响应
才生成新 challenge 执行 health/setup，状态码不参与显存分类。平台在逐资源锁内重验 exact active
membership、静态 claim、generation/control/runtime 与 token horizon，然后以同一 cold lease owner 把
Loading 原子收口为 Resident、CpuFallback、Unloaded 或 Unknown。Resident 仅在 GPU loaded、代际精确、空闲
且至少一个 pool 驻留时恢复 evictable；只有全池显式空且 CPU fallback 证明完整或 state
明确 unloaded 才释放预算。证明不完整全额保留 Unknown；durable subject 漂移时旧 owner 不写
allocation，只保留 uncertain lease 等 repair。Redis 响应丢失使用 exact target/lease/owner/generation
幂等重试，Resident 同期新快路 lease 不会破坏原 cold lease 的结果确认。heartbeat 保持到
allocation CAS 完成后才停止；无完整响应不发 health，但仍在 durable 锁域内收口 Unknown。

P4b-II-C2c-I 已增加空闲驱逐的 Redis CAS，以 exact allocation generation、零 lease、
membership 与 transition owner 封闭 `Resident → Draining → Unloading → Unloaded/Unknown`
状态跃迁。端点、鉴权、boot identity 或持久 claim 漂移都不得提交旧终态；
响应丢失只能以同 owner/generation/phase 精确重放。

P4b-II-C2c-II 已接通空闲 victim 选择。authority 只从目标完整 `gpu_resource_id` 的 ready
快照中选择 exact Resident、可驱逐、零 lease 且不高于请求者优先级的成员，并按
`(eviction_priority, last_used_at, backend_id)` 排序。候选预算不足、只有高优先级
或存在 busy/no-fit 时立即返回带 `Retry-After` 的结构化 503，不开启主动等待。

P4b-II-C2c-III 已将 victim generation/horizon、transition owner 和新鲜 challenge health 纳入
持久授权。`/drain` 必须先返回严格 `ready_to_unload` ACK，并由同一 challenge 的空闲
证明确认后才能进入 Unloading；`/unload` 之后只有新鲜全空驻留证明才释放预算，
其余结果保守收口 Unknown。owner 心跳受 durable hard deadline 限制，取消、断连、超时和
终态 CAS 响应丢失都会先精确重放或保守清理，不会提前释放 owner。

P4b-II-C2c-IV 已把上述原语组合为 cold capacity loop。authority 在取得 target cold intent
之前按顺序驱逐一个或多个空闲 victim，每个 victim 完成后重读同卡快照；任一驱逐
不确定即停止，不会换选下一个 victim。预算足够后再以新 challenge 重读并精确绑定
target cold subject，然后进入既有 reservation 与 cold admission。drain/unload 的同路由响应丢失
重试复用原 token/JTI、重新构造 grant；资源锁、Redis key、token 与证明始终绑定完整
`gpu_resource_id`，单卡与逻辑多卡共用同一路径且不串卡。
...
本阶段不开启 busy victim 的主动等待、cancel-drain、cooldown/FIFO 或实物双卡灰度，
这些继续归 P5。backend 重启降回 legacy 后的 managed-header 降级旁路已由 P4b-II-A 关闭；
P4b-II-C2c 不重新引入该兼容分支。生产 effective enforce 仍保持关闭。

验收：代码级可共驻、刚好容量、逐个驱逐多个空闲 victim、同级 LRU、高优先保护、busy fail-fast、完全放不下、Redis 故障、CPU fallback、warmup / reload / raw smoke-test 绕过、响应丢失重放与非默认卡资源隔离均已通过。`off` 模式的现有 ML client 行为无回归；实物多卡并行验收留给 P5。
...
P5a-I 已先完成 FIFO 接线所需的 Redis 原子门禁，但尚未让 authority 主动排队。显式携带
backend/card ticket 的 admission 现在必须证明该 ticket 仍存活、位于精确队首，并匹配
backend、owner 与 membership；缺失或过期 ticket 不再因队列看似为空而绕过。空闲驱逐 begin
也可在同一 Lua 内校验 card ticket 精确队首且不提前消费，响应丢失只允许相同 ticket/queue owner
幂等重放；真正的 ticket 消费仍留到 target cold admission 成功。生产 effective enforce 保持关闭。

P5a-II 已将这些原语接入惰性 authority。Resident 快路只在 backend 全局并发饱和后进入
backend FIFO；cold slow path 则先取得 card FIFO 精确队首，再做容量检查与空闲 victim 驱逐。
同一 card ticket 会贯穿多 victim begin 与最终 target admission，只在准入成功时原子消费。
`GPU_ARBITER_ADMISSION_TIMEOUT_SECONDS` 独立限制两级等待，ticket 使用固定 TTL 且不因轮询续期；
超时返回带 `Retry-After` 的结构化 503，请求取消、超时或响应丢失都会以精确 owner/ticket
清理。驱逐业务截止时间会早于 ticket/owner hard deadline，为 Unknown/Unloaded 终态 CAS 保留
独立窗口；这一阶段仍只驱逐零 lease 的 idle victim，busy drain/cancel、cooldown、实物多卡验收与
生产 effective enforce 保持关闭。

P5b-I 已建立 D14 的原子 cooldown fence。allocation schema 升级为 Redis v3，并新增必填
`not_evict_before_ms`；Resident 必须持有正截止时间，普通 transition 不得绕过 cold finalize 生成
Resident。v1/v2 或缺字段账本只会 fail-close，再由既有 challenge-bound proof reset 重建，禁止
原地补字段。升级时遗留的严格 v2 prepared reset 只能沿用原 reset identity/CAS/封闭域恢复，BEGIN
保持只读，COMMIT 清除旧 child 后才原子写出 v3。正常 cold residency 只在首次可信 Loading→Resident CAS 中用同一次 Redis
`TIME` 写入 `last_used_at_ms` 与保护截止时间；proof reset 重建 Resident 时则以固定
`prepared_at_ms+cooldown` 保守恢复，以上精确重放和 Resident 快路都不续期。Python 选择使用
快照携带的 Redis 观测时间预过滤，idle eviction Lua 在原子区复验并可返回累计容量所需的最早
`retry_at_ms`，未到期时不推进 generation、owner、transition 或 revision。配置
`GPU_ARBITER_RESIDENCY_COOLDOWN_SECONDS` 固定默认 30 秒、范围 1..3600 秒；允许长于 admission
timeout。

P5b-II 已让 cold authority 在 cooldown 阻断期间继续持有 exact card ticket，并按快照 Redis 时间与
Lua 返回的累计 `retry_at_ms` 计算等待量。每次等待同时受 admission deadline 与固定 ticket TTL 约束，
不轮询续期；超时映射为 `gpu_capacity_unavailable` 503 + `Retry-After`，取消与超时都会在关闭 store 前
精确清票。同一 ticket 继续
贯穿后续多 victim 和最终准入，不会在 victim 尚未可驱逐时提前消耗终态清理预留。等待到期后必须
重新读取卡快照，确认 cooldown 与容量，再允许 victim health、generation、drain 或 unload；等待本身
不会创建 transition owner。不同
`gpu_resource_id` 的等待各自独立；busy drain/cancel、实物多卡验收与生产 effective enforce 仍关闭。

P5c-I-A 已建立尚未接 authority 的 busy Redis transition CAS。显式 busy begin 仍必须位于 exact card
ticket 队首并通过既有 priority/LRU、cooldown、membership 与 generation 复验；它原子保留旧 workload
lease、推进 allocation generation、绑定 `require_idle=false` 的 transition owner 并进入 Draining。此后
victim 新 admission 被拒绝，旧 generation lease 仍可 heartbeat/release，同卡非 victim Resident 快路
继续可用。Draining→Unloading 必须再次确认 victim lease 为零；结果不确定时允许保留 lease 转 Unknown，
继续全额计费。cancel CAS 只允许 exact owner 下以更大 generation 从 Draining 回滚 Resident，保留原
cooldown；响应丢失重放不推进 revision，进入 Unloading 后拒绝迟到 cancel。strict busy subject、只读
health 分类、durable cancel generation/RESUME token 和 authority 有界双域等待属于后续子阶段。

P5c-I-B 已把 busy victim 的现场证明与 drain 后等待真值从终态 CAS 中拆开。
busy-capable subject 仍严格要求 Resident、`gpu_loaded=true`、`evictable=true`、fresh challenge、
capability、membership、registry claim、boot、source generation、control/runtime epoch 与 resource identity，
但允许旧 active/borrower 存在，也允许读取时恰好已清零。subject 另外冻结排序后的
pool-id 集合。drain generation 的 fresh health 使用单次 MVCC 快照，只读分类为
`draining_busy`、`ready_to_unload` 或 `uncertain`；它不取整卡长锁、不写 PostgreSQL/Redis，
也不把 backend 域 ready 当成 Redis lease-zero 证明。共享 wire 与 ML client 已同时验证
active/builder/borrower 任一非零的合法 busy drain ACK。authority 仍保持 idle-only。

P5c-II-A 已把 cancel generation 的“推进一次”与“精确重放”变成持久不变量。
PostgreSQL 为每个 backend 保留最后一份 cancel intent，将 drain/result generation、
membership、boot/control/runtime epoch、owner/hard deadline、`operation=evict`、token expiry、JTI、
稳定 pool-id 与 source subject 指纹绑在同一个 generation/horizon 事务。并发或进程重启后只有
全字段 exact-match 才返回原 generation/JTI；owner、expiry 或现场身份不同时 fail-closed，
不会盲目烧掉下一代；重启读取可从 intent 重建不依赖内存 source 的 exact cancel subject。
P5c-II-B 已把 passive RESUME 证明与账本收口补齐。平台按 durable intent 的 exact
generation/JTI/owner/`operation=evict`/hard deadline 稳定重签 token；五个 backend 首次 cancel 也必须同时
匹配原 drain owner 与 operation，同一正确 token 可精确重放。平台只接受 result generation、boot/control、
backend/resource、Resident、`gpu_loaded=true`、`evictable=true` 与完整稳定 pool-id 全部一致的 strict ACK，
随后以新 challenge health 在同一持久 proof domain 再验现场；active/builder/borrower 可非零。两份证明均可信
才允许 Redis 从原 drain generation 回切 result generation，任一不可信只会保留旧 lease/预算并收紧 Unknown，
响应丢失只做 exact CAS 重试。此阶段不调用真实 `/drain/cancel`，不接主动 busy authority，也不宣称
cancel/unload 已跨进程串行；P5c-III 必须先在 Redis owner 上原子冻结唯一 branch，使 arm-cancel 与
Draining→Unloading 只有一个成功，再接双域有界等待与取消安全编排。

P5c-III 已把主动 busy victim 编排接入同一 cold authority。空闲候选累计预算足够时仍只走 idle 路径；
不足时才允许 busy begin，并在 strict drain ACK 后按轮次获取新鲜 Redis 快照与新 challenge health，独立确认
victim lease 为零以及 backend active/builder/borrower 全零。等待期间不持有数据库行锁、advisory lock 或卡锁；
DRAIN、双域等待和 UNLOAD 受 admission/ticket 与 token 的较早工作 deadline 限制，transition owner 另固定
保留 30 秒 cancel 收尾窗口。

Redis transition owner 现以持久 `eviction_branch=cancel|unload` 冻结唯一方向。lease-zero
Draining→Unloading 在同一 Lua CAS 内 arm unload；取消路径先 exact 重放 durable intent、稳定签名，再以
resource/backend/owner/drain-generation arm cancel，只有成功后才发送真实 RESUME。两个 Redis client 并发时
只有一个分支获胜；unload 获胜后不发 RESUME，cancel 获胜后拒绝 unload。marker 不随 owner TTL/hard deadline
消失，过期后只允许 exact 分支终态与 release，或由两阶段 proof reset 证明重建；缺 key、mirror/branch/deadline
损坏均 fail-closed。strict cancel ACK + fresh health 最终提交 Resident/Unknown，响应丢失只做 exact prepare、
arm 与 commit 重放。单卡与多卡都按完整 `gpu_resource_id` 使用这条路径，跨资源分支与 FIFO 独立；实物显存、
并行和跨宿主同号卡证据仍由 P5c-IV 完成。

P5c-IV-A-I 已先封闭多卡 device set 配置旁路。五个 backend 在构造任何模型池、句柄池或
ORT session 前统一检查 NVIDIA/CUDA/ROCm 可见设备变量；单索引、单 GPU/MIG UUID 或显式无设备值可用，
逗号列表、多 UUID 或已暴露 GPU runtime 的 `all` 直接使启动失败。验收 runner 与证据格式留给 A-II；本机单卡多 backend /
同宿主双卡实测属于 B，真实第二宿主同号卡验收属于 C。A/B/C 全部通过前不勾选 P5c-IV。

P5c-IV-A-II 已建立严格 manifest、只读 preflight、仅限非生产且要求 exact run-id 二次确认的真实
dispatch authority runner，以及 `response-lost-after-http`、`cancel-after-grant`、`health-timeout`
三类单点故障注入。`run` 会先只读验证除数据库缓存证明外的所有安全门禁，再为范围内每个
Backend 刷新并持久化 challenge-bound health，最后重跑完整 preflight。这防止维护窗口暂停周期性
health worker 时，只读 preflight 的实时结果与
authority 读取的数据库证明分歧；刷新失败会在派发前 fail-closed，但不修改 claim、fence、
rollout 或 Redis 账本。证据同时冻结 PostgreSQL 控制窗口、Redis allocation/lease/有界 queue/transition、
challenge health、宿主物理卡 token、`nvidia-smi` 和真实 HTTP 窗口；无故障主报告由 verifier
从原始快照重算断言，不采信自报 passed 摘要，故障报告则在注入现场由 `run` 计算且不进入外部
verifier。报告脱敏 action body，原 manifest 摘要只用于关联原文件，不宣称可独立复验 body。
容量拒绝另有严格单 action 场景：只接受显式预期 `gpu_capacity_unavailable` 的 requester，
并要求 503 发生在 grant 和 Backend HTTP 之前，前后 allocation 与 committed 完全不变；verifier
会从原始 action 和账本快照重算该结论。
实物 busy victim 用例必须让 victim action 命中已有 `Resident` 模型池；`Loading`/builder 属于受保护现场，
不是 active drain 验收证据。
稳定窗口固定为 5 次、0.5 秒间隔、64 MiB 最大波动，并行重叠下限为 500 ms；跨宿主区间会按
PostgreSQL 时钟探针的完整 RTT 向内收缩，只采信保守重叠下界。90% 回收率只适用于
B 的隔离 full-unload 生命周期证据，requester 仍驻留的跨 Backend 驱逐不执行该阈值。本卡级 runner
不关闭 MIG 专项门禁。A-II 只关闭工具与证据合同，不代替 B/C 的实物结果。

P5c-IV-B 已在非生产 `gpu-node-a` 的两张 RTX 3090 24 GiB 上完成。资源分别为
`GPU-0d0ff398-3fa6-778c-8611-a45fbadecabb`（物理卡 0）与
`GPU-ac5e7f45-7a36-c3f8-1f65-a54f28bfcd31`（物理卡 1），逐卡
`allocatable_mb=18000`。四份无故障报告均由当前 verifier 从原始快照重算为 `passed`：

| run-id | 实物结果 |
|---|---|
| `p5c4-co-03` | 卡 0 上 Grounded-SAM2 9000 MiB 与 YOLO 4000 MiB 同时为 Resident，`committed_mb=13000`；5 点基线显存波动 2 MiB，动作后 5 点波动 0 MiB。 |
| `p5c4-eviction-06` | 卡 0 的 Grounded-SAM2 busy victim 从 generation 5 经 `resident → draining → unloading → unloaded` 到 generation 6；持久 transition 精确绑定 RapidOCR requester、`require_idle=false` 与 `eviction_branch=unload`。victim/requester 端到端分别为 4744.515/5147.150 ms，最终 Grounded-SAM2 Unloaded、RapidOCR 与 YOLO Resident，queue/lease/transition 均清空。同期卡 1 的全部 20 个物理显存样本均为 7646 MiB，证明卡 0 驱逐未污染卡 1。 |
| `p5c4-capacity-01` | RapidOCR 10000 MiB + YOLO 4000 MiB 已驻留且无合法 victim 时，Grounded-SAM2 9000 MiB 请求在 151.918 ms 内得到精确 `gpu_capacity_unavailable` 503；未取得 grant、未开始 Backend HTTP，前后 allocation 与 `committed_mb=14000` 完全一致。 |
| `p5c4-dual-05` | 冷态 Grounded-SAM2 在卡 0、SAM3 在卡 1 同时执行，HTTP 分别持续 812.274/20458.757 ms，精确重叠 789.218 ms，高于 500 ms 门槛；两张物理卡均观察到目标 Backend 的 Resident GPU 执行并以 Resident 终态收敛。 |

常规跨 Backend 驱逐后 requester 仍驻留，因此按冻结合同不对整卡执行隔离 full-unload 的 90%
硬断言。`p5c4-eviction-06` 中 victim 进程显存峰值 1562 MiB、卸载后 20 MiB，原始进程占用下降
约 98.7%，仅作为补充证据；隔离 full-unload 的正式阈值仍以 P1 各 Backend 的部署级生命周期
验收为准，不能把本次共享卡样本冒充新的制品 gate。

P5c-IV-C 已在两个真实宿主上完成同号卡验收。节点 A 使用 RTX 3090 的物理卡 0，资源为
`gpu-node-a/GPU-0d0ff398-3fa6-778c-8611-a45fbadecabb`、`allocatable_mb=18000`；节点 B 使用
RTX 4060 Laptop GPU 的物理卡 0，资源为
`gpu-node-b/GPU-58863733-f399-e302-3980-de3a0ead6df1`、`allocatable_mb=6000`。两个 Backend
都只看到逻辑 `cuda:0`，但持久资源域、物理 UUID 与 Redis 分片均不同。验收固定在平台提交
`0e1394eaa227af7f537e47290e7184d8531a8d44` 和 ONNXTools 镜像
`sha256:c02ce066784fcd0d6235de2f77d04fb2b23eca1cad6a436ecbdfe82b9fc6e735`；节点 B 的检测模型、
属性模型与车辆输入 SHA-256 分别为
`3773790750308d24824bbb4a22cd1601bceb90a61bf6939216dc9f3c44ca43d2`、
`b8d188ffc6f1bbdb49d91f8f29ad99dca08e4fbf0a0e4cb5d21c7acca41197ee` 与
`8ae93725f2d5827a217ec44dd3a8da0d9a580fb2c07de916ccfd17d4c1d89a69`。

`p5c4-cross-host-node-a-04` 让节点 A 冷态 Grounded-SAM2 执行 `base_plus/T` warmup，
`p5c4-cross-host-node-b-04` 同时让节点 B ONNXTools 执行 32 个 `vehicle-attr` 任务；两份主报告
均为 `passed`。外部 verifier 以各自 PostgreSQL 时钟探针完整 RTT 向内收缩 HTTP 窗口后，重算出
`1370.722 ms` 的保守跨宿主重叠，高于 500 ms 门槛，并确认资源、节点与物理 UUID 均不同而
GPU index 都是 0。单侧故障报告 `p5c4-cross-host-node-b-response-lost-02` 在节点 B 完成真实 HTTP
后注入 `response-lost-after-http`，只把精确坐标
`(gpu-node-b/GPU-58863733-f399-e302-3980-de3a0ead6df1, node-b backend, generation 1)`
收紧为保守状态；节点 A 同期保持零 lease、零两级 queue、零 transition，刷新证明后重新达到
`ready=true`、`committed_mb=9000`，并再次通过只读 preflight。由此关闭跨宿主资源域隔离、真实
并行与单侧故障不扩散三项门禁；release latch 仍保持默认关闭，生产启用必须由运维逐资源灰度。
...
P6-I 已建立逐资源持久 rollout 真值。`gpu_arbiter_rollouts` 以
`off → promoting → enforcing → demoting → off` 为正常闭环，故障进入 `blocked`；每次转换绑定
不可复用 transition UUID、严格单步 revision 与完成回执，数据库触发器拒绝非法跳态、资源身份改写、
漏 revision 和删除。`effective_mode` 与 `target_mode` 分开保存，因此 demotion 期间仍保守报告
effective enforce，同时停止新派发；响应丢失重放不会重复推进。`GPU_ARBITER_ROLLOUT_ENABLED`
默认关闭，关闭时保持既有 effective-off 与零 rollout DB 读取。开启后，业务派发已在
Backend HTTP 前消费该持久真值：缺行、过渡态、blocked 和数据库失联都 fail-closed，
未知 GPU resource 还会检查全局 rollout 边界；只有具备新鲜显式 CPU 证据的 backend 跳过该
GPU 数据库路径。该读取地基本身不能提交 `enforcing`。

P6-II-A 已把 health repair 接到同一持久转换，但后续安全审计确认原 repair
原语不会主动下发 `gate=enforce`。proof recovery 已先收紧为只接受 fresh `enforce`
health，因此即使 release latch 误开，legacy Backend 也只会保持 Unknown/not-ready，不会误提交
`enforcing`。P6-II-C 进一步在 `gpu_backend_fences` 持久 latest rollout control intent：
`reset|mode_enforce|mode_legacy`、exact transition、membership、boot、control epoch 与 token
horizon 必须整组存在；响应丢失或进程重启不会换 owner/JTI/epoch。promotion 只能按
signed full-reset health ACK → enforce-mode health ACK 顺序推进，demotion 则允许保留无活动请求的 resident
pool 切回 legacy。HTTP ACK 只表示远端已接受，所有步骤仍必须等 `probe_started_at`
严格晚于对应 token horizon 的 fresh health。P6-II-D 已将该顺序接入 health worker：
pending membership 本轮激活后先等下一轮 active health，没有 pending 成员时才按稳定 Backend UUID
顺序推进 reset/mode，所有成员都确认 enforce 后才进入 Redis proof reset。不同物理卡仍最多
四路并行，一张慢卡不会占用另一张卡的锁或 HTTP 时间片。
P6-II-B 已让资源管理端与超管 backend 列表读取 PostgreSQL rollout 真值，分开展示
持久 state、解析后 effective mode、transition/revision/blocker 与 Redis runtime。只有 desired
`enforce`、持久 `enforcing` 与 Redis ready 三者同时成立才报 enforce ready；持久活跃期
误关 release latch 或移除静态资源都报 blocker。P6-III 已让 worker 同时遍历 desired-enforce
资源与所有持久非 off rollout。回滚先将 Redis 锁存 not-ready，等 lease、两级 queue 和
transition 归零，再串行下发 legacy mode；全部 post-horizon health 确认后才完成 `off`，
不清 Redis、不假定已驻留模型卸载。配置已移除但持久过渡未结束的资源仍会从
Redis snapshot 恢复 allocatable 并继续 demotion。任一 partial ACK 进入 blocked/effective-enforce 且
派发保持阻断；若 desired 又改回 enforce，平台先完成已开始的 demotion，再在下一轮从
off 执行全新 promotion。P5c-IV-C 已关闭最后一项外部实物门禁；release latch 继续以默认关闭
交付，生产环境必须由运维逐资源显式开启并保留即时回滚能力。
...
回滚请求先立即停止新驱逐与新 admission，但不会在 backend 仍 enforce 时让平台直接进入 off。平台等待旧 control epoch token 过期且 Redis lease/queue/transition 安全收敛后，才逐个下发并确认 backend `gate=legacy`；全部 health 握手一致后完成 observe/off 切换。partial ACK 时资源 mode-transition fail-closed；若 desired 重新切回 enforce，依然先完成本次 demotion，下一轮再以新 transition 重做 reset 与 promotion。回滚不清 Redis、不假定真实模型已卸载，现有 backend 按自身 idle 策略自然收敛。进入 enforce 前，既有 `generation=null` residency 必须在 legacy gate 下暂停 workload、经受签名 `/lifecycle/reset` full-unload 并由 health 确认空池；API、GPU worker、其它调用 ML client 的 worker 与 backend 必须全部部署兼容实现并完成 mode handshake，避免旧进程绕过 gate。
...
- [x] P1c 完成 ONNXTools 代码纵切：固定句柄池、取消安全 executor、本地 lifecycle、full-pool cleanup 与部署级能力门槛落地。
- [x] 完成 ONNXTools 参考制品四 session 真实推理、两轮 generation 冷启与 full-pool unload 显存回落验收；冻结镜像、模型、输入、GPU、provider chain 与连续采样证据。
...
- [x] P3c-2b-IV 将 tombstone repair/GC 隔离到 `gpu.control` 单并发进程与独立最小权限数据库角色，普通应用撤销 membership/fence DELETE，每轮运行时复验角色边界，并以数据库触发器封闭孤立 fence 删除旁路。
...
- [x] P4b-II-C2a 建立严格 idle-unloaded cold subject，并以 membership→fence 二次复验持久推进 generation 与签发前 token horizon。
- [x] P4b-II-C2b-I 以逐卡 cold intent 封闭 generation 推进到 Redis reservation 的跨存储并发窗口，并保留 exact response-loss 重试。
- [x] P4b-II-C2b-II-A 接通 cold admission authority 的代际、reservation、token、heartbeat 与 Loading 顺序，失败时保守收敛。
- [x] P4b-II-C2b-II-B 以响应后 exact challenge 在持久锁域内立即分类 cold 终态，并完成幂等 Redis 收口与租约清理。
- [x] P4b-II-C2c-I 建立空闲驱逐的 exact owner/generation/phase Redis CAS，响应丢失只做同证明重放。
- [x] P4b-II-C2c-II 建立同卡空闲 victim 的 priority + LRU + backend-id 稳定选择，保护严格更高优先级并对 busy/no-fit fail-fast。
- [x] P4b-II-C2c-III 将 victim durable generation/horizon、新鲜 health proof、drain/unload 终态与取消安全清理收口。
- [x] P4b-II-C2c-IV 在 target cold intent 前编排单个/多个空闲 victim，并以新 challenge 重读 target；完整资源 ID 保持逻辑多卡隔离。
- [x] P5a-I 收紧显式 FIFO ticket 的存活队首校验，并让 idle eviction begin 可原子绑定 card ticket；authority 排队接线留给下一子阶段。
- [x] P5a-II 接通 Resident backend FIFO 与 cold card FIFO，以固定 ticket 期限、独立 admission timeout 和取消安全清理保证有界等待。
- [x] P5b-I 将 allocation 升级为 Redis v3 cooldown schema，以 Redis TIME 写入新 residency 保护窗口，在 Python/Lua 双层选择中原子阻断未到期 victim，并让升级时遗留的 v2 prepared reset 仅通过原上下文 COMMIT 安全收敛。
- [x] P5b-II 在 exact card ticket 上按 Redis cooldown 最早时刻有界等待，以固定 ticket TTL、admission deadline 和取消安全清票保持公平与跨卡隔离。
- [x] P5c-I-A 建立 busy victim 的 Redis 原子 begin、旧 lease 存续、新准入门禁、零 lease Unloading、newer-generation cancel 与保守 Unknown CAS；authority 仍保持 idle-only。
- [x] P5c-I-B 建立 strict busy-capable victim subject、稳定 pool-id 绑定与 challenge-bound drain health 只读三态分类，并固定 active/builder/borrower busy ACK 契约；authority 仍保持 idle-only。
- [x] P5c-II-A 持久绑定 busy drain cancel 的 source/result generation、owner、token horizon/JTI 与完整现场指纹，全字段精确重放不重复推进高水位。
- [x] P5c-II-B 稳定签发 exact RESUME claims，以 strict ACK + fresh health 双证明驱动 Resident/Unknown cancel CAS，并让五个 backend 精确绑定原 drain operation；真实 RESUME 与跨进程 branch fence 仍未接 authority。
- [x] P5c-III 在 Redis transition owner 上原子冻结持久 cancel/unload 唯一分支，接通 cancellation-safe 的 lease-zero + backend drain-health 双域有界等待、真实 RESUME、30 秒独立 cancel horizon 与 proof-reset owner 收尾。
- [x] P5c-IV-A-I 在五个 backend 启动点显式拒绝逗号多卡、多 UUID 与已暴露 GPU runtime 的无界 `all` device set。
- [x] P5c-IV-A-II 建立非生产验收 runner、机器可读证据格式、稳定显存采样阈值与软件故障注入。
- [x] P5c-IV-B 完成本机单卡多 backend 与同宿主双卡物理隔离、并行、驱逐与显存回落验收。
- [x] P5c-IV-C 完成真实第二宿主同号卡的资源域隔离、并行与单侧故障验收。
- [x] P6-I 建立默认关闭的 release latch、逐资源持久 rollout 状态、exact transition/revision/完成回执、数据库防跳态门禁与业务派发 fail-closed 读取。
- [x] P6-II-A 将 health worker transition 与 Redis ready 结果接入 exact rollout 提交，并将 ready proof 收紧到 fresh enforce gate。
- [x] P6-II-B 让管理面展示持久 rollout/effective 真值、Redis runtime 与精确 blocker。
- [x] P6-II-C 持久 exact reset/mode control intent，严格解析 reset ACK，并绑定 post-horizon health 确认。
- [x] P6-II-D 把 signed reset → enforce gate → Redis ready 顺序接入逐卡 health worker。
- [x] P6-III 将 Redis 运行时静默、安全 demotion/legacy ACK 与回滚收敛接入 rollout 状态。
- [x] 完成 ONNXTools 真实 GPU 回落验收与 P3–P6 代码及本机 exit criteria。
- [x] 完成 P5c-IV-C 真实第二宿主 exit criteria；外部门禁关闭，release latch 仍默认关闭并由运维逐资源启用。
...
Grounded-SAM2 与 SAM3 受管纵切。ONNXTools、RapidOCR、Grounded-SAM2 和 SAM3 已按各自参考镜像、
模型摘要与物理 GPU 冻结真实推理、重启和 full-unload 证据，部署门槛继续默认 opt-in；P1 已整体通过。
...
数据库 tombstone finalize、逐资源观测和防重入周期 worker 完成 P3 闭环。P3c-2b-IV 又把 repair/GC
隔离到独立 `gpu.control` 进程和最小权限 collector 角色，运行时拒绝同角色、普通应用 DELETE 或 collector
越权，并以 receipt/逐卡锁触发器封闭孤立 fence 删除旁路。effective enforce 与外部实物门禁仍属于后续阶段。
P4a-I 已交付统一派发接缝；P4a-II-A 已建立仅由 API/通用 worker/GPU
...
P4b-II-C2a 已建立 cold 目标的严格空池证明与 durable generation/horizon 授权原语，
P4b-II-C2b-I 又以 exact cold intent 封闭持久代际到 Redis reservation 的窗口，
P4b-II-C2b-II-A 已将 cold subject 接入 reservation、token、heartbeat 与 Loading 生产 authority；
P4b-II-C2b-II-B 又以响应后 exact challenge、durable stale 门禁、Redis 幂等终态 CAS 与先 allocation
后 lease 的清理顺序完成立即对账。P4b-II-C2c-I–IV 现已在同一惰性 authority 中接通
空闲 victim 的稳定选择、持久驱逐、drain/unload 健康证明、终态对账、多 victim 容量循环
与 target 新鲜重读，并以完整 `gpu_resource_id` 隔离单卡/逻辑多卡资源。生产 effective enforce、
P5a-I/II 已将精确 FIFO 票据门禁、Resident backend 队列和 cold card 队列接入同一 authority，
并以独立超时、固定 ticket TTL 与取消安全清理保证有界等待。P5b-I 又以 Redis v3 schema、
Redis TIME 和 Python/Lua 双层门禁锁定新 residency cooldown，旧 v2 账本只能 proof reset 重建；
P5b-II 已让 cold authority 在同一 exact card ticket 上按累计最早时刻等待，并由 admission deadline、
固定 ticket TTL、超时/取消精确清票和完整资源分片保证等待有界且不跨卡串行。P5c-I-A 先交付了保留旧
lease 的 Draining begin、零 lease Unloading、
newer-generation Resident cancel 与保守 Unknown Redis CAS。P5c-I-B 另已交付 strict busy proof、
稳定 pool-id 绑定与 drain health 只读三态分类；P5c-II-A 已补齐 durable cancel intent/generation 与 exact JTI
重放地基，P5c-II-B 已补齐 exact RESUME 签名、strict ACK + fresh health 双证明、Resident/Unknown CAS 与
五 backend 原 operation 绑定。P5c-III 已在逐资源 Redis owner 上持久冻结唯一 cancel/unload branch，并把
空闲优先的 busy victim 选择、lease-zero + fresh drain-health 双域有界等待、真实 RESUME、exact response-loss
重放、30 秒独立 cancel horizon、终态 release 与 proof reset 恢复接入同一 cancellation-safe authority；
单卡与逻辑多卡分支互不串扰。P5c-IV-A-I 已让五个 backend 在启动时硬拒绝多卡 device set；
A-II 又交付了非生产真实 authority runner、严格机器证据、物理卡身份、故障注入和重算 verifier。
B 已在同一台双 RTX 3090 节点上以 `p5c4-co-03`、`p5c4-eviction-06`、
`p5c4-capacity-01` 与 `p5c4-dual-05` 关闭：单卡共驻、busy 驱逐、Backend HTTP 前容量拒绝、
卡间无污染和双卡真实并行均由原始快照重算通过。C 又以节点 A RTX 3090 卡 0 与节点 B
RTX 4060 Laptop GPU 卡 0 的两个独立资源域完成真实跨宿主验收：`p5c4-cross-host-node-a-04` /
`p5c4-cross-host-node-b-04` 经 RTT 收缩后仍有 `1370.722 ms` 保守重叠；
`p5c4-cross-host-node-b-response-lost-02` 只收紧节点 B 的精确 generation 1 坐标，节点 A 清空
ephemera、刷新证明后仍通过 preflight。
P6-I 又冻结了重启可恢复的逐资源 rollout 状态、release latch 与业务派发 fail-closed 读取合同；
P6-II-A 已接入 durable health transition/Redis 提交并将 ready proof 强制绑定 enforce gate；
P6-II-B 已让管理面联合展示持久 rollout/effective 与 Redis runtime 真值；P6-II-C 又冻结了
reset/mode 的 exact 意图与 post-horizon ACK；P6-II-D/P6-III 已将 promotion 与保留 Redis 的安全
demotion 接入多卡 health worker。ONNXTools 参考制品已关闭 P1 实卡门禁，但部署能力仍默认 opt-in；
P5c-IV-C 已关闭最后一项外部实物门禁，P0–P6 全部完成。release latch 仍按安全默认值关闭，生产
effective mode 只能由运维按资源显式灰度开启。
