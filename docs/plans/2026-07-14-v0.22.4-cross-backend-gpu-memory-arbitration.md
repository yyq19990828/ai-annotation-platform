...
> 版本：v0.22.4 · 状态：P0 已完成 · P1a 共享 lifecycle wire 已实现 · P1b YOLO 纵切待执行 · 仲裁主体尚未实施 · 2026-07-14
>
...
P1b 起逐 backend 纵切，先以 YOLO 验证完整并发与生命周期边界。
...
- [x] P1a 冻结 D15 的 EdDSA / `kid` keyring，落共享 lifecycle wire、token codec 与 82 项协议测试；任何 backend 尚未宣告 managed capability。
...
P1a 已实现共享 wire 与验签契约；P1b–P6 待实施。五个 backend 的受管 residency / unload 尚未完成，Redis 账本、静态 claim、observe/enforce、驱逐与多卡灰度也未开始。下一步以 YOLO 完成首个 backend 纵切，再按同一契约扩展其余 backend。
