...
`null/null`，旧 backend 仍可 connected，并允许 P3 按严格 residency 重建保守账本：空载不写 allocation，
驻留按完整预算且固定 non-evictable；两者均不能获得激活或驱逐权。B1 不激活 membership、不读取 signer、
...
runtime/control epoch、不激活 membership，也不发送真实 mode 请求；不可逆编排留在 B2a-II，post-horizon active
proof 与 Redis 域收敛留在 B2b。
...
剩余工作：接入 active membership 与 runtime authority，把 Redis admission/lease/heartbeat/
...
真实 promotion 前还必须关闭 backend 重启降回 legacy 后的旁路：只有 generation/token 两个 header 都缺失时才允许
...
P3c-2b-II-B1 已提供 challenge-bound live-health 证据候选，P3c-2b-II-B2a 已提供不会放宽普通 repair 的
...
health/setup 数据库观测窗口和 canonical capability SHA-256 把受管能力纳入 challenge proof；P4a-II-B2a-I
已建立严格 token-only mode wire，但尚未执行真实握手。active membership、signed legacy ACK、runtime authority
与 Redis admission/lease 仍未接通。
