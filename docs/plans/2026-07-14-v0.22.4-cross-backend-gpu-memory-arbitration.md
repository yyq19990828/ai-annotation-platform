...
> 版本：v0.22.4 · 状态：P0 已完成 · ADR-0049 已接受 · P1 受管生命周期待执行 · 仲裁主体尚未实施 · 2026-07-14
>
> 地基基线：初始 `compute` 传输链由 `2026-07-13-v0.22.3-ml-backend-cpu-fallback-audit.md` 交付；本轮已按实际代码、容器镜像、运行中 `/health` 与定向测试完成 P0a 纠偏，并在 P0b 将 D2–D14 与不变量写入 ADR-0049。ADR Accepted 只表示设计冻结；完成 P1–P5 对应验收前不得开启仲裁 enforce。
>
...
- generation header 使用 `X-AAP-GPU-Generation`（规范正 int64 十进制字符串），token header 使用 `X-AAP-GPU-Admission-Token`；token 绑定 audience、registry id、resource id、boot id、generation、control epoch、scope、`jti` 与 `exp`，不复用 `Authorization`。`scope=mode|reset` 只携带 control epoch，不要求 generation header。
...
- `/setup.managed_lifecycle` 声明 protocol version、generation fencing、mode/drain/drain-cancel/unload/reset endpoint 与 generation/token header 名；缺失即 non-evictable。`residency.gpu_loaded` 与逐 pool `resident` 允许 `null` 表示 unknown，只有所有 pool/builder/borrower 明确为空时才显式 false。
...
- 已冻结错误码及触发 / HTTP / 重试语义：`gpu_arbiter_not_ready`、`gpu_capacity_unavailable`、`gpu_backend_concurrency_saturated`、`gpu_drain_timeout`、`gpu_arbiter_unavailable`、`gpu_config_invalid`。
...
工作：先在 `protocol_v2` 落 wire schema / header / 错误词表；仅把真正 framework-neutral 的 generation / transition reducer 放入 `backend_runtime`，asyncio/threading active guard 仍由各 backend 在本地生命周期锁实现。随后统一 `residency`、逐 pool device/provider、全池 unload、backend active/draining、generation fencing 与 admission token；不抽生命周期基类。旧调用保持兼容，但不具备完整契约者自报 `evictable=false`。CPU-capable 与 GPU-only 是显式 capability，不能从一次失败推断。
...
- `apps/api/alembic/versions/<next>_gpu_backend_fence.py`
...
P1–P6 待实施：ADR-0049 已 Accepted，但 Redis 账本、受管 residency / unload、静态 claim、observe/enforce、驱逐、多卡灰度均未开始。下一步进入 P1，并在此回填各阶段 commit、正式文档、预算校准、灰度结果与未尽事项。
