...
稳定 pool-id 绑定与 drain health 只读三态分类；durable cancel generation、authority 双域有界等待
和实物多卡验收仍未开启。
