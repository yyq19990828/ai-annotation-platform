...
剩余工作：P4b-II-B2b 把 dormant authority 注入 API/Celery 生产派发点并保持 off/observe 零 I/O；随后统一批量
失败记录，手工 unload 成功后立即对账，不等 60 秒。首个 enforce 子阶段
...
尚未注入生产调用点的 dormant Resident-only authority；cold admission、驱逐与 effective enforce 仍未接通。
