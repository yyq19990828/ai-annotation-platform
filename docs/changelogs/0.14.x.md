...
多 ML Backend 能力路由 · 一个项目注册多个后端时，工作台 AI 从「单 active 后端驱动一切」改成**按角色 + 能力路由**:交互工具(point/bbox/exemplar)自动路由到支持该 prompt 的交互后端，批量预标走批量后端，两条线**同时就绪**。重点解决「yolo 设为默认 → gsam2 交互失效」。纯前端。计划见 `docs/plans/archive/2026-06-09-v0.14.18-ml-backend-capability-routing.md`。
...
YOLO 预标可用性纵切 · 修通 YOLO 批量预标（此前必然 422）、暴露模型原生类别表供类别白名单勾选、采纳时可选项目类别。平台仍不做"模型类→项目标签"自动映射（NG6 保留，由 alias 配置 + 采纳人选承担）。计划见 `docs/plans/archive/2026-06-08-v0.14.16-preannotate-panel-capability-ux.md` §5。
...
AI 预标注面板深度优化（纯前端）·配置区常驻、按 backend 能力逐 model 变形、推理参数命名预设。计划见 `docs/plans/archive/2026-06-08-v0.14.16-preannotate-panel-capability-ux.md`。
...
- 新增 [ADR-0039](docs/adr/archive/0039-protocol-field-name-unification.md)，记录选择扁平 `model_variants` 与 role metadata 的原因；docs-site ADR 镜像同步更新。

## [0.14.14] - 2026-06-08

预标注可观测性 · 把 v0.14.13 的「前端 sessionStorage 猜测是否首次冷启动」换成「backend 真信号」。`PredictionResult` 加 `cache_hit / model_load_ms / pool_state` 三可选字段；`/health.pool` 三 backend 统一为 `PoolStatus`（cap / current_size / loaded_keys[] / last_evict）；新协议端点 `POST /warmup` 让前端 / 运维显式预热权重。计划见 `docs/plans/archive/2026-06-08-v0.14.14-predict-observability.md`。
...
预标注交互闭环 · 把 v0.14.12 在能力目录展示的 variant 富表达贯通到实际预标注链路，让用户在 AI 预标注页 / 工作台都能选 yolo `series/size` (或 gsam2 `sam_variant/dino_variant`、sam3 `model_variant`) 并持久化为项目级偏好。计划见 `docs/plans/archive/2026-06-08-v0.14.13-predict-ux-refinement.md`，配套 v0.14.14 / v0.14.15 路线在 `docs/plans/archive/2026-06-08-v0.14.14-predict-observability.md` / `2026-06-08-v0.14.15-protocol-field-unification.md`。
...
- v0.14.13 / v0.14.14 / v0.14.15 三份独立 plan 文档落地：
  - 本版主体 plan: `docs/plans/archive/2026-06-08-v0.14.13-predict-ux-refinement.md`
  - 下版可观测性: `docs/plans/archive/2026-06-08-v0.14.14-predict-observability.md` (cache_hit / pool 状态统一 / `/warmup` 端点)
  - 协议规范化: `docs/plans/archive/2026-06-08-v0.14.15-protocol-field-unification.md` (`model_variants` 扁平 dict / HTTP 422/503 / 删 `projects.ai_model`)

## [0.14.12] - 2026-06-08

接入第三个 ML backend：**yolo-backend**（ultralytics 多任务多系列）。计划见 `docs/plans/archive/2026-06-08-v0.14.12-yolo-backend.md`，抽象层决策见 [ADR-0038](docs/adr/archive/0038-defer-ml-backend-base-class.md)。本版交付：协议 v2 第一个「批量预标 backend」真实实例，covers detection / segmentation(instance) / keypoint / obb 四任务 × v8/v9/v10/v11/v12/v26/rt-detr 七系列预训练矩阵（共 80 个有效组合）。
...
- 新增 [ADR-0038 — ML backend 基类抽象推迟到 N≥4](docs/adr/archive/0038-defer-ml-backend-base-class.md)：写明本版只抽 schema + vocab，不抽 base class 的决策与未来触发条件。
- 计划文件 `docs/plans/archive/2026-06-08-v0.14.12-yolo-backend.md`：包含完整设计、80 组合矩阵核对、PR 拆分、验收清单、风险与回退。
...
协议能力目录与 ML Backend 注册解耦。计划见 `docs/plans/archive/2026-06-08-v0.14.11-protocol-capability-catalog.md`，决策见 [ADR-0037](docs/adr/archive/0037-protocol-capability-catalog-decoupling.md)。本版只解决一件事：「能力目录」从「已注册 backend 实例清单」抽离为「协议级能力定义 + 实例填充」双层视图，零接入用户也能完整看到平台支持的 9 类 AI 标注能力。
...
画布精细交互 Part A + 模型市场前端重构 Part B。计划见 `docs/plans/archive/2026-06-07-v0.14.10-canvas-precision-tools-and-attribute-mode.md`。
...
ML Backend 能力声明协议 v2(多模型目录 + infra)地基 + OCR / Doc Layout 首发模型族。计划见 `docs/plans/archive/2026-06-07-v0.14.9-model-capability-catalog-and-ocr-doclayout.md`。本版把能力建模从单模型快照升级到 model 粒度,并在协议 v2 之下落地 OCR / Doc Layout 输出约定;`/predict` 请求/响应 schema 不变,不新增 prediction 表。
...
Data Manager 保存视图 + 受控过滤 DSL + 只读项目任务运营面。计划见 `docs/plans/archive/2026-06-07-v0.14.8-data-manager-saved-views-filter-dsl.md`。本版只做视图、过滤、排序、列显隐与计数列,不做筛选结果批量写操作。
...
点云标注导出标准训练格式补丁。计划见 `docs/plans/archive/2026-06-07-v0.14.7-pointcloud-export-standard-formats.md`。本版为纯新增 serializer 和导出路由,不新增表/列/迁移。
...
6 相机实测体验 + 点云上色性能 + `point_mask_3d` 分割工具深化补丁。计划见 `docs/plans/archive/2026-06-07-v0.14.6-six-camera-experience-and-performance.md`。本版为纯前端切片,不新增后端表/列/端点。
...
3D 标注属性 + 标注效率补丁:补齐点云 3D 框属性编辑、多选批量操作、撤销/重做与复制/粘贴能力。计划见 `docs/plans/archive/2026-06-07-v0.14.5-3d-annotation-attributes-and-efficiency.md`。本版为纯前端切片,不新增后端表/列/端点。
...
scene 模式项目 + scene 感知分包补丁:把 scene 提升为项目级显式声明,补上 `by_scene` 分包、数据集 `has_scenes` 过滤与项目/数据集 kind 硬门。计划见 `docs/plans/archive/2026-06-06-v0.14.4-scene-mode-projects.md`。
...
点云导入鲁棒性 + 跨帧 UX 补丁:把 v0.14.2 真实 nuScenes 实测暴露的硬编码目录名、sensor/ego 坐标混用、跨 batch 跳转、sniffer 分歧不可见、overlay 档位偏小和 ZIP 多 scene 提示问题补齐。不新增表/列,不做数据迁移。计划见 `docs/plans/archive/2026-06-06-v0.14.3-import-robustness-and-crossframe-gaps.md`。
...
跨帧目标延续 UX:把 v0.14.0 的 scene + neighbors API 变成可用的标注效率特性。3D 工作台 `Shift+→` / `Shift+←` 一键把选中 box_3d 延续到同 scene 邻帧 task(共享 `group_id`),跳过去自动选中新框;三视图 / 主视图可叠加显示同 group_id 的前后 K 帧参考框。2D 图像序列同等用 `Alt+→` / `Alt+←`(2D 的 `Shift+方向` 已被 10px nudge 占用)。配套加 scheduler scene 连续标注调度开关。计划见 `docs/plans/archive/2026-06-05-v0.14.1-cross-frame-ux.md`。
...
跨 task 帧序列地基:`scenes` 模型 + `dataset_items.scene_id/frame_index` + neighbors API + 导入端口对齐 + manifest 透出。把 3D 点云逐帧 / 2D 抽帧序列 / 多段 mp4 拼接长录像统一到同一抽象,为 v0.14.1 跨帧 UX(`Shift+→` propagate / 邻帧叠加 / `useFrameNeighbors`)备好合法 backing。计划见 `docs/plans/archive/2026-06-05-v0.14.0-scene-and-frame-index-foundation.md`。
