...
- 实现代码位置:
  ...
  - 旧视频 SVG/DOM 栈已于 v0.16.5(2026-06-17)删除:`VideoStage` / `VideoFrameOverlay` / `VideoObjectsLayer` / `VideoTrackShape` / `VideoTextLayer` / `VideoInteractionLayer` / `VideoIssueLayer` / `VideoMediaLayer` / `VideoGridLayer` / `VideoAttachmentLayer` / `VideoBitmapLayer` / `VideoStageSurface` / `VideoSelectionActions` / `videoStageCoordinates` / `useChunkSamples` / `videoChunkDemux` + `videoKonvaFlag` + `experiment.videoKonva` 设置项一并移除;`VideoWorkbench` 无条件渲染 `VideoKonvaStage`,无 flag 分支。
  - v0.16.5 删栈前的对等补齐:切默认观察期遗留三处 Konva 栈缺口,删栈时一并补上——① 光标坐标上报(`onCursorMove` → 状态栏读出);② 本地视口/导航快捷键 `F`(fit)/`0`(实际尺寸)/`Home`·`End`(选中轨迹首末出现帧);③ issue 图钉点击(`VideoKonvaIssueLayer` 接 `onPinClick`,pointerdown `cancelBubble` 防误触发画框)。
