...
- 实现代码位置:
  ...
  - 视频 Konva 交互(v0.16.3):`videoKonvaInteraction.ts`(`useVideoKonvaInteraction` + `advanceDrag`/`resolveDragCommit` 纯函数)/ `VideoKonvaInteractionLayer.tsx`(句柄 + live 预览);命中复用 `videoStagePicking.ts`(泛型约束放宽到 `{ geom }`)、缩放复用 `ResizeHandles.applyResize`。**右键上下文菜单尚未迁**(旧栈深耦 `trackActions`/`contextMenu` 内联逻辑,留焦点跟进,见 v0.16.3 计划 §6)
  - 待迁/待删旧视频栈:`VideoObjectsLayer.tsx` / `VideoTrackShape.tsx` / `VideoTextLayer.tsx` / `VideoInteractionLayer.tsx` / `VideoIssueLayer.tsx` / `VideoMediaLayer.tsx` / `videoStageCoordinates.ts` 在 v0.16.5 删栈时移除
