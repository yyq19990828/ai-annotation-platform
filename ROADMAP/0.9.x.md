...
> 状态：**M0+M1 已落地**（M0 `01f8d45` / 2026-05-07；M1 v0.9.1 / 2026-05-07），M2-M5 进行中。后续 v0.10.x 在此基础上**新增**（不替换）SAM 3 backend，详见 [`0.10.x.md`](./0.10.x.md)。
>
...
### v0.9.2 — 工作台 `S` 工具 + 文本入口（M2，~6 工作日）

- [ ] 工作台新增工具 `S`（SAM 模式）：点击=positive point / Alt+点击=negative point / 拖框=bbox
- [ ] 右侧栏 AI 助手新增「文本提示」输入框 + 「找全图」按钮（仅 `S` 模式可见）
- [ ] `useInteractiveAI` hook：按 prompt 类型路由
- [ ] 中→英翻译开关（默认开，复用平台 LLM client；用户可关）
- [ ] 返回 polygon 以「待确认紫虚线」叠加在 Konva canvas，`Enter` 接受 / `Esc` 取消 / `Tab` 切候选
- [ ] 防抖：连续点击 80-120ms 合并请求
- [ ] 单测：`useInteractiveAI` 各 prompt 类型路由 + backend 失败 toast
- [ ] E2E：`annotation.spec` 加 SAM 路径

**验收**：标注员用 SAM 工具 10 分钟内标完 50 张图，AI 接管率 ≥ 60%；文本 "person" 在 3 张含人图都能返回正确 polygon。
