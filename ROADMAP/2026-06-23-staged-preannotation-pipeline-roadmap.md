...
> 状态：**Backlog / 0.18.x 序列**。本文件是「路径 B（平台层通用二阶段编排）」的规划草案，待「路径 A（backend 内部级联，v0.18.0）」落地验证后启动。路径 B 全部里程碑落在 **0.18.x patch 序列**（v0.18.1 → v0.18.3），不开新 minor。
>
> 创建于 2026-06-23。本轮细化目标：把要**提前对齐**的几个决策（尤其编排界面形态）拍板，避免 v0.18.0 的产出走偏。
