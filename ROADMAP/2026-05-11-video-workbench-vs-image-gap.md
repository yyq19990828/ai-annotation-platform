...
>
> 范围：以当前已落地的 [VideoStage](../apps/web/src/pages/Workbench/stage/VideoStage.tsx)（v0.9.16 + v0.9.17 + v0.9.18）为基线，与 [ImageStage](../apps/web/src/pages/Workbench/stage/ImageStage.tsx) 做逐项对比，沉淀下一阶段的优化方向。
>
...
- VideoStage 当前约 **700+ 行**，ImageStage **1100+ 行**。规模差距背后的核心是：VideoStage 是独立重写的 MVP，仍缺少多项 ImageStage 的工程基础设施（撤销重做、离线队列验证、冲突 diff、多选、属性面板、复制粘贴、AI 采纳/驳回、Minimap、浮动 Dock、Review diff 模式）。v0.9.18 已先补上快捷键中心化。
...
- 视频侧 M3 已落地 QC（轨迹断裂、极小框、同帧重叠），v0.9.18 已落地 M4 Video Tracks JSON 导出和 O1 快捷键中心化。当前最影响日常标注效率的剩余短板是 **track-aware 撤销重做 + offline/conflict 覆盖验证**。
...
| **快捷键** | 工具切换 B/P/V/S | ✅ | —（视频禁用图片绘制工具） | hotkeys.ts |
| | 1–9 切类别 | ✅ | ✅ | hotkeys.ts |
...
- **状态**：v0.9.18 已落地。
- **已完成**：在 `hotkeys.ts` 增加 video 命名空间；`useWorkbenchHotkeys` 通过 `videoMode` 分发；`VideoStage` 移除内部 keydown listener，改用 ref 暴露播放 / 逐帧控制。
- **已覆盖**：Space、←/→、Shift+←/→、Delete / Backspace、Tab / Shift+Tab、Esc、1–9；帮助面板新增“视频”分组。
- **后续注意**：如果后续给视频加入 bbox nudge，应另选 modifier（如 Alt+方向键），不要复用无修饰方向键。

#### O2 · 撤销重做（track-aware）
- **状态**：v0.9.19 已落地 keyframe 级 history。
- **痛点**：当前 shell 层 `history.push` 在 video 任务里只记录 annotation 级别的操作，**关键帧增删改不进 history**——用户改 3 帧后无法回滚。
- **已完成**：新增 `videoKeyframe` history command，单个 frame 的关键帧新增 / 修改 / 删除 / `absent` / `occluded` 切换只回滚该 frame，不吞掉整条 track；创建 / 删除 / 改类别仍复用 annotation 级 history。
- **工程量**：M。**影响**：标注员信心 / 误操作恢复。**依赖**：先确认 history 是否需要按"track 维度"组织。

#### O3 · 验证 / 修复 offline queue 与 conflict modal 在视频上的覆盖
- **状态**：v0.9.19 已补齐 create / update / rename 的离线队列 fallback；冲突 UI 第一版复用通用 `ConflictModal`。
...
- **已完成**：网络断开 / 5xx 时视频创建、关键帧更新和轨迹重命名进入现有 offline queue；409 版本冲突不进入离线暂存，继续唤起通用冲突弹窗。
- **工程量**：S（如果不需要改实现）/ M（如果发现 video_track 的 diff 渲染需要单独写）。**影响**：可靠性。
- **剩余风险**：ConflictModal 当前仍无 video keyframe diff；如需要展示两端关键帧差异，应作为 O3 后续增强独立设计。
...
#### O9 · M4 视频导出（独立里程碑）
- **状态**：v0.9.18 已落地 M4 首版。
- **已完成**：
  - `video-track` 项目复用 `format=coco` 兼容入口返回专用 Video Tracks JSON（`export_type="video_tracks"`），不是 COCO。
  - 支持 `video_frame_mode=keyframes|all_frames`，默认 `keyframes`。
  - `all_frames` 在后端按相邻有效关键帧线性插值；`absent=true` 阻断跨段插值；缺少 `frame_count` 时用最大已标注帧兜底。
  - `include_attributes=false` 会移除 attribute schema 和 annotation attributes。
  - video `format=yolo|voc` 返回 400，避免丢失 track 语义。
- **仍未做**：MOT Challenge / COCO Video 等行业格式转换；如需要，应作为新导出格式单独评估。
...
```
M5.1（基础设施）: O2（撤销重做）→ O3（offline/conflict 验证）
  ↓
...
- O1 已在 v0.9.18 落地；O2 是其它编辑增强的前提（没有撤销，用户不敢做新操作）。
- O3 不在表面但风险最大（数据丢失风险）。
- O5–O8 是用户能直接感受到的提速。
- O9 首版已在 v0.9.18 落地；后续行业格式转换应另列导出增强。
