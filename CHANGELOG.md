...
- **CSP style-src sections 群续推**：`pages/Projects/sections/*.tsx` 已清空 inline style，`eslint.config.js` 文件级 `no-restricted-syntax` guard 从枚举文件收口为 `src/pages/Projects/sections/*.tsx` glob；`BatchesSection` 原 1 处 CSS custom property hatch 改为三档 class 切换，也已清零。`TextOutputDefaultSelect` 补 `className` 透传，避免 GeneralSection 继续用 inline 传入控件样式。全站剩余约 2543 处 inline，下一批优先 `Workbench` / `Dashboard` / 共享 UI。→ [plan](docs/plans/2026-05-18-v0.10.12-csp-style-src-sections.md) · [ROADMAP §B](ROADMAP.md) · [迁移指南](docs-site/dev/how-to/migrate-inline-style-to-css-modules.md)
