...
- **视频对象轨迹支持 polygon 几何（后端底座）**：新增 `video_track_polygon` 轨迹类型，关键帧存归一化多边形顶点，帧间按
  弧长参数化插值（顶点数不等时先重采样到公共参数化 + 旋转对齐，减少形变扭曲）。与既有 `video_track_bbox` 平行、存量
  bbox 轨迹零迁移。（画布绘制/渲染的前端接入随后落地。）
