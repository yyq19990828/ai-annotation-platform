...
- **视频 AI 追踪打通「点种子 / 多目标种子」透传（`sam3_video_interactive`）**：PVS 交互追踪此前平台只能把选中标注的**框**当种子下发（`source_geometry`）；现在追踪请求可在 `prompt.seeds[]` 携带**点种子**（`{obj_id, points:[[x,y,label],...]}`，正/负点、归一化坐标）或**同时圈定多个目标各自的种子**，runner 只在种子窗（含原始种子帧的首窗）透传、后续窗仍靠上一窗末帧框跨窗续追（点锚在种子帧、不重发）。点种子 → PVS 逐帧记忆追随已真机验证（16 帧稳定跟随、框随目标缩放）。画布上「落点选目标 / 多点多目标」的前端交互入口随后续版本落地，本次为其打通后端链路。
