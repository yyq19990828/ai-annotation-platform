...
- **视频单帧/轨迹独立开关**: 后端 `ToolBinding` 新增 `video_modes: {box, track}`（仅视频 `bbox` 单位消费，`None` = 两者均可用，向后兼容）。项目设置「类别与属性」页在视频 `bbox` 单位下渲染两个开关，约束「至少保留一个可用」；工作台 `ToolDock` 据此过滤 `box`/`track` 工具。类别与属性绑定仍只有一套（`tool_bindings["bbox"]`），单帧框与轨迹框共享。
- **视频 hand 平移工具**: `VideoTool` 新增 `hand`（中立态），左键拖拽平移画布（复用右键 pan 路径），快捷键 `V` / `Alt+3`，与图片工作台一致；恒显示、不受 `video_modes` 过滤，作为开关全关时的兜底。`ToolDock` 视频工具栏按「单帧 / 轨迹 / 视图」分组加分隔线。
- **ESC 回归 hand**: 视频模式按 ESC 在取消当前草稿 / 选中的同时切回 `hand` 中立态。

### Changed

- **几何 type 命名规范化**: 轨迹矩形的 `annotation_type` / `geometry.type` 值 `video_track` → `video_track_bbox`（单帧 `video_bbox` 已符合 `video_<几何>` 形态、不变），为未来 `video_polygon` / `video_track_polygon` 留位。前后端字面量同步替换 + codegen；migration `0089` 搬迁存量 `annotation_type` 列与 `geometry->>'type'` 字段（可逆）。「轨迹」领域操作 / AI 任务标识符（`video_tracker_job`、`propagate_video_track` 等）不受影响。
- **视频导航快捷键收敛**: `←` / `→` 只负责帧导航（采样开启时按网格跳），`Shift+←/→` 仅作为采样开启时源帧 ±1 微调，`,` / `.` 只在选中轨迹时跳上 / 下关键帧；移除 `Alt+←/→` 关键帧跳转别名，减少同一组键在采样 / 选中状态下的多重含义。**行为变更**：旧版非采样模式下 `Shift+←/→` 的「±10 帧加速」与 `,` / `.` 在未选中轨迹时的「±1 帧」已一并移除，仅保留上述两类心智模型。
