...
- **核心文档图可以下载后继续编辑**. 系统全景、JWT 注销、视频 Chunk、Batch/Task 生命周期、开发与生产基础设施、部署演进、生产网络与持久化边界、审计通知、邀请注册、标注任务主链、AI 预标注与持久化作业、实时通知、视频追踪人机闭环，以及 Project、Batch、Task、Annotation、Review 模块全景改用内嵌 Excalidraw scene 与手写字体的 SVG；重复状态图、预标数据流、生产网络边界、通知可靠性边界和追踪审阅闭环共享 canonical 资产，文档站仍提供点击放大和可编辑源文件下载入口。迁移验收同时按当前 Compose、API 与前端调用链修正 worker、监控、邀请、任务提交、服务池绑定与物理实例路由、Celery / AsyncJob ID 边界、预标可见性、工作台取题、标注计数回写、审核副作用、生产端口绑定、对象存储的双网络视角、Redis 恢复风险、通知事务窗口与专用重连，以及视频追踪局部审阅和取消后部分候选等文档漂移。
