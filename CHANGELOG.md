...
- **GPU enforce 灰度获得持久逐资源 rollout 地基**：每张物理卡以 PostgreSQL 冻结 `off/promoting/enforcing/demoting/blocked` 状态、保守 effective mode、目标模式和 exact transition identity；数据库触发器拒绝跳态、漏 revision 与删除，完成回执支持响应丢失重放，并发 begin 只生成一个 transition。默认关闭的 release latch 同时禁止 worker enforce repair 并保持 effective-off；开启后 health worker 先持久进入 `promoting`，仅在 Backend gate 与 Redis ready 全部证明后以同一 transition 提交 `enforcing`，不就绪则落 `blocked/off`。业务派发在 Backend HTTP 前读取该真值，对缺行、过渡态、未知资源越界与数据库失联 fail-closed，而显式 CPU backend 不承担 GPU rollout 数据库依赖。资源管理端与超管 backend 列表现在同时展示持久 rollout、解析后 effective mode 和 Redis runtime，并对持久活跃期误关 release latch 给出精确 blocker。自动 demotion 尚未接线，生产 release latch 继续关闭。
