...
- **Data Manager 改为单视口数据工作台**：页面仅保留任务 / 对象 / 轨迹一层粒度切换，压缩项目标题与当前范围摘要；完整聚合移入右侧统计抽屉，搜索、排序、列设置、快捷筛选、可编辑条件芯片和可搜索字段选择器集中在结果工具栏。桌面端保留视图侧栏，窄屏改用视图下拉，整页不再随结果变成长页面，滚动只发生在结果表格和抽屉内。
