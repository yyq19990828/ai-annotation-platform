...
- **邻帧点云叠加**(3D 点云工作台,设置项 `点云 › 邻帧点云叠加`,默认关):开启后把前后各 ≤3 帧的点云用 ego 运动补偿(与叠框同一 `inv(T_当前)·T_邻帧` 刚体变换,作为点云对象矩阵在 GPU 端施加,无逐点 CPU 开销)对齐到当前帧车体系再渲染。**静止背景重合加密**(利于远处/稀疏处标注),**动态目标留沿运动方向的拖影**(时序运动可视化)。邻帧点用弱化冷灰蓝单色 + 低透明 + 略小点,与当前帧的高度色带 / 相机上色强区分。
...
- 只做**可视化叠加**辅助观察,不改标注落点、不在合并点云上画框;不做动态点剔除 / Kalman / 多 lidar(留后)。
